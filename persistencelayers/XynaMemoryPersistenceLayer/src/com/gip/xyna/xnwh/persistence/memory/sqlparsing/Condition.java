...
public class Condition {
  ...
  private static Condition convertExpressionIntoCondition2(String sqlString, Expression e) {
    if (e instanceof Comparison) {
      ...
      if (!(((Not) e).subExpression instanceof AndOrExpressions)) {
        //FIXME nur aus abw�rtskompatibilit�tsgr�nden zwischenschicht
  ...
  private class JavaGenContext {
    ...
    public int getNextParameterIndex() {
      return parameterIndex++; // vorherigen index zur�ckgeben
    }
  ...
  private final static Pattern findCharsToReplaceForStringLiteral = Pattern.compile("([\\\\\"])"); //backslashes und anf�hrungszeichen
  ...
  private String exprToJava(String expr, JavaGenContext context, String otherExpr, boolean escapeExprForLikeStatement)
    ...
    if (expr.startsWith("'")) {
       ...
       *    \"a (generierter javacode ben�tigt anf�hrungszeichen escaped)
       * \\\"a%     -> es wird nach strings, die mit \"a anfangen gesucht
       *    \\\\\"a
       * \\\\\"a%   -> es wird nach strings, die mit \\"a anfangen gesucht
       *    \\\\\\\\\"a
       *    
       * f�r string m�ssen anf�hrungszeichen und backslashes jeweils 1x escaped werden
       * f�r regexp m�ssen backslashes 1x escaped werden
       * 
       */
  ...
  private Map<String, String> generateWhereConditionBooleans(JavaGenContext context,
    ...
    if (conditions.size() == 0) {
      ...
      if (likeOperator && localConditionOperatorContainsQuotationMarks) {
        // weil der javaoperator ... + "").matches( keine �ffnende klammer hat; f�r Strings ist der Operator aber nur
