  ...
  private TraversalContinuation traverseTree(AbstractNode<E> node, TraversalCondition<E> condition, Handler<E> handler,
    ...
    try {
      loop : for (int i = 0; i < node.getNumberOfChildren(); i++) {
        ...
        if (checkCondition) {
          //muss ggfs nicht mehrfach ausgef�hrt werden (vgl RECURSE_ALL)
          result = condition.check(child, false);
        }
        if (result.isHit()) {
      /*    if (lockedNode != null) {
            if (handler.needsWriteLock()) {
              //lock upgrade nicht m�glich
              lockedNode.getLock().readLock().unlock();
              lockedNode.getLock().writeLock().lock();
              //TODO:
              //checken, dass die kinder dieses knotens nicht ge�ndert wurden
              //das kann passieren, wenn zb ein rebalance thread das writelock zuerst bekommt
              //dann ist man nun evtl an einer ganz anderen stelle im baum (sowohl child als auch parent k�nnen im baum ihre position ge�ndert haben)
              //idee 1: 
              //=> solange zum parent zur�ck, bis parent sich nicht ge�ndert hat.
              //von dort aus dann traverseTree erneut aufrufen
              //frage: was passiert mit bereits behandelten knoten (handler ausgef�hrt), die dann erneut gecheckt werden?
              //antwort: needsWriteLock UseCase => entfernen und/oder hinzuf�gen von knoten im baum. dann darf man handler nicht erneut ausf�hren
              
              //idee 2:
              //=> bei dem knoten weitermachen, auch wenn der knoten nun an einem anderen parent h�ngt, weil die position im baum
              //nicht so wichtig ist, wie die position in der sortierreihenfolge (diese ist aber gleich geblieben, weil der wert
              //des knotens sich nicht �ndern kann).
              //dann muss man bei der rekursion an den parent-aufruf den neuen parentknoten zur�ckgeben, weil sich dieser ge�ndert haben kann.
              //ausserdem muss die stacktiefe nicht mehr mit der tiefe des aktuellen knotens im baum �bereinstimmen. => kompliziert.
          ...
          //FIXME beim einf�gen oder delete anderen returnwert, weil hier die liste der kinder evtl ge�ndert wird? (removenode oder addnode)
        ...
        switch (result.recursionRequest()) {
          case RECURSE_IF_NEXTSIBLING_ALL_RECURSION :
            ...
            if (i < node.getNumberOfChildren() - 1) {
              ...
              AbstractNode<E> nextChild = node.getChild(i + 1); //FIXME performance: kann in n�chstem schleifendurchlauf wiederverwendet werden
          ...
          case RECURSE_CHILD_CHECK_NEXT :
            //rekursion f�r dieses kind, f�r das n�chste erneut checken: geoderte bedingungen
            continuation = traverseTree(child, condition, handler, null);
            if (continuation == TraversalContinuation.BREAK) {
              //z.b. maxrows wurde in rekursion erreicht
              return continuation;
          ...
          case NO_RECURSION_CHECK_NEXT :
            //keine rekursion f�r dieses kind, f�r das n�chste erneut checken: geoderte bedingungen
            break;
          case RECURSE_ALL :
            //f�r dieses und alle folgenden kinder rekursion (entspricht suchen nach > bedingung, wenn f�r alle folgenden kinder klar ist, was passieren muss)
          ...
          case RECURSE_CHILD_ONLY :
            //nur f�r das kind rekursion, und nicht f�r folgende (entspricht suchen nach knoten mit gleichheits-bedingung)    
            continuation = traverseTree(child, condition, handler, null);
            if (continuation == TraversalContinuation.BREAK) {
              //z.b. maxrows wurde in rekursion erreicht
              return continuation;
