...
public class Rebalancer<E extends HasSize> {
  ...
  public static int cnt;//TODO kann raus, ist nur f�r tests
    ...
    try {
      ...
      while (node.isTooBig()) {
        ...
        if (size == 1) {
          ...
          //gib die h�chsten size/2 knoten an den parent
      ...
      while (node.hasTooManyChildren()) {
        ...
        for (int i = indexForFirstMergingCandidate; i < size - 1; i++) {
          ...
          //TODO performance: (i+1)tes kind jetzt ist in n�chster iteration das i-te kind => muss man nicht erneut auslesen.
          if (childToMergeTo.getSubTreeSize() + node.getChild(i + 1).getSubTreeSize() <= childToMergeTo
            ...
            //i < i+1. d.h. erst removed anh�ngen, dann all seine kinder.
            ...
            //kein merging m�glich => beim n�chsten schleifendurchlauf muss man das nicht nochmal probieren
  ...
  /**
   * validieren, dass der knoten vom richtigen typ ist. falls nicht, wird der typ angepasst und dann �ber die kinder
  ...
  private LockedOrderedNode<?> transformParentLocked(AbstractNode<E> node, NodeTypeTransformer<E> nodeTypeTransformer,
    ...
    try {
      ...
      if (!node.isInvalid()) {
        //parentlock h�lt parentmethode. nodelock h�lt diese methode
  ...
  //FIXME performance: synchronize �ber cluster hinweg, damit knoten nicht mehrfach rebalanced werden.
  /*
   * zahl ist die anzahl der �nderungen an diesem knoten. 
   */
    ...
    try {
     /* Integer rootval = nodesToRebalance.get(root);
      ...
      if (nodesToRemove.size() > 100) {
        //leere knoten entfernen. dazu muss man nochmal isEmpty() checken, weil sich der leer-status des knotens inzwischen ge�ndert haben k�nnte
