...
public abstract class MemoryPersistenceLayerConnection implements PersistenceLayerConnection {
  ...
  private static class BLA extends Storable { //FIXME workaround um ant-compiler zum compilieren zu �berlisten.
  ...
  protected final void defaultCommit(TransactionCache transactionInformation) throws PersistenceLayerException {
    /*
     * ge�nderte zeilen haben zeilenlocks (sustained lock).
     ...
     * 
     * usecases:
     * - gleichzeitige �nderung oder queries for update: gesch�tzt durch zeilenlocks (sustained lock)
     * - gleichzeitige queries NOT for update: �berpr�fung des temporary locks muss verhindern, dass gleichzeitig die daten rausgegeben werden. 
     * 
     * problem: gleichzeitige query hat index-readlock, bevor es versucht, auf zeilen zuzugreifen und daf�r zeilen-readlocks holt.
     *          -> ist in preparedqueryformemory durch eine "assumeddeadlockexception" gel�st
     */
    ...
    if (transactionCacheData != null && transactionCacheData.size() > 0) {
      for (TransactionCacheTable<?> m : transactionCacheData) {
        ...
        final List<Lock> temporaryLocks = new ArrayList<Lock>(); //TODO ben�tigt man beim delete die locks auch?
        try {
  ...
  public final <E> E queryOneRow(PreparedQuery<E> query, Parameter parameter) throws PersistenceLayerException {
    ...
    TableObject<Storable, MemoryRowData<Storable>> t = checkTable(table);
    // FIXME "forUpdate" korrekt ermitteln und ber�cksichtigen
    ...
    if (queryForMemory instanceof PreparedCountQueryForMemory) {
      // FIXME count result unter Ber�cksichtigung der Transaktions-Daten updaten
    }
    return result;
