...
public class OptionsAction implements FilterAction {
  ...
  public void setAccessControlParameter(HTTPTriggerConnection tc, DefaultFilterActionInstance dfai) {
    ...
    //erlaube einfach das was angefragt wird. achtung: der browser k�nnte die antwort cachen. sch�ner w�re es, wenn man hier alles angibt
    String allow_method = tc.getHeader().getProperty("access-control-request-method");
    String allow_headers = tc.getHeader().getProperty("access-control-request-headers");
  ...
  /*
   * - falls der im request angegebene origin erlaubt ist, wird er in Access-Control-Allow-Origin zur�ckgegeben
   * - falls er nicht erlaubt ist
   *   - wird der erste in der property angegebene origin zur�ckgegeben (oder "*" falls vorhanden)
   *   - falls die property leer ist, wird versucht RMI_HOSTNAME_REGISTRY zur�ckgegeben
   ...
   * 
   * ein origin ist erlaubt, falls er in der xynaproperty als einer der kommaseparierten werte enthalten ist oder wenn die xynaproperty "*" enth�lt
   * oder wenn die property leer ist UND wenn der origin er dem RMI_HOSTNAME_REGISTRY wert entspricht und dieser keine IP oder localhost enth�lt.
