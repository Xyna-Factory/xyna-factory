...
public class ServiceGroupsPathNameObjectsIdRefactorAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.SERVICES_GROUPS;
  
  public ServiceGroupsPathNameObjectsIdRefactorAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, 5, Operation.Refactor, Type.serviceGroup, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 7
        && url.getPathElement(6).equals(PathElements.REFACTOR)
        && method == Method.POST;
  }

  @Override
  public String getTitle() {
    return "XMOM-Servicegroups-TypePath-TypeName-Objects-Id-Refactor";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5.data/Simple/deploy", "h5.data.Simple");
  }
