 ...
 *
 */
public class ServiceGroupsPathNameCloseAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.SERVICES_GROUPS;
  
  public ServiceGroupsPathNameCloseAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Close, Type.serviceGroup, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 5 
        && url.getPathElement(4).equals(PathElements.CLOSE)
        && method == Method.POST;
  }

  @Override
  public String getTitle() {
    return "XMOM-Servicegroups-TypePath-TypeName-Close";
  }
