 ...
 *
 */
public class ObjectsPathAction extends RuntimeContextDependendAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.OBJECTS;
  private final XMOMLoader xmomLoader;
  
  public ObjectsPathAction(XMOMLoader xmomLoader) {
    super();
  ...
  private FilterActionInstance listPath(RuntimeContext rc, Long revision, URLPath url, HTTPTriggerConnection tc) throws SocketNotAvailableException {
    ...
    boolean addAnyType = false;
    
    try {
      String path = url.getPathElement(2);
      if(path.startsWith("xprc.base")) {
        path = path.replaceFirst("xprc\\.base", "base");
        if(path.equals("base")) {
          addAnyType = true;
        }
      }
      ...
      if(addAnyType) {
        ObjectIdentifierJson anyType = Utils.createAnyTypeIdentifier(revision);
        xmomObjectsPath.getObjects().add(anyType);
        Collections.sort(xmomObjectsPath.getObjects(), sortType.getComparator());
      }
