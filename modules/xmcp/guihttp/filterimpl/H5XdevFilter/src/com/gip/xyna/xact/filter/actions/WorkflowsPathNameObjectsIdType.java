 ...
 *
 */
public class WorkflowsPathNameObjectsIdType extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.WORKFLOWS;
  
  public WorkflowsPathNameObjectsIdType(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, 5, Operation.Type, Type.workflow, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 7 
        && url.getPathElement(4).equals(PathElements.OBJECT)
        && url.getPathElement(6).equals(PathElements.TYPE)
        && method == Method.PUT;
  }

  @Override
  public String getTitle() {
    return "XMOM-Workflows-TypePath-TypeName-Objects-Id-Type";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5/TestPrototype/objects//type", "h5.TestPrototype");
  }
