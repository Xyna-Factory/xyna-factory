...
public class DatatypesPathNameReplaceAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.DATA_TYPES;
  
  public DatatypesPathNameReplaceAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Replace, Type.dataType, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.REPLACE)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Datatypes-TypePath-TypeName-Replace";
  }
