 ...
 *
 */
public class WorkflowsPathNameInsertAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.WORKFLOWS;
  
  public WorkflowsPathNameInsertAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, 5, Operation.Insert, Type.workflow, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        ...
        && url.getPathElement(6).equals(PathElements.INSERT)
        && method == Method.POST;
  }

  @Override
  public String getTitle() {
    return "XMOM-Workflows-TypePath-TypeName-Insert";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/lw/TestJsonWFCall/insert", "lw.testJsonWFCall"); //FIXME
  }
