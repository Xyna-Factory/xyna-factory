...
public class ExceptionsPathNameDeployAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.EXCEPTION_TYPES;
  
  public ExceptionsPathNameDeployAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Deploy, Type.exceptionType, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.DEPLOY)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Exceptiontypes-TypePath-TypeName-Deploy";
  }
