 ...
 *
 */
public class WorkflowsPathNameClipboardIndexPaste extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.WORKFLOWS;
  
  public WorkflowsPathNameClipboardIndexPaste(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, 5, Operation.PasteFromClipboard, Type.workflow, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 7 
        && url.getPathElement(4).equals(PathElements.CLIPBOARD)
        && url.getPathElement(6).equals(PathElements.PASTE)
        && method == Method.POST;
  }

  @Override
  public String getTitle() {
    return "Clipboard-Index-PasteFrom";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5/TestSerial/objects/pasteFromClipboard", "h5.TestSerial"); //FIXME
  }
