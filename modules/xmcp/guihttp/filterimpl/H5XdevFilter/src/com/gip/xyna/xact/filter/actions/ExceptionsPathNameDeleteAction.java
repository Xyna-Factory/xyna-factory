...
public class ExceptionsPathNameDeleteAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.EXCEPTION_TYPES;
  
  public ExceptionsPathNameDeleteAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.DeleteDocument, Type.exceptionType, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.DELETE)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Exceptiontypes-TypePath-TypeName-Delete";
  }
