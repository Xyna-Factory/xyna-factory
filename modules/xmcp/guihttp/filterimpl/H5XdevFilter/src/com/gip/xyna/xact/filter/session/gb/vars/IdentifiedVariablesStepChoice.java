...
public class IdentifiedVariablesStepChoice extends IdentifiedVariablesStep implements InputConnectionProvider {
  ...
  //wird vom autosnapping (->dataflow) bef�llt
  ...
  private void createChoiceOutput() {
    ...
    /*
     * outputs f�r die common outputs erstellen oder wiederverwenden.
     ...
     * 
     * dazu alle ehemaligen outputs ber�cksichtigen
     */
  ...
  private AVariable getPreviousCommonOutput(VarType commonOutputType) {
    ...
    for (int i = 0; i < stepChoice.getChildSteps().size(); i++) {
      ...
      for (AVariable avar : assign.getOutputVars()) {
        //achtung, nicht die useroutputs zur�ckgeben
  ...
  protected void setFlags(AVariableIdentification var, VarUsageType usage) {
    var.setConstPermission(ConstPermission.FOR_BRANCHES); // TODO: Ist das immer korrekt? Besser pr�fen, ob var in getUserOutputs() ist, oder?
    if (stepChoice.getDistinctionType() == DistinctionType.TypeChoice) {
   ...
   * (A11) + (A2) => (A)
   * (A11,A2) + (A2) => (A)
   *   (A2) w�re auch denkbar, aber lieber eine ambiguit�t mehr als einen fall, wo man manuellen output ben�tigt
   * (A11,A21) + (A12,A3) => (A1,A)
   * 
   */
  ...
  public void addPossibleOutput(List<AVariableIdentification> singleLaneOutput, int branchIndex) {
    if (singleLaneOutput == null) {
      return; //lane f�r berechnung von common output ignorieren (lane endet mit throw)
    }
    ...
    /*
    * Algorithmus:
    * f�r jeden typ aus Source1: (sortierung nach hierarchie-tiefe)
    ...
    *       goto 1 (sortierung ber�cksichtigen!)
     */
