...
public class QueryUtils {
  ...
  private static String createHelperMappingExpression(List<String> filterConditions, StepMapping stepMapping) {
     ...
     * Der Index ist abh�ngig von der Anzahl der Input-Variablen.
     */
    ...
    /*
     * Das Mapping erzeugt eine FilterCondition, die sp�ter der Input der Query-Function wird.
     * Die FilterCondition bekommt den Member formula mit der Expression gesetzt, welche die eigentliche FilterBedingung ist.
     * Eventuell vorhandene dynamische Teile (Variablen) werden aufgel�st.
     * Dies ist notwendig, da die Query-Function dies selbst nicht kann. Die Query-Function kennt nur %0%, was das eigene Storable repr�sentiert.
    ...
    for (int i = 0; i < filterConditions.size(); i++) {
      ...
      if (isDynamicExpression(condition)) {
        ...
        // Bei gemischten Expressions (dynamische und statische Inhalte) muss zus�tzlich noch nicht gequotete doppelte Anf�hrungszeichen entfernt werden.
  ...
  /**
   * Ermittelt ob die Expression einen dynamischen Teil enth�lt.
  ...
  /**
   * Erzeugt mit Hilfe des XFLLexer die Expression, welche den dynmischen Teil aufl�st und quoted.
   * @param expression
   * @return
   */
...
//            || lexem2.getType() == TokenType.ARGUMENT_SEPERATOR) {
  ...
  /**
   * Erzeugt den Teil der Expression, der Backslashes und doppelte Anf�hrungszeichen quoted.
   * Das Quoten erfolgt mit Hilfe von replaceall.
   * Die vielen Backslashes entstehen dadurch, dass hier bereits f�r die sp�ter erzeugte Java-Klasse gequoted werden muss.
   * Ben�tigt man in der Regex von replaceall einen Backslash zum Quoten, so braucht man im XML 4 Backslashes.
   ...
   *    replaceall(%0%.name,"\\\\","\\\\\\\\")
   *    
   * 2. Doppeltes Anf�hrungszeichen quoten
  ...
  private static String prepareDynamicPart(String part) {
    ...
    * Dadurch verschieben sich alle Indexe. Dies wird hier wieder f�r das Mapping korrigiert
    * */
  ...
  public static List<String> extractQueryFormulas(String input, int inputVarCount) {
    ...
    /*
     * Die GUI bekommt als erste Variable das Storable (Fakevariable). Dadurch m�ssen f�r die GUI alle Variablenindexe bei dynamischen FilterConditions um 1 erh�ht werden.
     * Dies ben�tigt die GUI um die einzelnen Member des Storable ermitteln und darstellen zu k�nnen.
     */
     ...
     *  - Dynmische Variablen m�ssen vor dem Aufruf des Querys entfernt werden, da das Query keine Variablen auswerten kann.
     *  - Eventuell vorhandene Backslashes und einfache Anf�hrungszeichen m�ssen entsprechend gequoted werden.
     *    Dies erfolgt durch den zweimaligen Aufruf von replaceall. Da das replaceall bereits f�r eine Java-Klasse gequoted ist, gibt es entsprechend viele Backslashes.
     *  - Alle dynamischen Variablen in den Query-Conditions m�ssen Input-Variablen des Mappings sein.
     *  - Das Mapping erzeugt ein FilterCondition-Object, welches sp�ter die erste Input-Variable der Query-Function ist. %0% innerhalb des concat steht f�r das Storable.
     * 
     */

    // nicht ben�tigte Teile aus der Expression entfernen
    
    /*
     * Erster Teil des Mappings entfernen. ( bis zum concat(" )
     * Hier ist die Besonderheit, dass die erste Variable des Mappings abh�ngig von der Anzahl der Input-Variablen ist.
     * Die erzeugte FilterCondition ist die erste Output-Variable des Mappings und bekommt somit die n�chste fortlaufende Id nach den Input-Variablen.
    ...
    for (String formula : formulas) {
      ...
      if(isFormulaDynamic(formula)) {
        /*
         * Bei dynamischen FilterConditions muss der Teil entfernt werden, der sp�ter zur Laufzeit die dynamischen Variablen aufl�st.
