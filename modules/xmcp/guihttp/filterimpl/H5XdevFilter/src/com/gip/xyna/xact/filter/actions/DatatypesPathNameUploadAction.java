 ...
 *
 */
public class DatatypesPathNameUploadAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.DATA_TYPES;
  
  public DatatypesPathNameUploadAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Upload, Type.dataType, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 5 
        && url.getPathElement(4).equals(PathElements.UPLOAD)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Datatypes-TypePath-TypeName-Upload";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5.data/Simple/upload", "h5.data.Simple");
  }
