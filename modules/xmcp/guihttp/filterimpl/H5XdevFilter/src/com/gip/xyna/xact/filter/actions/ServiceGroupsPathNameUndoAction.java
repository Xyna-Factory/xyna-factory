 ...
 *
 */
public class ServiceGroupsPathNameUndoAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.SERVICES_GROUPS;
  
  public ServiceGroupsPathNameUndoAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Undo, Type.serviceGroup, false);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 5 
        && url.getPathElement(4).equals(PathElements.UNDO)
        && method == Method.GET;
  }

  @Override
  public String getTitle() {
    return "XMOM-ServiceGroups-TypePath-TypeName-Undo";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/lw/TestJsonWFCall/undo", "lw.testJsonWFCall"); //FIXME
  }
