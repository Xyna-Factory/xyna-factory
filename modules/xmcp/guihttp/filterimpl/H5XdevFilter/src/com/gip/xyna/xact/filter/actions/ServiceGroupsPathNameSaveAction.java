...
public class ServiceGroupsPathNameSaveAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.SERVICES_GROUPS;
  
  public ServiceGroupsPathNameSaveAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Save, Type.serviceGroup, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.SAVE)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Servicegroups-TypePath-TypeName-Save";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5.data/Simple/save", "h5.data.Simple");
  }
