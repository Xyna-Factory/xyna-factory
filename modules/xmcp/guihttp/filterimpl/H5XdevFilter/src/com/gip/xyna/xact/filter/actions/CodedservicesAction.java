 ...
 *
 */
public class CodedservicesAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.CODED_SERVICES;
  
  public CodedservicesAction(XMOMGui xmomGui) {
    super(xmomGui, 0, 0, Operation.Create, Type.codedService, true);
  }
  ...
  private FilterActionInstance listPath(RuntimeContext rc, Long revision, URLPath url, HTTPTriggerConnection tc) throws SocketNotAvailableException {
    ...
    try {
      ...
      if(tc.getParameters() != null && tc.getParameters().containsKey("sort")) {
        sortType = SortType.valueOf(tc.getFirstValueOfParameter("sort"));
      }
      
      XmomObjectsPath xmomObjectsPath = new XmomObjectsPath(xmomGui.getXmomLoader(), "codedServices");
      xmomObjectsPath.filterTypes(ObjectIdentifierJson.Type.codedService);
