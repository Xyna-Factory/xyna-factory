...
public class ExceptionsPathNameRefactorAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.EXCEPTION_TYPES;
  
  public ExceptionsPathNameRefactorAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Refactor, Type.exceptionType, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.REFACTOR)
        && method == Method.POST;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Exceptiontypes-TypePath-TypeName-Refactor";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/h5.exception/Simple/deploy", "h5.exception.Simple");
  }
