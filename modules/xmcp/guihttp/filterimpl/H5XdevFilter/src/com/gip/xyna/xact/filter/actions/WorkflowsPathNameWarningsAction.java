...
public class WorkflowsPathNameWarningsAction extends XmomGuiAction{

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.WORKFLOWS;


  public WorkflowsPathNameWarningsAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.Warnings, Type.workflow, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH) 
        && url.getPathLength() == 5 
        && url.getPathElement(4).equals(PathElements.WARNINGS)
        && method == Method.GET;
  }

  @Override
  public String getTitle() {
    return "XMOM-Workflows-TypePath-TypeName-Warnings";
  }

  @Override
  public void appendIndexPage(HTMLPart body) {
    HTMLPart paragraphDef = body.paragraph();
    paragraphDef.link(BASE_PATH+"/lw/TestJsonWFCall/warwnings", "lw.testJsonWFCall"); //FIXME
  }
