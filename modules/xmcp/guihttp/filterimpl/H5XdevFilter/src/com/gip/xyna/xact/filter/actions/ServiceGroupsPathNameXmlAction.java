...
public class ServiceGroupsPathNameXmlAction extends XmomGuiAction {

  private static final String BASE_PATH = "/" + PathElements.XMOM + "/" + PathElements.SERVICES_GROUPS;
  
  public ServiceGroupsPathNameXmlAction(XMOMGui xmomGui) {
    super(xmomGui, 2, 3, Operation.ViewXml, Type.serviceGroup, true);
  }
  ...
  protected boolean matchRuntimeContextIndependent( URLPath url, Method method) {
    return url.getPath().startsWith(BASE_PATH)
        && url.getPathLength() == 5
        && url.getPathElement(4).equals(PathElements.XML)
        && method == Method.GET;
  }
  
  @Override
  public String getTitle() {
    return "XMOM-Servicegroups-TypePath-TypeName-Xml";
  }
