...
public class Dataflow {
  ...
  public static Pair<AVariable, Step> determineSource(ScopeStep parentScope, AVariable parameter, Step stepWithParameterInput, String sourceId,
    ...
    /*
     * von einer variable zur n�chsten hangeln, bis man an einem output eines schrittes angekommen ist
     * man muss das rekursiv machen, weil in den auditdetails nur die ausgef�hrten schritte angezeigt werden, aber nicht choices etc
     * falls man auf den output einer choice verbinden wollen w�rde (wie im alten flash monitor), muss man die rekursion reduzieren/weglassen.
     * 
     * findSource(X)
     *   1: X=workflow-output
     *   2: X=step-input
     ...
     * 1. id gen�gt f�r eindeutigkeit
     * 
     * source von objekt innerhalb von Foreach kann auf objekt innerhalb von Foreach oder von vorne dran zeigen.
     * source von objekt kann nie von au�erhalb von Foreach auf innerhalb von Foreach zeigen. nur auf output von foreach-scope.
     * 
     * 
     * die parameter haben alle ids+source/target-ids gesetzt. 
     * die werden beim parsen (Step.parseParameter) des audit-xmls von den step-inputs/outputs �bernommen.
     */
     ...
     *   in assign zugeh�rige inputid suchen -> rekursion
     * stepfunction/mapping/assign-input: id von globaler variable
     *   suche, wo id zugewiesen wird (stepid steht als sourceid am dataobjekt. aber da k�nnen mehrere sourceids stehen
     *   (was derzeit nicht unterst�tzt wird -> lieber �ber alle steps iterieren und entsprechenden output suchen, output-/targetvarids = x?)
     *     1) stepfunction/stepmapping
     *       return (step, outputvar)
     *     2) assign von stepchoice
     *       im assign zugeh�rige inputid suchen -> rekursion
     *     3) workflow-input
    ...
    if (stepWithParameterInput instanceof WFStep) {
      //die inputs des wf-outputs findet man immer �ber das globale assign am ende des workflows
    ...
    if (stepWithParameterInput instanceof StepFunction || stepWithParameterInput instanceof StepMapping
      ...
      for (Step step : allSteps(parentScope)) {
        ...
        /*
         * TODO das gleiche f�r retrycounter... die k�nnen auch verschachtelt sein. dazu muss xyna das aber auch in den parametern richtig unterst�tzen
         * 
         * wann muss man den retrycounter von != -1 auf -1 setzen? wenn man beim "weiterhangeln" den bereich verl�sst, wo das retry passiert.
         ...
         * befindet sich ein schritt in einem catchblock, gibt es folgende f�lle:
         ...
         * 3) es gibt globalen UND lokalen retry (dann m�sste es 2 retrycounter geben)
        ...
        if (!(step instanceof StepForeach) && step.getParameter(foreachIndicesOfStep, retryCounterForStep) == null) {
          //schritt weiter unten oder schritt in choice-lane, die nicht ausgef�hrt wurde, etc
          continue;
        }
  ...
  /*
   * gibt die tiefe D in dem sinne zur�ck, dass step-parameter bzgl dieses steps D foreachindices beinhalten.
   * d.h. toplevel step => 0
