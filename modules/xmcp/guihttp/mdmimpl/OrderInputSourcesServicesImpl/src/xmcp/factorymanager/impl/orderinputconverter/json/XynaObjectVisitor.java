...
public class XynaObjectVisitor extends EmptyJsonVisitor<GeneralXynaObject> {
  ...
  public JsonVisitor<?> objectStarts(String label) throws UnexpectedJSONContentException {
    if (META_TAG.equals(label)) {
      ...
      return new XynaObjectVisitor(); //komplexe membervariable oder das k�nstliche json-objekt f�r die complexlist (welches $meta und $list enth�lt)
    }
  }
  ...
  public void list(String label, List<String> values, com.gip.xyna.utils.misc.JsonParser.JsonVisitor.Type type)
    ...
    if (field != null) {
      ...
      try {
        ...
        //FIXME eigtl m�sste man den typ �ber generationbase herausbekommen. hier kann man boxed/unboxed listen nicht unterscheiden
  ...
  private void adjustValue(GeneralXynaObject gxo, String label, String value) throws UnexpectedJSONContentException {
    ...
    if (field != null) {
      ...
      try {
        ...
        if (primitiveType != null) {
          field.set(gxo, primitiveType.fromString(value));
        } else {
          throw new UnexpectedJSONContentException("Unexpected attribute: " + label);
        }
  ...
  public static Field findField(Class<? extends GeneralXynaObject> clazz, String label) {
    try {
      return clazz.getDeclaredField(label);
    } catch (NoSuchFieldException | SecurityException e) {
      Class<?> superClass = clazz.getSuperclass();
      if (GeneralXynaObject.class.isAssignableFrom(superClass)) {
        return findField((Class<? extends GeneralXynaObject>) superClass, label);
      } else {
        return null;
      }
    }
  ...
  private GeneralXynaObject getObject() {
    if (object == null) {
      ...
      try {
        Class<?> clazz = deriveClassFromInfo();
        object = (GeneralXynaObject) clazz.getDeclaredConstructor().newInstance();
      } catch (XNWH_OBJECT_NOT_FOUND_FOR_PRIMARY_KEY | Ex_FileAccessException | XPRC_XmlParsingException | ClassNotFoundException
          | InstantiationException | IllegalAccessException | XPRC_InvalidPackageNameException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException e) {
        throw new RuntimeException(e);
      }
    }
    return object;
  ...
  public void emptyList(String label) throws UnexpectedJSONContentException {
     ...
     * falls complexListWrapper, ist das boolean-flag leider nicht auf true gesetzt, weil dazu h�tte man einem listenelement begegnen m�ssen (gibt aber ja dann keine)
     ...
     *   label = $list
     * fall2: simple liste
     *   dann zeigt metainfo auf den typ des complexen typs, der die simple liste enth�lt
