    ...
    outer : while (++pos < len) {
      ...
      switch (n) {
        case '\"' :
          ...
          if (validate && pos == len) {
            throw new InvalidJSONException(start, "String does not terminate");
          }
          break;
        case '{' :
        ...
        case '-' :
          ...
          while (pos < len) {
            n = json.charAt(pos);
            switch (n) {
              //this list is optimized for performance of correct jsons. invalid jsons could 
              //be parsed better by stopping the number token earlier when encountering invalid characters.
              case ' ' :
              case ',' :
              case '}' :
              case ']' :
