...
public final class TextConfigTokenReader {
  ...
  private static TextConfigToken createToken(final String line, final int lineNumber) throws ConfigFileReadException {
    String value = null;
    ...
    if (spacesCount % 2 == 1) {
      throw new ConfigFileReadException("Expected an even amount of spaces at the beginning of line <" + lineNumber + ">: <" + line + ">");
    }
    int level = spacesCount / 2;
    String key = indentedKey.substring(spacesCount);
    if ("".equals(key)) {
      throw new ConfigFileReadException("Expected key to not be an empty string at line <" + lineNumber + ">: <" + line + ">");
    }
