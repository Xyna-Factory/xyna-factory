 ...
 */
public final class EContainerTlvEncoder extends AbstractTypeOnlyTlvEncoder {
  ...
  public EContainerTlvEncoder(final int typeEncoding, final String subencoding, final Map<String, TlvEncoder> subNodeEncodings) {
    super(typeEncoding);
  ...
  protected void writeTypeOnlyNode(final TypeOnlyNode node, final OutputStream target) throws IOException {
    ...
    if (subNodesData.size() > RadiusConfigurationEncoder.MAX_TLV_LENGTH) {
      throw new IllegalArgumentException("Subnodes contain more than " + RadiusConfigurationEncoder.MAX_TLV_LENGTH + " bytes: <" + node
          + ">.");
    }
