...
public final class VendorSpecificInformationTlvReader implements TlvReader {
  ...
  private static Logger logger = Logger.getLogger(VendorSpecificInformationTlvReader.class);
  ...
  public VendorSpecificInformationTlvReader(InputStream inputStream, final Set<Integer> paddingTypeEncodings) {
    if (inputStream == null) {
  ...
  public Tlv read() throws TlvReaderException {
    ...
    List<Byte> value = readValue(length - 2);
  ...
  private int readEnterpriseNr() throws TlvReaderException {
    int enr = -1;
    try {
      enr = (inputStream.read() * 256 * 256 * 256 + inputStream.read()) * 256 * 256 + inputStream.read() * 256 + inputStream.read();
    } catch (IOException e) {
