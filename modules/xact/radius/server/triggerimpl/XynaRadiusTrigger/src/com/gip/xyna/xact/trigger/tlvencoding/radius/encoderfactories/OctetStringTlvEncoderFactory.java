...
import com.gip.xyna.xact.trigger.tlvencoding.radius.encoders.OctetStringTlvEncoder;
...
public final class OctetStringTlvEncoderFactory extends AbstractTypeWithValueTlvEncoderFactory {
  ...
  protected TlvEncoder createTypeWithValueTlvEncoder(final int typeEncoding, final Map<String, String> arguments) {
    int minLength = 1;
    int maxLength = RadiusConfigurationEncoder.MAX_TLV_LENGTH;
  ...
  private static int parseLengthValue(final Map.Entry<String, String> entry) {
    ...
    if (!value.matches("([1-9][0-9]?)?[0-9]")) {
      throw new IllegalArgumentException("Key <" + entry.getKey() + "> contains an illegal value: <" + entry.getValue() + ">.");
    }
    int parsedValue = Integer.parseInt(value);
    if (parsedValue > RadiusConfigurationEncoder.MAX_TLV_LENGTH || parsedValue < 1) {
      throw new IllegalArgumentException("Argument <" + entry.getKey() + "> is not in range [1, "
          + RadiusConfigurationEncoder.MAX_TLV_LENGTH + "]: <" + entry.getValue() + ">.");
    }
    return parsedValue;
  }
}
