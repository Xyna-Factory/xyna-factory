 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
package com.gip.xyna.xact.trigger.tlvencoding.radius.encoderfactories;
...
import com.gip.xyna.xact.trigger.tlvencoding.radius.encoders.Unsigned1to7ByteIntegerTlvEncoder;
...
public class UnsignedIntegerTlvEncoderFactory extends AbstractTypeWithValueTlvEncoderFactory {
  ...
  protected TlvEncoder createTypeWithValueTlvEncoder(int typeEncoding, Map<String, String> arguments) {
    if (!arguments.containsKey("nrBytes")) {
      throw new IllegalArgumentException("Mandatory argument <nrBytes> missing from provided arguments: <" + arguments + ">.");
    }
    int nrBytes = -1;
    Long minValue = null;
    Long maxValue = null;
    ...
    for (Map.Entry<String, String> entry : arguments.entrySet()) {
      try {
        ...
        throw new IllegalArgumentException("Argument <" + entry.getKey() + "> has an illegal value: <" + entry.getValue() + ">.", e);
      }
    }
    if (nrBytes < 8) {
      return new Unsigned1to7ByteIntegerTlvEncoder(typeEncoding, nrBytes, minValue, maxValue, multipleOf);
    } else {
      throw new IllegalArgumentException("No <UnsignedInteger> implementation available for <nrBytes=" + nrBytes + ">.");
    }
  }
}
