...
public class SNMPTrigger extends EventListener<SNMPTriggerConnection, SNMPStartParameter> {
  ...
  public void start(final SNMPStartParameter sp) throws XACT_TriggerCouldNotBeStartedException {
    ...
    snmp.addCommandResponder(new CommandResponder() {
      ...
      public void processPdu(CommandResponderEvent e) {
        if (!e.isProcessed()) {
          try {
            // event zu queue hinzuf�gen. receive methode liest dann aus queue aus...
            if (!events.offer(e, capFullTimeout, TimeUnit.MILLISECONDS)) {
              // �berlastet => nichts tun TODO herausfinden, ob man statt dessen in bestimmten f�llen einen fehler
              // zur�ckgeben sollte?
            }
  ...
  //in snmpserviceimpl sehr �hnlich!
  private static byte[] getLocalEngineId(InetAddress ip, int port) {
    ...
    //darf maximal 27 bytes lang sein. 2 f�r portbytes, 4-8 f�r address, 4 rest, 2 revision, macht zusammen 12 - 16
  ...
  /**
   * f�r jede andere per snmpv3 bekannte snmp engine (also eine, mit der man in der vergangenheit mal kommuniziert hat)
   * werden bootcnt+boottime gespeichert und beim zugriff gegen den letzten gespeicherten wert auf plausibilit�t
  ...
  public SNMPTriggerConnection receive() {
    while (true) {
      ...
      if (e == null) { //queue empty => wartend pollen. diese threads m�ssen beim trigger-stop unterbrochen werden
        lock.lock();
        try {
  ...
  /**
   * f�hrt runnable aus, sofern oder sobald der trigger gestartet wurde
   */
