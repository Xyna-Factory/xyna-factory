...
public class SNMPTriggerConnection extends TriggerConnection {
  ...
  /**
   * ruft requestHandler.x auf, je nachdem um was f�r einen snmp event es sich handelt und gibt die
   * im requestHandler gebaute xynaorder zur�ck.
   ...
   * zur�ck an den client. nicht geeignet um f�r einen snmp request eine xynaorder zu erstellen. daf�r besser anderes
  ...
  public void handleEvent(RequestHandler requestHandler) throws XynaException, InterruptedException {
    ...
    // etwas zu �ndern.
    //TODO bei v1/v2 community �berpr�fen
    PDU command = event.getPDU();
    PDU response = createResponsePdu(command.getRequestID());

    if (command.size() == 0) {
      // kein Fehler, leere Varbind-Liste zur�ckgeben
    } else {
      VarBindList vbl = varBindTypeConverter.toVarBindList(command);
      ...
      try {
        ...
        return; // oid wurde von requesthandler nicht als g�ltig erkannt => n�chsten filter versuchen
