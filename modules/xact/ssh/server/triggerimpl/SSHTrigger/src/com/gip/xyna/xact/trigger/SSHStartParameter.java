...
public class SSHStartParameter extends EnhancedStartParameter implements SSHServerParameter, SFTPSubsystemParameter {
  ...
  public static final StringParameter<Integer> PORT =
      StringParameter.typeInteger("port").documentation(Documentation.de("Port").en("Port").build()).defaultValue(22).build();
  public static final StringParameter<String> ADDRESS = StringParameter.typeString("address").documentation(Documentation
  ...
  public static final StringParameter<Auth> AUTH = StringParameter.typeEnum(Auth.class, "auth")
      .documentation(Documentation.de("Authentication").en("Authentication").build()).defaultValue(Auth.needless).build();
  public static final StringParameter<ErrorHandling> ERROR_HANDLING = StringParameter.typeEnum(ErrorHandling.class, "errorHandling")
      .documentation(Documentation.de("Fehlerbehandlung").en("Error handling").build()).defaultValue(ErrorHandling.exception).build();
  ...
  public static final StringParameter<String> ORDERTYPE_INIT = StringParameter.typeString("ordertype_init").documentation(Documentation
  ...
  public static final StringParameter<String> ORDERTYPE_EXEC = StringParameter.typeString("ordertype_exec").documentation(Documentation
      ...
      .en("Name of ordertype started for every command. signature (xact.ssh.server.SSHSession, xact.connection.Command) -> (xact.connection.Response)")
      .build()).build();
  ...
  public static final StringParameter<Boolean> SHELL = StringParameter.typeBoolean("shell")
      .documentation(Documentation.de("Erlaube Shell").en("Enable Shell").build()).defaultValue(true).build();

  public static final StringParameter<Boolean> OTC = StringParameter.typeBoolean("otc")
      .documentation(Documentation.de("Benutze einmal Passwörter").en("One Time Credentials").build()).defaultValue(false).build();

  public static final StringParameter<Boolean> SCP = StringParameter.typeBoolean("scp")
      .documentation(Documentation.de("Erlaube SCP").en("Enable SCP").build()).defaultValue(false).build();

  public static final StringParameter<Boolean> SFTP = StringParameter.typeBoolean("sftp")
      .documentation(Documentation.de("Erlaube SFTP").en("Enable SFTP").build()).defaultValue(false).build();

  public static final StringParameter<Boolean> NATIVE_FILE_ACCESS = StringParameter.typeBoolean("fileAccess")
      .documentation(Documentation.de("Erlaube Zugriff auf das Dateisystem").en("Enable filesystem access").build()).defaultValue(false)
      .build();

  public static final StringParameter<String> NATIVE_FILE_ROOT = StringParameter.typeString("fileRoot")
      .documentation(Documentation.de("Basis Verzeichnis für Dateizugriff").en("Root directory for filesystem access").build()).optional()
      .build();
  ...
  public static final StringParameter<String> NATIVE_FILE_PREFIX = StringParameter.typeString("filePrefix")
      ...
      .defaultValue("/StaticXfc/").build();

  public static final StringParameter<Duration> SFTP_TIMEOUT = StringParameter.typeDuration("requestTimeout")
      .documentation(Documentation.de("Die maximale Wartezeit einer SFTP- oder SCP-Anfrage").en("Timeout for SFTP or SCP requests").build())
      .defaultValue(Duration.valueOf("15 min", null)).build();

  private static final List<BuiltinDHFactories> defaultKEX = BaseBuilder.DEFAULT_KEX_PREFERENCE;
  public static final StringParameter<List<String>> KEX = StringParameter
      .typeList(String.class, "kex",
                StringParameter.ListSeparator.COMMA_WHITESPACE)
      .documentation(Documentation
          .de("KEX Algorithmen, Werte:\n         ("
              + String.join(",", BuiltinDHFactories.VALUES.stream().map(BuiltinDHFactories::getName).collect(Collectors.toSet()))
              + ")\n         ")
          .en("KEX algorithms, values:\n         ("
              + String.join(",", BuiltinDHFactories.VALUES.stream().map(BuiltinDHFactories::getName).collect(Collectors.toSet()))
              + ")\n         ")
          .build())
      .defaultValue(defaultKEX.stream().map(BuiltinDHFactories::getName).collect(Collectors.toList())).build();

  private static final List<BuiltinMacs> defaultMAC = BaseBuilder.DEFAULT_MAC_PREFERENCE;

  public static final StringParameter<List<String>> MAC =
      StringParameter.typeList(String.class, "mac", StringParameter.ListSeparator.COMMA_WHITESPACE)
          .documentation(Documentation
              .de("MAC Algorithmen, Werte:\n         ("
                  + String.join(",", BuiltinMacs.VALUES.stream().map(BuiltinMacs::getName).collect(Collectors.toSet())) + ")\n         ")
              .en("MAC algorithms, values:\n         ("
                  + String.join(",", BuiltinMacs.VALUES.stream().map(BuiltinMacs::getName).collect(Collectors.toSet())) + ")\n         ")
              .build())
          .defaultValue(defaultMAC.stream().map(BuiltinMacs::getName).collect(Collectors.toList())).build();

  private static final List<BuiltinSignatures> defaultAuth = BaseBuilder.DEFAULT_SIGNATURE_PREFERENCE;
  public static final StringParameter<List<String>> AUTH_ALGO = StringParameter
      .typeList(String.class, "authAlgo",
                StringParameter.ListSeparator.COMMA_WHITESPACE)
      .documentation(Documentation
          .de("Authentifizierungs Algorithmen, Werte:\n         ("
              + String.join(",", BuiltinSignatures.VALUES.stream().map(BuiltinSignatures::getName).collect(Collectors.toSet()))
              + ")\n         ")
          .en("Authentification algorithms, values:\n         ("
              + String.join(",", BuiltinSignatures.VALUES.stream().map(BuiltinSignatures::getName).collect(Collectors.toSet()))
              + ")\n         ")
          .build())
      .defaultValue(defaultAuth.stream().map(BuiltinSignatures::getName).collect(Collectors.toList())).build();

  private static final List<BuiltinCiphers> defaultCipher = BaseBuilder.DEFAULT_CIPHERS_PREFERENCE;

  public static final StringParameter<List<String>> CIPHER = StringParameter
      .typeList(String.class, "cipher", StringParameter.ListSeparator.COMMA_WHITESPACE)
      .documentation(Documentation
          .de("Verschlüsselungs Algorithmen, Werte:\n         ("
              + String.join(",", BuiltinCiphers.VALUES.stream().map(BuiltinCiphers::getName).collect(Collectors.toSet())) + ")\n         ")
          .en("Cipher algorithms, values:\n         ("
              + String.join(",", BuiltinCiphers.VALUES.stream().map(BuiltinCiphers::getName).collect(Collectors.toSet())) + ")\n         ")
          .build())
      .defaultValue(defaultCipher.stream().map(BuiltinCiphers::getName).collect(Collectors.toList())).build();

  public static final List<StringParameter<?>> ALL_PARAMS =
      Arrays.<StringParameter<?>> asList(PORT, ADDRESS, AUTH, HOST_KEY, ERROR_HANDLING, IDLE_TIMEOUT, SHELL, ORDERTYPE_INIT, ORDERTYPE_EXEC,
                                         ORDERTYPE_CLOSE, OTC, SCP, SFTP, NATIVE_FILE_ACCESS, NATIVE_FILE_PREFIX, NATIVE_FILE_ROOT,
  ...
  private boolean passwordauth =
      AUTH.getDefaultValue() != null ? AUTH.getDefaultValue().equals(Auth.both) || AUTH.getDefaultValue().equals(Auth.password) : false;
  private boolean publickeyauth =
      AUTH.getDefaultValue() != null ? AUTH.getDefaultValue().equals(Auth.both) || AUTH.getDefaultValue().equals(Auth.publickey) : false;;

  private boolean alwaysauth = AUTH.getDefaultValue() != null ? AUTH.getDefaultValue().equals(Auth.needless) : false;
  ...
  public StartParameter build(Map<String, Object> paramMap) throws XACT_InvalidTriggerStartParameterValueException {
    ...
    if (param.fileRoot == null) {
      ...
      if (prop != null)
        param.fileRoot = XynaFactory.getInstance().getFactoryManagement().getProperty(prop);
    }
  ...
  public String getHostkeyAlgorithm() {
    return "RSA"; // ToDo: Add startparameter for hostkey type
  }


  @Override
  public int getHostkeySize() {
    return 4096; // ToDo: Add startparameter for hostkey type
  }
