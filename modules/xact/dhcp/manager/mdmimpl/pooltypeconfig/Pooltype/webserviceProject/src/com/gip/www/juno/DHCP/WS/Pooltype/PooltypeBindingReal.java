...
import com.gip.www.juno.DHCP.WS.Pooltype.Messages.*;
...
public class PooltypeBindingReal{

  private static TableHandler _handler = new PooltypeHandler();
  ...
  public Row_ctype insertRow(InsertRowRequest_ctype insertRowRequest) throws java.rmi.RemoteException {
    ...
    try {
      ...
      Row_ctype ret = new WebserviceHandler<Row_ctype>().insertRow(input, _handler, username,
          password);
      adjustDefaultPoolTypeIfNecessary(ret, username, password);
      return ret;
    } catch (java.rmi.RemoteException e) {
      deployInfo.setLog(e);
      throw e;
  ...
  private final static String selectOtherDefaultPoolType = "select * from pooltype where isDefault = \"yes\" AND NOT poolTypeID = ";
  

  private void adjustDefaultPoolTypeIfNecessary(Row_ctype defaultInQuestion, String username, String password) throws RemoteException {
    if (defaultInQuestion.getIsDefault() != null &&
        defaultInQuestion.getIsDefault().equals("yes")) {
      DBTableInfo table = _handler.getDBTableInfo();
      DBSchema schema = ManagementData.translateDBSchemaName(table.getSchema(), _handler.getLogger());
      SQLUtilsContainerForManagement sql = SQLUtilsCache.getForManagement(schema, _handler.getLogger());
      DBReader<Row_ctype> reader = new ReflectionTools.DBReader<Row_ctype>(table, defaultInQuestion, _handler.getLogger());
      String queryCommandSql = selectOtherDefaultPoolType + defaultInQuestion.getPoolTypeID();
      SQLCommand queryCommand = new SQLCommand();
      queryCommand.sql = queryCommandSql;
      List<Row_ctype> previousDefaults = new DBCommands<Row_ctype>().query(reader, queryCommand, sql, _handler.getLogger());
      for (Row_ctype row_ctype : previousDefaults) {
        row_ctype.setIsDefault("no");
        new WebserviceHandler<Row_ctype>().updateRow(row_ctype, _handler, username, password);
      }
    }
  }
