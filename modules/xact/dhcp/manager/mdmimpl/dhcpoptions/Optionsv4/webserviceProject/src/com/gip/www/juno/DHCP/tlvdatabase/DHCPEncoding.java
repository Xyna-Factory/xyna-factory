...
@Persistable(primaryKey = DHCPEncoding.COL_ID, tableName = DHCPEncoding.TABLENAME)
public class DHCPEncoding extends Storable<DHCPEncoding> implements Comparable<DHCPEncoding> {


  private static final long serialVersionUID = -3363326642316655775L;
  public final static String TABLENAME = "optionsv4";
  ...
  public DHCPEncoding() // fuer Storable benoetigt
  {

  }


  public DHCPEncoding(final int id, final Integer parentId, final String typeName, final long typeEncoding,
  ...
  public String getMySQLInsert()
  {
    int readonlyflag = 0;
    if(this.readOnly) readonlyflag = 1;
    return "INSERT INTO `optionsv4` (`id`, `parentid`, `typename`, `typeencoding`, `enterprisenr`, `valuedatatypename`, `valuedatatypeargumentsstring`, `readonly`, `status`, `guiname`, `guiattribute`, `guifixedattribute`, `guiparameter`, `guiattributeid`, `guifixedattributeid`, `guiparameterid`, `guiattributewertebereich`, `guifixedattributevalue`) VALUES ("+this.id+","+this.parentId+",'"+this.typeName+"',"+this.typeEncoding+","+this.enterpriseNr+",'"+this.valueDataTypeName+"','"+this.valueDataTypeArgumentsString+"',"+readonlyflag+",'"+this.statusflag+"','"+this.guiname+"','"+this.guiattribute+"','"+this.guifixedattribute+"','"+this.guiparameter+"',"+this.guiattributeid+","+this.guifixedattributeid+","+this.guiparameterid+",'"+this.guiattributewertebereich+"','"+this.guifixedattributevalue+"')";
  }
  ...
  private static ResultSetReader<DHCPEncoding> reader = new ResultSetReader<DHCPEncoding>() {

    public DHCPEncoding read(ResultSet rs) throws SQLException {
      DHCPEncoding e = new DHCPEncoding();
  ...
  @Override
  public ResultSetReader<? extends DHCPEncoding> getReader() {
    return reader;
  }


  @Override
  public <U extends DHCPEncoding> void setAllFieldsFromData(U dataIn) {
    DHCPEncoding data = (DHCPEncoding) dataIn;
    id = data.id;
  ...
  public int compareTo(DHCPEncoding compareObject) {
