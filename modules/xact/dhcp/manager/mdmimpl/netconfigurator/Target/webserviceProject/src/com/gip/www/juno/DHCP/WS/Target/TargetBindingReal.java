...
import com.gip.juno.ws.db.tables.dhcp.TargetHandler;
import com.gip.juno.ws.exceptions.DPPWebserviceException;

import com.gip.www.juno.DHCP.WS.Target.Messages.*;
import com.gip.www.juno.Gui.WS.Messages.*;

public class TargetBindingReal{

  private static TableHandler _handler = new TargetHandler();
  ...
  public String updateRowPkIgnoreEmpty(UpdateRowPkIgnoreEmptyRequest_ctype updateRowPkIgnoreEmptyRequest)
        throws java.rmi.RemoteException {
    DeploymentTools.Row deployInfo = new DeploymentTools.Row();
    try {
      InputHeaderContent_ctype header = updateRowPkIgnoreEmptyRequest.getInputHeader();
      String username = header.getUsername();
      String password = header.getPassword();
      UpdateRowPkIgnoreEmptyInput_ctype input = updateRowPkIgnoreEmptyRequest.getUpdateRowPkIgnoreEmptyInput();
      ChangeMonitor<Row_ctype> monitor = new ChangeMonitor<Row_ctype>(_handler, _handler.getLogger());
      deployInfo.setService(_handler.getTablename() + ".Update");
      deployInfo.setUser(username);
      deployInfo.setTarget(ChangeMonitor.Constant.MANAGEMENT);

      String ret = new WebserviceHandler<Row_ctype>().updateRowPkIgnoreEmpty(input.getConditions(),
