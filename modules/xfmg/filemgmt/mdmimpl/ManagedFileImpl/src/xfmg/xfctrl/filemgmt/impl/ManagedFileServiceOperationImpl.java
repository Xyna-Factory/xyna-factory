...
import xfmg.xfctrl.filemgmt.ManagedFileServiceOperation;


public class ManagedFileServiceOperationImpl implements ExtendedDeploymentTask, ManagedFileServiceOperation {
  ...
  public ManagedFile createTemporaryFile(XynaOrderServerExtension correlatedOrder) {
    try {
      java.io.File file =  java.io.File.createTempFile("managedFile_", null);
      checkRight(file, Action.insert, correlatedOrder.getCreationRole());
      ManagedFile mgedFile = new ManagedFile();
      mgedFile.setPath(file.getAbsolutePath());
      return mgedFile;
    } catch (IOException e) {
  ...
  static void checkRight(java.io.File file, Action action, Role role) {
    if (role == null) {
      return;
    }
    try {
      ...
      if (!rightCache.hasRight(UserManagement.ScopedRight.FILE_ACCESS.getKey(), action.toString(), file.getAbsolutePath())) {
        throw new RuntimeException("FileAccess " + action.toString() + "@" + file.getAbsolutePath() + " not allowed for " + role.getName());
      }
