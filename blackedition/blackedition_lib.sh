...
f_indent () {
  ...
  shift 1
  local OUTPUT="$@"
  echo "${OUTPUT}" | sed "s+^+${INDENT}+" #Einr√ºcken
}
...
install_xynafactory () {
  ...
  #Schreibschutz f√ºr xynafactory.sh
...
update_xynafactory () {
  ...
  #alles im server-Verzeichnis kopieren au√üer log4j2.xml, server.policy, xynafactory.sh
  ${VOLATILE_CP} -rp server ${INSTALL_PREFIX}/.
  ...
  #f√ºr Umstieg auf log4j2 muss die Property 'jvm.option.log4j" angepasst werden
...
check_install_oracle_cluster () {
  #Suche nach Pattern, daher kein  if [ -e ojdbc*.jar ]; mˆglich!
...
import_applications () {
  ...
  local SKIPPED_APP_LIST="";   #Apps die bereits vorhanden sind und deshalb √ºbersprungen werden
...
merge_files () {
  ...
  #Common Base ist nˆtig, evtl. versuchen anzulegen
