...
* Xyna Server lokal starten
  1. Verzeichnis /etc/xyna/environment/ muss fÃ¼r den Entwickler schreibbar sein
  ...
  5. fÃ¼r logging-Ausgaben blackedition/server/product_lib.sh anpassen:
...
* Server mit lokal installierter GUI+tomcat
  ...
  5. Falls gewÃ¼nscht: in server/xynaserver.sh Logging aktivieren, indem man in server/xynaserver.sh "> /dev/null" durch "> server.log" oder ">> server.log" ersetzt.
  6. server/mysql.properties anpassen
  7. install_black_edition.sh ausfÃ¼hren. UnerwÃ¼nschte Komponenten lassen sich durch Kommandozeilenparameter unterdrÃ¼cken. (Hint: install_black_edition.sh -h)
...
* Build-Skripte starten
  1. FÃ¼r die Kompilierung von Flex mit ant mÃ¼ssen dem Skript VM Argumente mitgegeben werden:
...
svn ci -m "Merge aus Trunk rev 176003: <UrsprÃ¼nglicher Kommentar>" .
...
======================
Identifiziere Revision X im Source-Zweig (z.b. Trunk), die die aktuellste Revision im Branch sein soll und Versionsname des Branches (anhand der letzten gelieferten Versionsnummer). Dann fÃ¼hre aus:
...
Bau einer Lieferung
###################
ein lokaler build is möglicherweise aufgrund von fehlender npm/node Einrichtung oder Platzbedarf nicht auf dem eigenem Entwicklungsrechner möglich, login4 ist fÃ¼r einen delivery build eingerichtet worden.
...
Der Text fÃ¼r die Releasenotes kann per Workflow aus Bugzilla-Daten erstellt werden:
Workflow 
  xdoc.bugzilla.wf.GenerateReleasenotesXynaSingleVersion
oder fÃ¼r gröÃŸere Versionsintervalle mit
