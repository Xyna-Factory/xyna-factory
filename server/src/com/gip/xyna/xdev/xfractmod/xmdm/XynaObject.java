...
public abstract class XynaObject implements GeneralXynaObject {
  ...
  /**
   * cache f�rs fromXML, damit da nicht jedes mal die xmls geparst werden m�ssen. wird bei jeden deployment invalidiert.
   */
  ...
  public void onDeployment() throws XynaException {
    // defaultm�ssig passiert hier nichts. abgeleitete objekte k�nnen hier:
  ...
  //ACHTUNG: wenn neue (auch abstrakte) instanzmethoden hinzugef�gt werden, auch in JavaServiceImplementation im generierten code beachten! am besten mal abstrakten instanzservice testen
  ...
  //abw�rtskompatible version from generalFromXml, die nur xynaobjekte zur�ckgibt
   ...
   * hat, welches aber mehrere solcher Data-Elemente enth�lt. im letzeren fall wird ein container-objekt um die
  ...
  /**
   * falls e eine liste beschreibt, wird eine xynaobjectlist zur�ckgegeben, 
   * falls ReferenceName und ReferencePath fehlen, wird null zur�ckgegeben, 
      ...
      try {
        ...
        if (dom == null) {
          ...
          if (isDom) {
            ...
            //revision separat �bergeben, weil die ist evtl anders als dom.getRevision und kann kind-members aufl�sen
  ...
  //TODO unversionedSet unterst�tzen. z.b. f�r xmompersistence
  ...
  public static void undeploy(Class<? extends XynaObject> clazz)
    ...
    try {
      ...
      // deploymenttask ausf�hren und entfernen.
      //abw�rtskompatibilit�t. fr�her konnte man deploymenttasks bei xynaobject registrieren
  ...
  private void callThreadForDeploymentHandler(final DeploymentTask task, final DeployCommand command) throws XynaException {
    ...
    try {
      ...
      if (command == DeployCommand.DEPLOY) {
        ...
        //kein fehler werfen, weil beim undeploment dann inkonsistenzen entstehen k�nnen
    ...
    if (unDeploymentRunnable.getThread() != null) {
      ...
      switch (behaviorAfterOnUndeploymentTimeout) {
        ...
        case KILLTHREAD :
          Thread thread = unDeploymentRunnable.getThread();
          if (thread != null) {
            // hoffen wir, dass wir das richtige Runnable t�ten und nicht ein anderen Undeployment-Handler
            // abschie�en, weil unseres gerade fertig geworden ist?
            if (logger.isDebugEnabled()) {
  ...
  public static class XMLHelper {
    ...
    //GeneralXynaObject zul�ssig f�r AnyType-Listen
    ...
    public static void appendException(StringBuilder xml, String varName, Throwable e, long version, XMLReferenceCache cache) {
      if (e != null) {
        if (e instanceof GeneralXynaObject) {
          ...
          //spezialbehandlung f�r spezielle exceptions
    ...
    public static void appendData(StringBuilder xml, String variableName, XynaObject value, long version, XMLReferenceCache cache) {
      if (value != null) {
        xml.append(value.toXml(variableName, false, version, cache)); //TODO sch�ner w�re es, den StringBuilder zu �bergeben
      }
    }
  ...
  public ObjectVersionBase createObjectVersion(long version, IdentityHashMap<GeneralXynaObject, DataRangeCollection> changeSetsOfMembers) {
    //default-implementierung (schlechte performance) f�r nicht codegenerierte objekte
