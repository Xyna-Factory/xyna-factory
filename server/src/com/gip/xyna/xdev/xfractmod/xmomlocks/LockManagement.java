      ...
      if (lock == null) {
        ...
        if (lock.getUser() == null || lock.getUser().equals(creator)) {
          if (lock.getSessionId().equals(sessionId)) {
            return true; // reentrant FIXME das f�hrt zu f�lschlichen Fehlermeldungen "Autosave not locked", wenn man zwei separate threads mit der gleichen sessionid hat
          } else {
