        ...
        if (elis != null) {
          for (int j = 0; j < elis.length; j++) {
            ...
            try {
              ...
              throw new RuntimeException(e); //sollte nicht vorkommen, weil man oben bereits �berpr�ft hat, dass der trigger existiert
      ...
      if (node.getType() == DependencySourceType.DATATYPE) {
        ...
        if (f.exists()) {
          ...
          try {
            ...
            throw new RuntimeException(e); //dann ist dependency node ung�ltig
          } catch (XPRC_InternalObjectMayNotBeUndeployedException e) {
            throw new RuntimeException(e); //dann ist package ung�ltig
          } catch (XPRC_EXISTING_DEPENDENCY_ONUNDEPLOYMENT e) {
            throw new RuntimeException(e); //dependencies sollen auch undeployed werden
          }
