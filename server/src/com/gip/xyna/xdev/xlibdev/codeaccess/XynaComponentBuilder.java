...
public class XynaComponentBuilder {
  ...
  private void prepare(ComponentCodeChange modifiedComponent) throws Ex_FileAccessException {
    // TODO exception klassen generieren? erstmal nicht so wichtig, kann der benutzer einchecken? -> m�sste er eigtl nicht!
    
    // nicht mehr ben�tigte classfiles entfernen
    ...
    if (modifiedComponent.getComponentType() == ComponentType.CODED_SERVICE) {
      try {
        if (modifiedComponent.getParsedDOMFromSaved(codeAccess.getRevision()).hasJavaImpl(false, null)) {
          //servicedefinition jar zu den classfiles hinzuf�gen
  ...
  /**
   * erstellt und gibt die liste aller artefakte (jars, xmls, etc) zur�ck, die kopiert werden m�ssen, weil sie sich ge�ndert haben
   */
  private FileUpdate[] createFilesForDeployment(ComponentCodeChange modifiedComponent, String version)
    ...
    if (modifiedComponent.getComponentType() == ComponentType.SHARED_LIB) {
      ...
      //wurden noch andere verwendete jars ge�ndert?
  ...
  private String getClassPathForCompile(ComponentCodeChange modifiedComponent) {
    // abh�ngige jars ermitteln (shared libs, etc)
      ...
      case CODED_SERVICE :
        try {
          ...
          if (dom.hasSuperTypeWithInstanceMethods(null)) {
            //evtl leitet der datentyp von anderen datentypen ab, die ben�tigt man auch als jars
            boolean first = true;
  ...
  private void appendClassPathForOtherCodedService(StringBuilder classPath, DOM dom)
    ...
    //was h�tte das f�r vorteile/nachteile gegen�ber aus saved/deployed nehmen? wenn es compilef�hig ist, sollte es identisch sein
    //ansonsten?
