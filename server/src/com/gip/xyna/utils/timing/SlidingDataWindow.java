...
public class SlidingDataWindow implements Serializable {
  ...
  private double size; //summe �ber die werte der schubladen
   ...
   * nun kann man objekte mit absoluten werten einf�gen, und diese werden immer derart eingef�gt, dass
   * der bisher h�chste eingef�gte wert in der letzten schublade liegt. die anderen liegen relativ dazu in 
   * ihren schubladen. werte, die mehr als n*width kleiner sind als der aktuell gr��te wert, werden nicht 
   * eingetragen.
   * 
  ...
  /**
   * f�gt wert zu schublade hinzu
   * @return false, falls die position ausserhalb der g�ltigen range liegt 
   */
  ...
  /**
   * f�gt wert relativ zu min/max position evtl verteilt auf mehrere schubladen hinzu.
  ...
  public void addInRange(long minPosition, long maxPosition, double value) {
    ...
    synchronized (lock) {
      if (size == Double.MIN_VALUE) {
        getIndex(minPosition, true); //muss nach maxIdx nochmal ermittelt werden. wenn man es nicht ausf�hrt, wird aber evtl nicht weit genug geslided.
      }

      int maxIdx = getIndex(maxPosition, true);
      if (maxIdx < 0) {
        //liegt vollst�ndig ausserhalb des aktuellen datawindows
        return;
      }
      ...
      int l = data.length;
      if (maxIdx < minIdx) {
        //ringbuffer ber�cksichtigen -> sp�ter modula l nehmen
        maxIdx += l;
      }
  ...
  /**
   * ermittelt den aktuellen index und sorgt f�r konsistenz, falls das sliding window weiter slidet.
   * negativ, falls ausserhalb des g�ltigen bereichs
   */
   ...
   * ausserhalb der g�ltigen range liegt
   */
  ...
  /**
   * summe �ber die schubladen inhalte, ohne die daten zu �ndern
   */
  public double size() {
    synchronized (lock) {
  ...
  /**
   * summe �ber die schubladen inhalte, wobei zu der �bergebenen position geslided wird
   */
