 ...
 * bearbeitet werden m�ssen.<br>
 * Im Unterschied zu den �blichen Timer und TaskQueue-Implementierungen werden hier keine Runnable-
 * oder TimerTask-Implementierungen gespeichert, sondern einfachere Work-Objekte. Damit ergibt sich 
 * die Einschr�nkung, dass nicht v�llig verschiedene Algorithmen ausgef�hrt werden, sondern immer 
 * die gleiche {@link com.gip.xyna.utils.timing.TimedTasks.Executor Executor}-Implementierung f�r 
 * alle Work-Objekte. Daf�r kann das Work-Objekt aber auch so etwas einfaches sein wie ein Long.
 *  
 */
public class TimedTasks<W> implements Iterable<W> {

  /**
   * Executor wird f�r jedes Work-Objekt aufgerufen, um dieses zu bearbeiten.
   */
  public interface Executor<W> {
    
    /**
     * Ausf�hren der �bergeben Arbeit
     * @param work
     */
    public void execute(W work);
    
    /**
     * Falls beim Ausf�hren von execute irgenein Fehler auftritt, muss dieser behandelt werden, 
  ...
  /**
   * Task-Objekt speichert Work und Ausf�hrungszeitpunkt timestamp und k�mmert sich um
  ...
  /**
   * Implementierung des zugrundeliegenden Threads, der solange wartet, bis eine Work ausgef�hrt werde muss 
   * und dann den Executor diese Work ausf�hren l�sst.
   */
  private class Runner implements Runnable {
    ...
    public void run() {
      ...
      try {
        while( running ) {
          ...
          waitForTasks();
          //warten, bis aktueller Task ausgef�hrt werden darf
          Task<W> task = waitForTask();    
          if( ! running ) {
            break;
          }
          //Task erhalten, diesen ausf�hren
          if( task != null ) {
            try {
    ...
    /**
     * Wartet, bis Tasks verf�gbar sind
     */
    ...
    /**
     * Wartet, bis der Task mit der h�chsten Priorit�t ausgef�hrt werden darf  
     * @return auszuf�hrender Task<W> oder null
     */
      ...
      synchronized (tasks) {
        while( running ) {
          ...
          if( timeToWait <= 0 ) {
            ...
            //=> einfach im n�chsten Schleifendurchlauf  tasks.peek() auswerten und weiterwarten oder Task zur�ckgeben
          }
        }
      }
      return null;
  ...
  /**
   * Entfernen eines Work-Objekts, welches equals(..) zu dem �bergebenen work ist
   * @param work
  ...
  /**
   * Z�hlen aller Work-Objekte, die vom Filter als passend erkannt werden
   * @param filter
   * @return
   */
  ...
  /**
   * Stoppen des ausf�hrenden Threads
   */
  public void stop() {
    synchronized (tasks) {
      running = false;
  ...
  /**
   * Starten des ausf�hrenden Threads (in einem neuen Thread)
   */
  public void restart() {
    restart(false);
  }
  ...
  /**
   * Ausf�hren aller Works bis zum �bergebenen Zeitstempel 
   * @param timestamp
   */
  public void executeAllUntil( long timestamp ) {
    ...
    synchronized (tasks) {
      ...
      tasks.notify(); //Wecken des ausf�hrenden Threads
    }
  }

  /**
   * Ausf�hren aller vom Filter getroffenen Works bis zum �bergebenen Zeitstempel
   * @param timestamp
   */
  ...
  private List<Task<W>> removeTasks(TaskFilter<W> filter) {
    ...
    //Remove �ber iterator und iterator.remove ist O(N^2)!
    synchronized (tasks) {
