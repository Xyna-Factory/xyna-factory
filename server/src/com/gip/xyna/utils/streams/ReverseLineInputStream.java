 ...
 * 
 * funktioniert nicht f�r alle encodings, aber f�r UTF-8 funktioniert das.
...
public class ReverseLineInputStream extends InputStream {
   ...
   *         bufferend
   *         
   *    bufferstart (globaler index, darf negativ sein, wenn buffer den startbereich des files �berschneidet)
   *    bufferend
   ...
   * B: currentLineEnd (bufferrelativer index. falls index>buffer.length, dann endet die zeile nicht im buffer, sondern in (einem der/dem) n�chsten)
   *    zeigt auf den index, der nach \n ist
   *    
   *    d.h. bei \n\n ist A=0, B=1 f�r das erste \n und A=1 und B=2 f�r das zweite
   *    
   */
  ...
  //gehe von currentLineStart soweit zur�ck, bis ein zeilenumbruch oder start vom file gefunden wird, so dass dieser innerhalb des buffers liegt.
  private void findPrevLine() throws IOException {
    ...
    currentPos = currentLineStart; //jetzt die n�chste zeile von beginn aus auslesen
  }
  ...
  //kopiere von relativer bufferposition "start" bis maximal zu currentLineEnd in das �bergebene byte array
