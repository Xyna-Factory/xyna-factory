...
/**
 * 
 * Aus Auftragsdaten eines Auftrags (�blicherweise Priority und Wartezeit) wird eine einzige Zahl 
 * "Urgency" berechnet. 
...
public class Scheduler<O,I> implements Algorithm {
  ...
  /**
   * Runnable wird ausgef�hrt, w�hrend Scheduler pausiert: konsistente Daten 
   * @param r
   */
  ...
  public void exec() {
    ...
    //Queues sind leer: nun warten, bis Scheduler von au�en geweckt wird
  }
  ...
  private void tryScheduleLocked() {
    try {
      //Sammeln der neuen Auftr�ge
      entranceQueue.drainTo(urgencyOrders);

      //Wiedereinstellen umzuordnender Auftr�ge
      ...
      //Schleife �ber alle zu schedulenden Auftr�ge
      ...
      if (!loopEndedRegularily) {
        //endlosschleife von nicht erfolgreichen scheduler-l�ufen verlangsamen, damit sich system besser erholen kann
        loopFailedCounter ++;
  ...
  private boolean loopOverUrgencyOrders() {
    ...
    try {
          ...
          switch( scheduleResult.getType() ) {
            ...
            case Tag:
              ...
              if( scheduleResult.isHide() ) {
                ...
                //die dieses Tag ebenfalls ben�tigen. (Urgency<O>s m�ssen bereits getaggt sein) 
      ...
      return true; //Schleife regul�r durchlaufen
    } finally {
