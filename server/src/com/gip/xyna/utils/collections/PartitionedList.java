...
/**
 * Liste die intern aus zwei Partitionen besteht. Elemente 0 bis n geh�ren zu Sorte E, die danach 
 * folgenden zu Sorte F.
 * Die erste Partition ist beschr�nkt. Falls zuviele Elemente in der Liste sind, werden die hinteren Elemente
 * zu Sorte F transformiert.
 * Falls die erste Partition schrumpft, werden Elemente aus der zweiten Partition wieder zur�cktransformiert 
 * und in Partition E geladen.
 * 
 * Es wird nur garantiert, dass nie mehr Elemente von Sorte E in der Liste sind als �ber die Beschr�nkung
 * angegeben, es k�nnen aber weniger sein.
 * 
 * UseCase: Teure, dicke Objekte in Partition E, billige Identifier-Objekte in Partition F.
 * 
 * ACHTUNG: Damit contains, containsAll, remove, removeAll oder retainAll funktionieren k�nnen, 
 * muss E eine equals-Methode haben, die die Transformation E -&gt; F -&gt; E �berlebt. 
 * D.h. die nicht �berschriebene Default-Equals-Implementierung �ber den Identity-Hashcode 
 * funktioniert h�chstwahrscheinlich nicht! 
 * 
...
public class PartitionedList<E, F> extends AbstractList<E> {
  ...
  public interface Transformator<E, F> {
    ...
    /**
     * Muss derart transformiert werden, dass man das urspr�ngliche Objekt vergessen werden kann
     */
    public F transformTo(E object);

    /**
     * E-object geht nicht verloren, das transformierte Objekt wird nur vorr�bergehend ben�tigt.
     */
  ...
  private final List<E> innerListTypeA; //beschr�nkt
  private final ReadWriteLock l = new ReentrantReadWriteLock();
  private final List<F> innerListTypeB; //TODO alternative implementierung k�nnte dieses objekt auch persistieren.
  ...
  public PartitionedList(Transformator<E, F> transformator, int maxNumberOfElementsOfTypeA) {
    ...
    lowerBoundSize = (int) Math.round(maxNumberOfElementsOfTypeA * 0.65); //65% unter maxwert => nachladen auf ~82% bef�llung
    upperBoundSize = maxNumberOfElementsOfTypeA;
  }
  
  public void addFirst(E o) {
    add(0, o);  //vorne einf�gen, nicht in der der N�he der Transformations-Grenze
  }
    ...
    private void checkUpperBoundInIter() {
      if (innerListTypeA.size() >= upperBoundSize) {
        ...
        for (int i = 0; i < innerListTypeA.size() - targetSize; i++) {
          //previous elemente entfernen und an id-liste anh�ngen
    ...
    private void checkLowerBoundInIter() {
      if (innerListTypeA.size() <= lowerBoundSize) {
        if (innerListTypeB.size() > 0) {
          ...
          //zur�ck zum urspr�nglichen index:
