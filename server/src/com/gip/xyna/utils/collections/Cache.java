...
public class Cache<U, V> {
  ...
  /*
   * problem: ein thread entfernt eines oder mehrere objekte aus dem cache, und gleichzeitig l�uft die
   ...
   * es darf nie passieren, dass der cache ein objekt enth�lt, welches begonnen wurde zu erstellen, bevor es
    ...
    while (value == null || value == PLACE_HOLDER) {
      ...
      if (value == null) {
        ...
        if (((ConcurrentHashMap)map).putIfAbsent(key, PLACE_HOLDER) == null) {
          //place holder, damit man bemerkt, wenn der cache geleert wird, w�hrend man den value erstellt
