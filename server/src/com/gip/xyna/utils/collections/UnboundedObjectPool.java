 ...
 * 
 * Es werden nur Objekte verwaltet, die gerade frei sind, d.h. mit get() herausgegeben k�nnen. sobald sie herausgegeben wurden, kann der Verwender
 * mit den Objekten machen was er will.<p>
 * 
 * Objekte im Pool werden aufger�umt, wenn sie so lange wie der angegebene Timeout nicht verwendet wurden.
 * Dabei wird nicht garantiert, dass die Identit�t der Objekte ber�cksichtigt wird, d.h. alle Objekte sind gleichwertig. 
 * Es kann also sein, dass ein Objekt aufger�umt wird, welches kurze Zeit vorher noch verwendet worden ist.
 * <p>
 * 
 ...
 * CachedObject cached = pool.get();
    ...
    Alternativ kann man es nat�rlich auch jedes Mal testen oder sowas
 *
 */
public class UnboundedObjectPool<T extends Closeable> {
   ...
   * [2] -> vor 5 min benutzt
   * falls timeout nun 18 min ist, wird in 3 min das �lteste objekt aufger�umt, d.h. danach ist der stand dann:
   * [0] -> vor 13 min genutzt
   * [1] -> vor 8 min genutzt
   * 
   * beim get() wird immer der niedrigste eintrag (h�chster index) entfernt
   * beim addIfNeeded() wird ein neuer eintrag angelegt (h�chster index) mit (vor 0 min genutzt)
   */
  private final List<Long> lastAccess = new ArrayList<>();
  private final int timeoutClose; //wenn ein objekt solange nicht benutzt wurde, wird es aufger�umt
  private final int minSize; //wenn weniger objekte im pool sind, wird beim addIfNeeded sicher hinzugef�gt.
  //objekte beim addIfNeeded ablehnen, falls es ein objekt gibt, was so lange nicht in verwendung ist. muss <= timeoutClose sein
  //idee: wieso neues objekt hinzuf�gen, wenn es noch so lange nicht verwendete objekte gibt...
  ...
  /**
   * F�gt Objekt zu Pool hinzu, wenn minSize nicht erreicht ist, oder wenn das �lteste Objekt weniger lange als timeoutDenyNew wartet.
   * @return gibt false zur�ck, falls Objekt nicht zum Pool hinzugef�gt worden ist. In diesem Fall ist der Aufrufer f�r das Objekt zust�ndig (close aufrufen)
   */
  ...
  /**
   * r�umt alle objekte im pool auf
   */
  public void close() {
    synchronized (objects) {
      checkClosed();
