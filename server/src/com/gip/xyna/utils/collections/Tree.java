...
public class Tree<K,V> {
  ...
  /**
   * Existieren Branches?
   * @return TreeStatus.Unknown, falls dies nicht bekannt ist, weil nicht vollst�ndig initialsiert, 
  ...
  /**
   * Hat der Baum einen Zweig mit dem angegebenen Namen? Es werden nur Unterzweige �berpr�ft, 
   * nicht der aktuelle Zweig.  
   * @param branchKey
   * @param depthLimit
   * @return TreeStatus.BranchFound, wenn ein Zweig gefunden wurde.
   *   Ansonsten TreeStatus.Unknown, wenn ungef�llte Zweige existieren;
   *   TreeStatus.LimitReached, wenn die Suchtiefe depthLimit �berschritten wurde;
  ...
  /**Gibt aus der Predecessor-Hierarchie den ersten gefunden Eintrag zur�ck, 
   * der entweder nicht gef�llt ist oder das depthLimit �berschreitet.
   * @param depthLimit
  ...
  /**
   * Liefert alle Knoten des Baums zur�ck
   * @param depthLimit
   * @return
   */
  ...
  private void fillAllTreeNodes(HashMap<Tree<K, V>, Integer> all, int depth, int depthLimit ) {
    ...
    //Reihenfolge hier etwas komplizierter, daf�r aber immer Eintr�ge mit minimaler depth 
   ...
   * wahrscheinlich die Maximale Suchtiefe �berschritten worden.
   * @param depthLimit
   * @return
   */
   ...
   * Sie gibt unter Umst�nden ein falsches Ergebnis "false" zur�ck,
   * wenn im Baum noch ungef�llte Zweige vorliegen. 
   * @return
   */
  ...
  /**
   * Austausch der Zweige der beiden B�ume
   * @param tree1
   * @param tree2
   */
   ...
   * @param treeBuilder Interface, welches die konkreten Einzelaufgaben beim Baum-Bauen l�st
  ...
  private static class RecursiveTreeBuilder<K,V,T extends Tree<K,V>> {
    ...
    private LinkedHashSet<K> cycleData = new LinkedHashSet<K>(); //cycleData dient zur Detektion 
                                                                 //von zirkul�ren Referenzen beim F�llen des Baums.
    ...
    private void createTreeBranches(T tree) {
      ...
      if( branches == null ) {
        //tree muss weiter ungef�llt bleiben
        unfillableTrees.add(tree);
      } else {
        //Branches anlegen und f�llen
  ...
  public interface TreeBuilder<K,V,T extends Tree<K,V>> {
    ...
    /**
     * Liefert einen Tree-Knoten zur�ck, entweder neu gebaut oder ein bereits bestehender Baum. 
     * @param key
     * @return
     */
    T createTree( K key );
    ...
    /**
     * Wird f�r ungef�llte Tree-Knoten gerufen, um diese zu f�llen
     * @param key Identifikation des Tree-Knotens
     * @return alle Branch-Keys, null falls Tree-Knoten ungef�llt bleiben muss
     */
