...
/**
 * map, aus der value objekte sicher entfernt werden k�nnen, wenn sie von keinem anderen client mehr verwendet werden.
 ...
 * dass nicht ein anderer thread gleichzeitig das zu entfernende objekt noch als g�ltig sieht.<p>
 * 
 * beispiel: value der map ist eine liste, die nur dann entfernt werden soll, wenn sie leer ist.<p>
 * 
 * achtung: um synchronisierung des values an sich muss man sich selbst k�mmern.<p>
 * 
 * verwendungsmuster:
 * <pre>
 ...
 *   ....
 *   
 *   //es ist sichergestellt, dass l nun nicht aus der map entfernt wird, solange f�r das remove nur cleanup() verwendet wird
 *   ....
 * 
 *   l.remove(element);
 * } finally {
...
public abstract class ConcurrentMapWithObjectRemovalSupport<K, V extends ObjectWithRemovalSupport> extends ConcurrentHashMap<K, V> {
   ...
   * ansonsten wird das existierende objekt zur�ckgegeben.
  ...
  public V lazyCreateGet(K key) {
    ...
    while (true) {
      ...
      //object wurde gerade wieder gel�scht
    }
    return v;
  }
  ...
  /**
   * tr�gt �bergebenes objekt falls nicht vorhanden in map ein.
   * ansonsten wird das existierende objekt zur�ckgegeben.
   * markiert objekt als in benutzung, sodass es nicht aus der map entfernt werden kann,
   * solange man nicht {@link #cleanup(Object)} aufgerufen hat.
   */
    ...
    if (v.shouldBeDeleted()) {
      if (v.markForDeletion()) {
        ...
        if (v.shouldBeDeleted()) {
          ...
          //anderer thread hatte es nach unserer ersten �berpr�fung kurz gehabt und ge�ndert
          v.unmarkForDeletion();
        }
      }
    }
  ...
  /**
   * verarbeitung des values zum key ohne sich um das create und cleanup k�mmern zu m�ssen.
   */
  ...
  //wenn man alles l�schen w�rde, kann es zu NPEs kommen, weil der obige code davon ausgeht, sich auf die markierungen verlassen zu k�nnen
  //TODO eigtl ben�tigt man entsprechende behandlung auch f�r iterator-remove, normales remove, etc
  /**
   * l�scht jedes key value paar, dessen value nicht in benutzung ist ohne �berpr�fung von {@link ObjectWithRemovalSupport#shouldBeDeleted()}
   */
  @Override
  public void clear() {
    for (K key : keySet()) {
