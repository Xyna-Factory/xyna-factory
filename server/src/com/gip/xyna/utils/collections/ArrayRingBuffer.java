...
public class ArrayRingBuffer<E> {
  ...
  private int start; //N�chster zu belegender Index
  ...
  /**
   * RingBuffer Inhalt wird als Array zur�ckgegeben. �ltestes Element ist an Stelle 0, neustes Element an Stelle (size-1).
   * 
   * Falls �bergebenes Array zu klein ist, wird ein neues erzeugt, ansonsten wird es bef�llt. 
   * @return
   */
  ...
  public E[] getOrdered(E[] array) {
    ...
    if (size == capacity && start != 0) {
      System.arraycopy(arr, start, ret, 0, capacity-start); //alten objekte fangen da an, wo der n�chste einf�gepunkt ist
      System.arraycopy(arr, 0, ret, capacity-start, start); //neuen objekte enden da, wo der n�chste einf�gepunkt ist
    } else {
  ...
  /**
   * @param relativeIndex 0 -&gt; �ltestes element
   * @return
   */
  @SuppressWarnings("unchecked")
