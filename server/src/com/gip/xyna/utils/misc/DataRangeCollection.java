 ...
 * die bestimmung dieser teilmenge passiert �ber eine datasource, die im konstruktor �bergegeben wird. 
 ...
 * 
 * den ausgezeichneten werten k�nnen auch noch values zugeordnet werden (int). diese haben dann immer g�ltigkeit bis zum n�chsten ausgezeichneten wert. 
 * 
 * 
 * beispiel f�r verwendung: siehe unittests.
 * 
 * eine m�glichkeit f�r die verwendung ist z.b. eine art versionscache. wenn man wissen m�chte, welche versionsnummern es gibt, und die anfrage lazy gemacht wird,
...
public class DataRangeCollection {
  ...
  //TODO performance: binary search statt linearer suche in den ranges verwenden (insbesondere, wenn die anzahl der ranges gr��er wird)

  public interface DataSource {

    /**
     * f�gt alle datapoints zum set dazu, die zwischen start und end liegen
     */
  ...
  private static class Interval {
    ...
    /*
     * i-ter value ist der value f�r das i-te intervall zwischen datapoints.
  ...
  /**
   * mittels der datasource werden datapoints im gegebenen interval hinzugef�gt
   */  
  ...
  private void handleMerge(long start, long end, List<Interval> touched) {
    ...
    List<Interval> interim = createInterim(touched); //zwischenst�cke erzeugen
  ...
  private Interval merge(long start, long end, List<Interval> touched, List<Interval> interim) {
    ...
    //merge nach und nach jeweils ein interim mit dem n�chsten touched interval. alles zusammen ergibt das gro�e neue gemerg-te interval
    for (int i = 1; i < touched.size(); i++) {
      ...
      if (touched.get(i).values[0] > 0) {
        //ersten wert aus touched �bernehmen
  ...
  /**
   * f�gt zum set die existierenden datapoints zwischen den beiden �bergebenen punkten hinzu
   * es wird nur unterst�tzt, dass start und end vollst�ndig in einem existierenden intervall liegen.
   * @param start
   * @param end
  ...
  /**
   * falls noch kein intervall oder nur intervalle mit gr��eren werten existieren, wird ein intervall angelegt, welches nur diesen punkt enth�lt.
   * falls bereits ein intervall mit kleineren werten als der �bergebene punkt existiert, wird dieses erweitert bis zum �bergebenen punkt.
