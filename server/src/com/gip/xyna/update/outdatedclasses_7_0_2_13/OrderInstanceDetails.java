...
public class OrderInstanceDetails extends OrderInstance {
  ...
  public void convertAuditDataToXML() throws XPRC_CREATE_MONITOR_STEP_XML_ERROR, Ex_FileAccessException {
  ...
  public boolean convertAuditDataToXML(Long revision, boolean removeAuditDataAfterwards) throws XPRC_CREATE_MONITOR_STEP_XML_ERROR, Ex_FileAccessException {
    if (auditData != null) {
      ExecutionType type;
      try {
      ...
      if (XynaProperty.ORDER_INSTANCE_BACKUP_STORE_AUDITXML_BINARY.get()) {
        auditDataAsXML = null;
        auditDataAsXMLb = auditData.toXML(getId(), getParentId(), getStartTime(), getStopTime(), getExceptions(), type, revision, removeAuditDataAfterwards);
      } else {
        auditDataAsXML = auditData.toXML(getId(), getParentId(), getStartTime(), getStopTime(), getExceptions(), type, revision, removeAuditDataAfterwards);
  ...
  private static class OrderInstanceDetailsReader implements ResultSetReader<OrderInstanceDetails> {
    ...
    public OrderInstanceDetails read(ResultSet rs) throws SQLException {
      ...
      OrderInstance.fillByResultSet(oi, rs, true);
      oi.auditData =
  ...
  public void updateProcessInformationForServiceDestination(XynaOrderServerExtension order) {
    if (order.getExecutionDestination() instanceof ServiceDestination) {
      ...
      auditData.setProcessIfUnset(sd.getOriginalFqDataTypeName() + "." + sd.getServiceName(), -1L /* should not be called during update */); 
    }
  }
