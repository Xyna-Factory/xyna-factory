    ...
    try {
      ...
      try {
        ...
        for (Role role : roles) {
          ...
          //Datenmodell-Rechte zu Rollen hinzuf�gen, die vorher die Rechte EDIT_MDM
          ...
          //DeploymentMarker-Rechte zu Rollen hinzuf�gen, die vorher das Recht EDIT_MDM hatten
          if (editMdmRight) {
            newScopedRights.add(ScopedRight.DEPLOYMENT_MARKER.allAccess());
          }
          
          //DeploymentItem-Rechte zu Rollen hinzuf�gen, die vorher die Rechte EDIT_MDM,
          ...
          //OrderInputSource-Rechte zu Rollen hinzuf�gen, die vorher das Recht EDIT_MDM hatten
          if (editMdmRight) {
            newScopedRights.add(ScopedRight.ORDER_INPUT_SOURCE.allAccess());
          }
          
          //CronLikeOrders-Rechte zu Rollen hinzuf�gen, die vorher das Recht EDIT_MDM hatten
          if (startOrderRight) {
            newScopedRights.add(ScopedRight.CRON_LIKE_ORDER.allAccess());
          }
          
          //Read-OrderTypes-Recht zu allen Rollen hinzuf�gen, die nicht bereits ein Ordertypes-Recht besitzen
          if (!orderTypesRight) {
            newScopedRights.add(ScopedRight.ORDER_TYPE.getKey() + ":" + Action.read + ":*:*:*");
          }
          
          //Read-Capacities-Recht zu allen Rollen hinzuf�gen, die nicht bereits ein Capacities-Recht besitzen
          ...
          //Sichtbarkeitsrechte f�r GUI zu allen Rollen hinzuf�gen
