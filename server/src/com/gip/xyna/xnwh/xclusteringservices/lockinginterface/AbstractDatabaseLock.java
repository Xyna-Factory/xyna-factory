...
public abstract class AbstractDatabaseLock implements DatabaseLock {
  ...
  /**
   * Persistiert das ClusteringServicesLockStorable. Falls dies nicht klappt, werden Retries durchgef�hrt, solange der
    ...
    try {
      ...
      if (this.useConnection) {
        try {
          ...
          //Fehler tritt nur in Ausnahmef�llen im ClusterState DISCONNECTED_SLAVE auf,
   ...
   * durchgef�hrt, solange der ClusterState != DISCONNECTED_SLAVE ist Ansonsten wird abgebrochen mit der zuletzt
  ...
  private void lockStorable() throws PersistenceLayerException, InterruptedException, LockFailedException {
    ...
    do {
      try {
        ...
        persistStorable(); //wirft Exceptions, daher Austieg, falls persistStorable auch auf Fehler st��t
        retryException = null;
      } catch (PersistenceLayerException e) {
