 ...
 * Dazu wird eine tempor�re Datei angelegt, auf welcher die Dateioperationen durchgef�hrt werden.
 * Beim commit-Aufruf wird die tempor�re Datei umbenannt. Dieses rename wird mittels eines OS-Aufrufs
 * durchgef�hrt und sollte atomar sein (in jedem Fall unter *nix).
 * 
 * In dieser Klasse findet kein Locking zur Vermeidung von konkurrierenden Dateioperationen statt. Dies
 * muss, wenn gew�nscht, in bei der Verwendung selbst gemacht werden.
 * 
...
public class TransactionFile extends File {
  ...
  // File-Objekt, f�r welches ein transaktionsgesicherter Zugriff gew�hrleistet wird
  ...
  /**
   * Konstruktor erzeugt neues File-Objekt, auf dem Dateioperationen durchgef�hrt werden k�nnen,
   * die bis zu einem commit die Originaldatei nicht modifizieren.
   * 
   * @param fileObject  Datei, f�r welche eine tempor�re Datei angelegt werden soll 
   */
   ...
   * 
   * @param fileObject  Datei, f�r welche eine tempor�re Datei angelegt werden soll
   * @param directoryPath   Ordner, in dem tempor�re Datei gespeichert wird 
   */
  ...
  public boolean checkNotModified() {
    //beim commit wird das file �berschrieben, und damit wieder writeable gemacht
  ...
  /**
   * F�hrt alle �nderungen auf der Datei durch. 
   * Commit darf nur einmal ausgef�hrt werden. F�r
  ...
  public void commit() {
    ...
    if (!exists() && deleted) {
      ...
      // Unter Windows muss Datei ggf. vorher gel�scht sein, bevor sie umbeannt werden kann --> Verlust der Atomarit�t
  ...
  /**
   * Kopiert Dateiinhalt von Originaldatei zur tempor�ren Datei, um an dieser Daten anh�ngen zu k�nnen.
   */
