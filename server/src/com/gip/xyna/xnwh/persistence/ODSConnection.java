...
public interface ODSConnection extends Connection {
  ...
  /**
   * falls listenToPersistenceLayerChanges=true, bleibt zur�ckgegebenes command wirksam, wenn die zugrundeliegende
   * persistencelayer ge�ndert wird. achtung, diese preparedcommands werden nicht garbagecollected, bis zum
   * servershutdown
   */
  ...
  /**
   * falls listenToPersistenceLayerChanges=true, bleibt zur�ckgegebene query wirksam, wenn die zugrundeliegende
   * persistencelayer ge�ndert wird achtung, diese preparedqueries werden nicht garbagecollected, bis zum servershutdown
   */
  ...
  /**
   * wird auch ausgef�hrt, wenn rollback nicht funktioniert.
   */
  ...
  /**
   * Stellt sicher, dass alle ben�tigten Connections vorliegen, um auf die �bergebenen Storables zugreifen zu k�nnen.
  ...
  /**
   * verbindet die andere connection mit dieser (und andersherum), so dass intern ben�tigte connections aus dem gleichen connectionpool nicht doppelt aufgemacht werden.
   * genauer: falls nach diesem aufruf neue connections aufgemacht werden (in einer der beiden connections), wird falls m�glich eine bestehende connection wiederverwendet.
   * 
   * beispiel:
   * OrderArchive persist-&gt;HISTORY + delete from OrderBackup.DEFAULT sollen �ber den gleichen Connectionpool gehen.
   */
