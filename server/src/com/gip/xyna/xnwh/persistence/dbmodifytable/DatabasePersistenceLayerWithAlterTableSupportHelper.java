...
public class DatabasePersistenceLayerWithAlterTableSupportHelper {
  ...
  public static <T extends Storable> void addTable(DatabasePersistenceLayerConnectionWithAlterTableSupport connection,
    ...
    if (tableExists) {
      ...
      //typen von spalten ermitteln und auf kompatibilit�t pr�fen
      //falls nicht kompatibel => fehler werfen.
      //falls nicht existent => spalte hinzuf�gen
  ...
  public static <T extends Storable> void checkColumn(DatabasePersistenceLayerConnectionWithAlterTableSupport connection,
    ...
    if (columnsAreCompatible) {
      //zb varchar / varchar
      //muss noch �berpr�fen, ob varchar gr��e auch ok ist.
      if (colInfo.isTypeDependentOnSizeSpecification()) {
        if (col.size() > 0) {
          if (col.size() > colInfo.getCharLength()) {
            //TODO das ist eigtl etwas gemogelt, die gr��e k�nnte f�r verschiedene typen auch verschiedene auswirkungen haben.
        ...
        } //else keine gr��e angegeben im storable, dann ist sie wohl nicht so wichtig
      } //else dann passen die typen unabh�ngig von einer gr��enangabe im colInfo.type
    ...
    //k�nnte zb sein, dass vorhanden=TINYINT, recommended=MEDIUMINT
