...
public class QueryGenerator {
  ...
  public QueryPiplineElement<?, ?> parse(XMOMStorableStructureInformation info, List<String> selectedUnresolvedColumns, IFormula condition,
    ...
    FormulaParsingResult fpr = PersistenceExpressionVisitors.parseFormula(condition, info, paras);
  ...
  QueryPiplineElement<?, ?> build(XMOMStorableStructureInformation info, List<String> selectedUnresolvedColumns, FormulaParsingResult fpr,
    ...
    if (querySplitNecessary) {
      ...
      DataQueryPiplineElement dataQuery = new DataQueryPiplineElement(buildDataQuery(queryRoot, selectedColumns, aliasDictionary),
                                                                      ...
                                                                      info.getColInfoByVarType(VarType.PK));
      ...
      queryBuilder.append(" FROM ")
                  .append(queryRoot.localInfo.getTableName());
  ...
  private void appendSortCriterion(SortCriterion[] sortCriterion, StringBuilder queryBuilder, XMOMStorableStructureInformation info, AliasDictionary dictionary) {
    if (sortCriterion!= null && sortCriterion.length > 0) {
      ...
      for (SortCriterion sc : sortCriterion) {
        ...
        queryBuilder.append(" ");
        if (dictionary == null) {
          queryBuilder.append(orderBy.getColumn().getParentStorableInfo().getTableName()).append(".").append(orderBy.getColumn().getColumnName());
        } else {
          queryBuilder.append(dictionary.getTableAlias(orderBy.getAccessPath())).append(".").append(orderBy.getColumn().getColumnName());
        }
        if (sc.isReverse()) {
  ...
  private String buildSelection(QueryTreeNode queryTreeNode, List<QualifiedStorableColumnInformation> columns, AliasDictionary dictionary) {
    ...
    for (QualifiedStorableColumnInformation column : columns) {
      ...
      if (isJoinedReachable(queryTreeNode, column)) {
        ...
        if (table == null) {
          build.append(column.getColumn().getParentStorableInfo().getTableName());
        } else {
          build.append(table);
        }
        build.append('.')
  ...
  private static String buildJoinCriterion(QueryTreeNode node, boolean useInnerJoinForCurrentNode, AliasDictionary dictionary) {
    StringBuilder build = new StringBuilder();
  ...
  private static String buildJoinCriterionRecursivly(QueryTreeNode node, boolean useInnerJoinForCurrentNode, AliasDictionary dictionary) {
    StringBuilder build = new StringBuilder();
  ...
  private static String buildJoinCriterion(QualifiedStorableColumnInformation datatypeColumnToJoin, boolean useInnerJoin, AliasDictionary dictionary) {
    return buildJoinCriterion(datatypeColumnToJoin.getColumn(), datatypeColumnToJoin.getAccessPath(), useInnerJoin, dictionary);
  }
  
  
  private static String buildJoinCriterion(StorableColumnInformation column, List<StorableColumnInformation> accessPath, boolean useInnerJoin, AliasDictionary dictionary) {
    ...
    build.append(" JOIN ")
      .append(column.getStorableVariableInformation().getTableName());
    if (ownTable != null) {
      build.append(" ")
           .append(ownTable);
    }
    build.append(" ON ");
    if (parentTable == null) {
      build.append(column.getParentStorableInfo().getTableName());
    } else {
      build.append(parentTable);
    }
    build.append('.');
    if (column.getStorableVariableType() == StorableVariableType.EXPANSION) {
      build.append(column.getParentStorableInfo().getPrimaryKeyName())
           .append(" = ");
      if (ownTable == null) {
        build.append(column.getStorableVariableInformation().getTableName());
      } else {
        build.append(ownTable);
      }
      build.append('.');
      ...
      build.append(parentFk.getColumnName());
    } else {
  ...
  String buildDataQuery(QueryTreeNode qtn, List<QualifiedStorableColumnInformation> columns, AliasDictionary dictionary) {
    ...
    if (order.size() > 0) {
      ...
      while (orderByIterator.hasNext()) {
        ...
        queryBuilder.append(dictionary.getOrCreateColumnAlias(orderbb)).append(" DESC"); //damit das rootelement am ende kommt (hat als listen-index �berall NULL)
  ...
  String buildDataQueryRecursivly(QueryTreeNode currentNode,
    ...
    if (currentNode.accessPath.size() <= 0) {
      queryBuilder.append(currentNode.localInfo.getTableName());
    } else {
      queryBuilder.append(currentNode.accessPath.get(0).getParentStorableInfo().getTableName());
    }
  ...
  String buildConditionQuery(XMOMStorableStructureInformation rootInfo, Collection<QualifiedStorableColumnInformation> columnsWithConditions, AliasDictionary dictionary, boolean count, QueryParameter queryParameter) {
    ...
    queryBuilder.append(" FROM ").append(rootInfo.getTableName());
   ...
   * (1) %0% gilt implizit und liefert nur Prim�rschl�ssel (uid und timestamp) zur�ck.
   * (2) POINTER_AUF_REFERENZIERTE_DATEN liefert deren Prim�rschl�ssel
   * (3) POINTER_AUF_SIMPLE_DATEN gibt simple Werte zur�ck
   * (4) POINTER_AUF_EXPANDIERTE_DATEN gibt Werte der simplen Membervariablen und Prim�rschl�ssel referenzierter Member zur�ck. Listen und komplexe Membervariablen werden ausgespart.
   * (5) POINTER_AUF_DATEN.* liefert rekursiv alle simplen und expandierten Membervariablen zur�ck, sowie alle Prim�rschl�ssel referenzierter Membervariablen.
      ...
      for (String variableAccess : unresolvedTransformedColumns) {
        ...
        for (int currentPosition = 1; currentPosition < variableAccessParts.length; currentPosition++) {
          ...
          if (currentAccessPart.equals("*")) { // (5)
            ...
            if (info == null) {
              //evtl wegen flattening nichts zu finden. vom flatteningtransformator wurden aber ja vollst�ndige pfade bereits ersetzt. d.h. es kann sich
              //h�chstens um einen teilweisen pfad handeln. z.b. ist a.b.c.d eine in a geflattete referenz. dann k�nnte der accesspath a.b.c.* oder a,b.c sein.
              ...
              for (QualifiedStorableColumnInformation entry : currentContext.getColumnInfoAndPath(ColumnInfoRecursionMode.ONLY_LOCAL)) {
                if (entry.getColumn().isFlattened() && entry.getColumn().getPath().startsWith(relativePathToParentXMOMStorable + ".")) {
                  ...
                  //geflachte member die einen tieferen pfad haben, nur dann hinzuf�gen, wenn accessPath mit * endet.
                  if (endsWithStar || 
  ...
  private void appendConditionOnCurrentVersionIfNecessary(StringBuilder queryBuilder, Parameter params, XMOMStorableStructureInformation info,
    ...
    if (info.usesHistorization() && !queryParameter.queryAcrossHistory()) {
      ...
      queryBuilder.append(info.getTableName())
                  .append(".")
                  .append(columnName)
                  .append(" = ?");
      params.add(true);
  ...
  static class DataQueryPiplineElement<P, T>  extends QueryPiplineElement<P, T> {
    ...
    DataQueryPiplineElement(String sqlString, ResultSetReader<T> reader, StorableColumnInformation rootPk) {
      this(sqlString, reader, new Parameter(), rootPk);
    }
    
    DataQueryPiplineElement(String sqlString, ResultSetReader<T> reader,  Parameter params, StorableColumnInformation rootPk) {
    ...
    protected void prepare(Collection<P> previousResult) {
      ...
      conditionBuilder.append(" WHERE ").append(rootPk.getParentStorableInfo().getTableName()).append(".")
                      .append(rootPk.getColumnName());
