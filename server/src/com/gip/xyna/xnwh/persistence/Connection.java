...
public interface Connection {
  /**
   * �bernimmt vorgenommene �nderungen dieser transaktion und gibt locks wieder frei
   * @throws PersistenceLayerException
   */
  public void commit() throws PersistenceLayerException;
  /**
   * verwirft vorgenommene �nderungen dieser transaktion und gibt locks wieder frei
   * @throws PersistenceLayerException
   */
  ...
  /**
   * insert+update von objekten. die objekte darin sollten  �ber annotations persistable und column
   * konfiguriert sein. beim udpate werden immer alle spalten aktualisiert. m�chte man nur einzelne
   ...
   * 
   * @param cmd
   * @param paras werden, anders als bei {@link #query(PreparedQuery, Parameter, int)}, direkt �bernommen,
   ...
   * @param query
   * @param parameter 
   *        Anfragen verwenden " um Bereiche und \ um einzelne Zeichen zu Escapen. Bei Like-Anfragen m�ssen alle
   *        %, die nicht als Wildcards interpretiert werden sollen, escaped werden.
   *        Alle unescapten % m�ssen durch PersistenceLayer-spezifische Wildcards ersetzt werden.
   *        Au�erdem m�ssen alle PersistenceLayer-spezifischen Steuerzeichen escaped werden.
   *        zB. soll "name like '%baum"%"wald%'" oder "name like '%baum\%wald%'" die Zeichenfolge baum%wald treffen k�nnen.
   * @param maxRows -1 f�r unbegrenzt. ansonsten gibt es maximal soviele elemente zur�ck
   * @return
   * @throws PersistenceLayerException
   */
  ...
  //TODO:*querymethode, die nur die referenzen zur�ckgibt, falls m�glich. d.h. ohne resultsetreader benutzung
  //     *update-methode, die java-updates auf objekten definiert, die per query ausgesucht werden. das update wird dann "unten"
  //        ausgef�hrt. vorteil: zb in clusterpersistencelayer braucht man nicht die gesamten objekte �ndern, sondern kann die
  //        transformationen speichern. in anderen persistencelayers kann man sie u.a. in sql-updates umwandeln oder
  //        direkt ausf�hren
  
  /**
   * f�llt das �bergebene objekt �ber seine defaultquery und nimmt als parameter den primarykey der in dem �bergebenen
  ...
  /**
   * @return true, falls ein objekt mit dem gleichen PK wie das �bergebene bereits existiert
   */
  public <T extends Storable> boolean containsObject(T storable) throws PersistenceLayerException;

  /**
   * f�hrt ein upsert durch, d.h. vorhandene Zeilen (erkannt an id) werden geupdated, nicht vorhandene zeilen werden eingef�gt
   * @param <T>
  ...
  /**
   * Hiermit k�nnen Transaction Properties an den PersistenceLayer durchgereicht werden. Diese werden je nach
