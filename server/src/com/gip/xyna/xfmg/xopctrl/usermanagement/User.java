...
public class User extends Storable<User> {
  ...
  public void throwPasswordExpiredException() throws XFMG_UserAuthenticationFailedException {
    if (XynaProperty.PASSWORD_EXPIRATION_EXCPETION_UNIQUE.get()) {
      throw new XFMG_PasswordExpiredException(name); //eindeutige Exception f�r abgelaufene Passw�rter
    }
    throw new XFMG_UserAuthenticationFailedException(name);
  }
  ...
  /**
   * �berpr�ft, ob das Passwort bereits einmal verwendet wurde.
   * @param newPassword
  ...
  /**
   * Setzt das Passwort im User. Wird auch aufgerufen, wenn das Passwort nur neu verschl�sselt wird, ohne
   * dass es ge�ndert wurde.
   */
