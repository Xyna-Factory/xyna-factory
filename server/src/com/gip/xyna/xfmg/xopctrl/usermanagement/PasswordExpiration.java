...
public class PasswordExpiration implements Serializable{
  ...
  private Duration remainingDuration; //verbleibende Zeitdauer, bis das Passwort abl�uft, falls state=limited

  
  public static enum ExpirationState {
    unlimited(false),   // unbegrenzt g�ltig
    limited(false),     // l�uft nach einer bestimmten Zeit ab
    expired(true),      // bereits abgelaufen (d.h. G�ltigkeitsdauer �berschritten)
    setByAdmin(true);   // ung�ltig weil durch Admin gesetzt
    
    private boolean invalid; //gibt an, ob das Passwort ung�ltig ist und nicht mehr verwendet werden darf
  ...
  /**
   * Ermittelt die aktuelle G�ltigkeitsdauer f�r das Passwort des Users.
   * @param user
   * @return
   */
  public static PasswordExpiration calculate(User user) {
    if (passwordMustBeChanged(user.getPasswordChangeReasonEnum())) {
      return PasswordExpiration.setByAdmin(); //Passwort wurde durch Admin gesetzt und muss nun durch Benutzer ge�ndert werden
    }
  ...
  /**
   * Ermittelt, ob das Passwort des Users noch ge�ndert werden darf.
   * @param user
   * @return
   */
  public static PasswordExpiration changeAllowed(User user) {
    ...
    if (passwordMustBeChanged(user.getPasswordChangeReasonEnum())) {
      expirationDays = 0; //Passwort wurde durch Admin gesetzt und muss nun durch Benutzer innerhalb der changeAllowedDays ge�ndert werden
    }
    
    if (expirationDays < 0 || changeAllowedDays < 0) {
      return PasswordExpiration.unlimited(); //nicht abgelaufen oder �nderung immer erlaubt
    }
