...
@XynaObjectAnnotation(fqXmlName = "xact.ldap.TrustEveryone")
public class TrustEveryone extends SSLParameter {

  private static final long serialVersionUID = -1L;
  private static final Logger logger = CentralFactoryLogging.getLogger(TrustEveryone.class);


  protected static class InternalBuilder<_GEN_DOM_TYPE extends TrustEveryone, _GEN_BUILDER_TYPE extends InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>> extends SSLParameter.InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>  {

    protected InternalBuilder(TrustEveryone instance) {
      super(instance);
    }

    public TrustEveryone instance() {
      return (TrustEveryone) instance;
    }

  }

  public static class Builder extends InternalBuilder<TrustEveryone, Builder> {
    public Builder() {
      super(new TrustEveryone());
    }
    public Builder(TrustEveryone instance) {
      super(instance);
    }
  }

  public Builder buildTrustEveryone() {
  ...
  protected void fillVars(TrustEveryone source, boolean deep) {
    super.fillVars(source, deep);
  }

  public TrustEveryone clone() {
    return clone(true);
  }

  public TrustEveryone clone(boolean deep) {
    TrustEveryone cloned = new TrustEveryone();
  ...
  public String toXml(String varName, boolean onlyContent, long version, com.gip.xyna.xdev.xfractmod.xmdm.GeneralXynaObject.XMLReferenceCache cache) {
    ...
    if (!onlyContent) {
      ...
      XMLHelper.beginType(xml, varName, "TrustEveryone", "xact.ldap", objectId, refId);
    } else {
      objectId = -1;
    }
    if (objectId != -2) {
  ...
  */
  ...
  public static Field getField(String target_fieldname) throws InvalidObjectPathException {
    ...
    try {
      foundField = TrustEveryone.class.getDeclaredField(target_fieldname);
