...
public class ScopedRightCache {
  ...
  private void distributeRightNodes(String[] scopeParts, int index, BranchingTreeNode currentNode) {
    ...
    //abh�ngig vom Typ wird der Rechtebereich nun in eine der Maps des aktuellen Knotens eingetragen
    ...
    //den n�chsten Teil des Rechts untersuchen
  ...
  /**
   * Liefert f�r einen scopePart, der Anteile enth�lt die ausgeschlossen werden sollen,
   * ein Pair aus einem Pattern das zutreffen soll und einer Liste der Pattern, die
   * nicht zutreffen d�rfen
   * @param excludingPart
   * @return
   */
  private static Pair<Pattern, List<Pattern>> generatePatternForExcluding(String excludingPart) {
    ...
    //diese Pattern d�rfen nicht zutreffen
  ...
  private static enum ScopedRightPartType {
    ...
    /**
     * der scopePart enth�lt mindestens ein '*' als Wildcard
     */
    PARTIAL_WILD,
    
    /**
     * der scopePart enh�lt keine Wildcards oder Ausschl�sse
     */
    PRIMITIVE,
    
    /**
     * der scopePart enth�lt Teile, die ausgeschlossen werden sollen,
     * z.B. A*!{Ab,*e} liefert alle Strings, die mit 'A' anfangen, au�er 'Ab' und denen, die mit 'e' aufh�ren
     */
    EXCLUSION;
    ...
    public boolean covers(String[] scopeParts, int index) {
      ...
      for (Pair<Pair<Pattern,List<Pattern>>, BranchingTreeNode> patternedPair : excludingBranches.values()) {
        ...
        if (patternedPair.getFirst().getFirst().matcher(scopeParts[index]).matches()) {
          //die Pattern aus dem zweiten Teil d�rfen nicht zutreffen
