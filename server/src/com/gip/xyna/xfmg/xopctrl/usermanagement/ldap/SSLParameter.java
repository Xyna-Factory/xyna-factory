...
@XynaObjectAnnotation(fqXmlName = "xact.ldap.SSLParameter")
public abstract class SSLParameter extends XynaObject {

  private static final long serialVersionUID = -1L;
  private static final Logger logger = CentralFactoryLogging.getLogger(SSLParameter.class);
  ...
  protected static class InternalBuilder<_GEN_DOM_TYPE extends SSLParameter, _GEN_BUILDER_TYPE extends InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>>  {
    ...
    public SSLParameter instance() {
      return (SSLParameter) instance;
    }
  ...
  protected void fillVars(SSLParameter source, boolean deep) {
  }

  public abstract SSLParameter clone();

  public abstract SSLParameter clone(boolean deep);
  ...
  public String toXml(String varName, boolean onlyContent, long version, com.gip.xyna.xdev.xfractmod.xmdm.GeneralXynaObject.XMLReferenceCache cache) {
    ...
    if (!onlyContent) {
      ...
      XMLHelper.beginType(xml, varName, "SSLParameter", "xact.ldap", objectId, refId);
    } else {
      objectId = -1;
    }
    if (!onlyContent) {
