...
/**
 * sort daf�r, dass {@link ContainerClass} von diesem classloader geladen wird, und das laden nicht delegiert wird. nur
 * das laden von abh�ngigen klassen wird an den parent delegiert.
 */
public class ClassLoaderWrapper extends ClassLoader {
 ...
 /*   if (EXECUTION_DURING_TESTS) {
      if (new File("classes.test").exists()) { //FIXME: das geh�rt nicht in den produktivcode
        try {
  ...
  protected synchronized Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
    ...
    if (c == null) {
      if (name.equals(ContainerClass.class.getName())) {
        //daf�r ist der classloader wrapper gedacht: die containerklasse nicht mit einem fremden classloader laden,
        //sondern mit diesem, damit man kontrolle �ber die parent-classloader hat
        c = findClass(name);
      } else {
        //alle anderen klassen k�nnen nun delegiert werden, automatisch zuerst an den gew�nschten parent-classloader
