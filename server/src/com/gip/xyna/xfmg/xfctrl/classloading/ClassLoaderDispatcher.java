...
/**
 * schnittstelle f�r alles was mit classloading zu tun hat verwaltet classloaders f�r diverse objekte: - mdm datentypen
...
public class ClassLoaderDispatcher extends FunctionGroup {
  ...
  //Outdated Filter in einer eigenen Map halten, da diese zus�tzlich eine parentRevision haben
  ...
  /**
   * sucht auch in abh�ngigen anderen revisions, falls nicht in dieser gefunden 
   */
  ...
  public Class<XynaObject> loadMDMClass(String name, boolean createClassLoaderIfNotExisting, String originalXmlPath,
    ...
    //      eine map name->classloader w�re z.b. gut
    Set<Long> requirements = new HashSet<Long>();
    XynaFactory.getInstance().getFactoryManagement().getXynaFactoryControl().getRuntimeContextDependencyManagement().getDependenciesRecursivly(revision, requirements);
    for (long req : requirements) {
    ...
    if (rootRevision != null) {
      //es l�uft gerade eine deserialisierung, und es ist zus�tzlicher revision kontext bekannt
  ...
  /**
   * sucht classloader in der entsprechenden map oder falls nicht gefunden �ber die revision-dependencies 
   */
  ...
  */
   ...
   * anderen classloader ausser dem �bergebenen ber�hrt werden (zb implizit durch deployment/instanziierung
   * von zugeh�rigen mdmobjekten oder sowas).
  ...
  /**
   * �ber ClassLoaderBuilder kann ein neuer ClassLoader angelegt werden, falls er noch nicht existiert
   */
  ...
  /**
   * gibt classloader aus entsprechender map zur�ck 
   */
  public ClassLoaderBase getClassLoaderByType(ClassLoaderType clt, String classLoaderName, Long revision,
                                              Long parentRevision) {
  ...
  private void redeployRMIImpls() {
    ...
    // rmi k�nnte eine beliebige mdm klasse erzeugen => immer redeploy!
    try {
  ...
  private void reloadSharedLibInternally(final String name, boolean reloadRMI,
    ...
    //w�hrend einem reloadSharedLib d�rfen die Trigger und Filter nicht enabled werden
  ...
  /**
   * l�d abh�ngige filter classloader auch neu
   */
  public void reloadTrigger(final String fqTriggerClassName, final String[] sharedLibs, final Long revision) throws XFMG_ClassLoaderRedeploymentException {
    ...
    if (tcl != null) {
      ...
      // alles locken w�hrend des reloads 
      for (Lock lock : allLocksAsWriteLockInOrderForDeadLockSecurity) {
        lock.lock();
      }
      try {
        tcl.reloadDependencies(new ClassLoaderSwitcher() {
          ...
          public void switchClassLoader() throws XynaException {
            ...
            // dependencies nach "oben" �ndern sich nicht
  ...
  /**
   * in anderen classloader, die den �bergebenen classloader als dependencyToReloadIfClassLoaderChanges eingetragen haben,
   * den �bergebenen classloader als dependency entfernen.
   * d.h. anpassung der classloader von objekten, die von dem �bergebenen classloader verwendet werden.
   */
  ...
  private void removeFromAllDependenciesToReloadInOtherClassLoaders(String fqClassName, ClassLoaderType type,
      ...
      for (MapWrapper<String, ClassLoaderBase> map : classLoaderMap.get(t).values()) {
        for (ClassLoaderBase clb : map.map.values()) {
          if (clb.getRevision().equals(revision)
               || deps.contains(clb.getRevision()) //abh�ngige Revisions
    ...
    //TODO die OutdatedFilter m�ssten wahrscheinlich gar nicht durchsucht werden, da sie von niemandem
  ...
  private class ClassLoaderDispatcherDeploymentHandler_UndeployOld implements DeploymentHandler {
    ...
    public void begin() throws XPRC_DeploymentHandlerException {
      //FilterClassLoader wollen sich das Lock holen, was zu Deadlocks f�hren kann. Deshalb bereits hier holen
      XynaFactory.getInstance().getActivation().getActivationTrigger().lockManagement();
    }
  ...
  private class ClassLoaderDispatcherDeploymentHandler_ReloadClassLoaders implements DeploymentHandler {
    ...
    public void exec(GenerationBase object, DeploymentMode mode) throws XPRC_DeploymentHandlerException {
      ...
      if (object instanceof WF) {
        ...
        if (createNewClassLoader) {
          ...
          if (!firstClassLoaderCreation) {
            // dependencies nach "oben" �ndern sich nicht
            //ist nur f�r datentypen notwendig, weil der classloader oben entfernt und neu angelegt wurde
    ...
    private void classloaderDependencyHandling(ClassLoaderBase newClassloader, GenerationBase object, boolean firstClassLoaderCreation,
      ...
      //dependencies entfernen, die beim "deployNew" wieder hinzugef�gt werden. 
  ...
  /*
   * 
   * dependencies werden erst entfernt, und dann nach dem deployment wieder hinzugef�gt, damit
   * es w�hrend des deployments keine unn�tigen classloader abh�ngigkeiten gibt, die eh durch das deployment
   * bereits ausgetauscht+deployed werden.
   * 
   * das entfernen passiert �ber removeFromAllDependenciesToReloadInOtherClassLoaders
   * 
   * usecase: deploying B. 
   * (pfeil bedeutet "x benutzt y")
   * abh�ngigkeitsbaum vorher               nachher
   * 
   *     A                                    A
   *     |                                    |
   *     v                                    v
  ...
  private void addDependencies(GenerationBase o, ClassLoaderType type) throws XPRC_DeploymentHandlerException {
    for (GenerationBase gb : getDependentObjectsUsedByClassLoader(o)) {
      ...
      if (gb instanceof WF) {
        // keine wf abh�ngigkeit, weil der wf �bers processing aufgerufen wird
  ...
  private Set<GenerationBase> getDependentObjectsUsedByClassLoader(GenerationBase o) {
    ...
    /*
     * eigtl m�ssten die dependencies gen�gen, die beim classloading hinzugef�gt werden.
     * 
     * leider gen�gen die nicht, sondern m�ssen um folgende dependencies erweitert werden:
     * f�r jede methoden signatur (instanzmethode oder statische methode)
     *   f�r alle inputs und outputs
     *     abh�ngigkeit setzen, dass datentyp mit den methoden diese typen verwendet.
     * 
     * 
     ...
     *        wenn das passieren w�rde, w�re alles gut.
     *        
     *        dadurch wei� der server nicht, dass y von x benutzt wird (classloading-technisch) und es passiert kein class-reloading von x
     *        nachdem y neu erzeugt wurde. das w�re erstmal nicht kritisch - aber das verify ist offenbar dumm, und sieht dann zwei verschiedene
     *        y, die vom gleichen x benutzt werden k�nnten und signalisiert deshalb einen verifyerror
     *        
     *        => es gibt verschiedene m�glichkeiten dem server beizubringen, dass y von x benutzt wird:
     *        1) wie hier: explizit abh�ngigkeiten setzen
     *        2) daf�r sorgen, dass classloader x das loadclass auf y aufruft
    ...
    return ret;
    //holzhammer methode von fr�her erzeugt viel zu viele abh�ngigkeiten: return o.getDependenciesRecursively().getDependencies(false);
  }
  ...
  /**
   * wenn ein classloader tempor�r entfernt wird (usecase: deploymentproblem), muss man sich
   * f�r das n�chste deployment die classloadingdeps merken, weil die verwendenden objekte
  ...
  public List<ClassLoaderBase> recreateClassLoadersWithoutRevisionDependencies(Set<Long> revisionsToRecreate, Set<Long> revisionDependenciesToRemove) {
    ...
    try {
      ...
      //weil er beim handling der dependencies automatisch mit ber�cksichtigt wird
      ...
      try {
        for (ClassLoaderBase clb : classloaders) {
          ...
          try {
            ...
            //weil sp�ter deswegen ein redeployment mit neuer codegenerierung etc durchgef�hrt wird.
            if (logger.isDebugEnabled()) {
              RevisionManagement rm = XynaFactory.getInstance().getFactoryManagement().getXynaFactoryControl().getRevisionManagement();
              String rc;
              try {
  ...
  //aus abw�rtskompatibilit�tsgr�nden: xtf verwendet das
  ...
  public void validateClassLoaders(CommandLineWriter clw) {
    ...
    //TODO missing classloaders: also f�r wieviele objekte im deploymentitemmgmt gilt, dass sie deployed sind, es aber keinen classloader gibt
  }
  ...
  private void validateXMOMStorableStructureCache(CommandLineWriter clw, ClassLoaderBase clb,
    ...
    if (superTypeName.equals(XMOMPersistenceManagement.STORABLE_BASE_CLASS)) { //TODO auch f�r tiefere ableitungshierarchien checken
