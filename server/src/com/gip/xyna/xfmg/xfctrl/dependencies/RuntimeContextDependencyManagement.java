...
public class RuntimeContextDependencyManagement extends FunctionGroup {
  ...
  /*
   * TODO alle verwender sollten aus performancegr�nden evtl besser auf getDependenciesRecursively umgestellt werden, weil sie sonst selbst
   * eine behandlung daf�r ben�tigen, die dependencies nicht doppelt zu �berpr�fen.
   */
  ...
  /**
   * Welche RuntimeContexte ben�tigt owner?
   * @param owner
   * @return
   */
  ...
  public List<RuntimeContext> getParentRuntimeContextsSorted(RuntimeDependencyContext rc) {
    ...
    //f�r die gui so sortieren, dass immer erst die kinder, dann die parents benachrichtigt werden
  ...
  /**
   * �berpr�ft (rekursiv), ob die childRevision in der Abh�ngigkeitshierarchie der parentRevision enthalten ist
   */
  ...
  /**
   * gibt die revision zur�ck, in der originalXmlName definiert ist, abh�ngig von den aktuellen dependencies von parentRevision 
   * 
  ...
  public synchronized void modifyDependencies(RuntimeDependencyContext owner, Collection<RuntimeDependencyContext> newDependencies, String user, boolean force, boolean publishChanges, IRuntimeDependencyLock lock) throws PersistenceLayerException, XFMG_CouldNotModifyRuntimeContextDependenciesException {
    ...
    try {
      ...
      if (hasRevision(owner)) {
        ...
        try {
          ...
          try {
            ...
            //XMOM Updates f�r den owner und alle Parents
            //(f�r Application Definitions muss kein XMOM Update geschickt werden)
        ...
        // regenerate der objekte, f�r die sich der deploymentstatus ge�ndert hat (zum guten oder schlechten)
        ...
        //bei �nderungen an Requirements einer ApplicationDefinition muss der ApplicationDefintion-Cache im ApplicationManagement geleert werden
      ...
      if (publishChanges) {
        //Multi-User-Event f�r Dependency �nderungen am owner
  ...
  public synchronized void modifyDependencies(Collection<RuntimeContextDependencyChange> changes, String user, boolean force, boolean publishChanges, IRuntimeDependencyLock lock) throws PersistenceLayerException, XFMG_CouldNotModifyRuntimeContextDependenciesException {
    ...
    try {
      ...
      /*
       * callsites in den neu hinzugef�gten revision wieder herstellen
       * am einfachsten w�re: in allen parent-revisions statereports neu berechnen
       * das w�re aber auch teuer und macht potentiell viel arbeit die unn�tig ist.
       * => wie kann man das reduzieren?
       * nur f�r alle invalidierten callsites in den removedrevisions den statereport neu erzeugen. (entweder sind diese objekte in der menge der "toRegenerate", oder
       * sie sind in den neuen addedrevisions aufl�sbar).
       * noch sparsamer w�re: die invalidierten callsites direkt in den addedrevisions eintragen ohne den statereport der betroffenen objekte
       * neu zu berechnen (f�r die objekte, die nicht in toRegenerate sind, d.h. ihr call muss ja weiterhin funktionieren).
       * aber das w�re fehleranf�lliger - vielleicht sp�ter, falls von der performance her gew�nscht.
       * 
       */
      for (Long revisionToRegenerate : toRegenerate.keySet()) {
        DeploymentItemRegistry dir = dism.getRegistry(revisionToRegenerate);
  ...
  public static class RuntimeContextDependencyChange {
    ...
    //transient nur als marker, dass die variablen tempor�r sind
  ...
  private void updateDependencyRegister(Map<Long, Map<XMOMType, Collection<String>>> toRegenerate,
    ...
    //dependencyregister aufr�umen, evtl schl�gt das unten stehende deployment ja fehl und kommt nicht bis zum deploymenthandler, der das auch tun w�rde
    ...
    if (removedRevisions.size() > 0) {
      //die nicht neu generierten objekte, die dependencyregister eintr�ge haben, m�ssen diese evtl neu erstellen, weil sie nun auf eine andere revision zeigen
      //es muss nach benutzten dr-eintr�gen gesucht werden, die auf eine der gel�schten revs zeigen (oder auf eine der children davon, die nicht mehr erreichbar sind)
      for (RuntimeContext parentRC : runtimeContextsToPublishAsXMOMUpdate) {
        ...
        try {
          ...
          //ignore, das muss oben dr�ber sicher gestellt werden, dass das hier nicht passiert
          continue;
        }
        ...
        for (Entry<XMOMType, Collection<String>> e : getAllObjects(rev).entrySet()) {
          ...
          for (String name : xmomObjects) {
            if (regeneratedObjects != null && regeneratedObjects.contains(name)) {
              //deployment hat die dependencyregistereintr�ge bereits neu angelegt
              continue;
            }
            if (type.equals(XMOMType.DATATYPE) || type.equals(XMOMType.EXCEPTION) || type.equals(XMOMType.WORKFLOW)) {
              DependencyNode dependencyNode = dr.getDependencyNode(name, DependencySourceType.from(type), rev);
  ...
  /**
   * s.u. gleichnamige methode. aber berechnung passiert, bevor die dependencies ge�ndert worden sind
   */
  private Set<Long> collectRevisionsNotReachableAnyMoreBeforeChange(Long parentRev, Set<Long> removedRevisions, Set<Long> addedRevisions) {
    // returns {removedRevisions} U {children von removedRevisions} \ {children von parentRev nach �nderung}
  ...
  /**
   * removedRevisions waren urspr�nglich von parentRev erreichbar, aber jetzt nicht mehr.
  ...
  private void addDependenciesToParentWorkspace(ApplicationDefinition applicationDefinition, Collection<RuntimeDependencyContext> newDependencies, String user, boolean force, boolean publishChanges) throws PersistenceLayerException, XFMG_CouldNotModifyRuntimeContextDependenciesException {
    ...
    //die Abh�ngigkeiten der ApplicationDefinition zu den Workspace Abh�ngigkeiten hinzuf�gen (falls nicht bereits vorhanden)
    ...
    for (RuntimeDependencyContext rc : newDependencies) {
      if (hasRevision(rc)) { //nur Abh�ngigkeiten zu anderen Workspaces und Applications vererben
  ...
  // additive �nderungen ohne Sperrung zulassen
  ...
  private ValidationResult validate(RuntimeDependencyContext owner, Collection<RuntimeDependencyContext> newDependencies) throws XFMG_DependencyStillUsedByApplicationDefinitionException, XNWH_OBJECT_NOT_FOUND_FOR_PRIMARY_KEY, PersistenceLayerException {
    //falls Dependencies von Workspaces entfernt werden, d�rfen sie nicht als Dependency bei
...
/**
 * �berpr�ft ob eine von einem Workspace zu entfernende Abh�ngigkeit noch von einer 
 * ApplicationDefinition verwendet wird.
 * @param owner
 * @param newDependencies
 * @throws XFMG_DependencyStillUsedByApplicationDefinitionException falls eine zu entfernende Abh�ngigkeit noch verwendet wird
 */
  ...
  */
  ...
  private class RuntimeDependencyLock implements IRuntimeDependencyLock {
    ...
    /**
     * stoppt applications und trigger, um sie nach der dependency-�nderung wieder zu starten
     * locking �ber DeploymentManagement
     */
    public void lock(boolean workflowProtectionNecessary) {
  ...
  private class TriggerAndFilterHandling {
    ...
    /**
     * sammelt 
     * - trigger instanzen, die in kind-revisionen sind, die wegen dem change neu gestartet werden m�ssen, damit entweder filter 
     *   entfernt oder hinzugef�gt werden.
     ...
     *   sie eine abh�ngigkeit vermissen und nach dem change dann funktionieren k�nnten.
     *   
     * TODO: objekte, die durch das entfernen von dependencies ung�ltig werden, sollten nicht nur laufzeitm�ssig deregistriert 
    ...
    private void handleTriggerAndFilter() {
      ...
      try {
        ...
        //[1] -> die objekte, die eigtl entfernt werden m�ssen (also whitelist)
    ...
    private boolean filterInstanceIsDependentOnRemovedRevs(FilterInstanceInformation fii) throws PersistenceLayerException {
      ...
      RevisionManagement rm = XynaFactory.getInstance().getFactoryManagement().getXynaFactoryControl().getRevisionManagement();
      /*
       * checken, ob zugeh�riger trigger in zu entfernenden revisions (inkl dependencies) enthalten ist.
       * es gen�gt, dies f�r den trigger zu checken, weil triggerinstanz+filter auch den trigger verwenden.
       * 
       * falls ja, ist die filterinstance betroffen, und muss neu geladen werden (inkl zugeh�riger triggerinstance)
       ...
       * alle diese f�lle sind durch disable+enable gut abgehandelt.
       */
      ...
      try {
        try {
          ...
          //ok, evtl wird das ja durch das �ndern der dependencies gefixt.
          return true;
        }
  ...
  //wird derzeit nicht verwendet, soll sp�ter bei der migration laufender auftr�ge helfen
  ...
  public static boolean getThreadLocalValueForDeserializationWithNewRevisions() {
    //h�sslich, dass man immer threadlocals anlegt und wieder removen muss. threadlocal ben�tigt einen getter, der readonly ist.
  ...
  private static class ValidationResult {
    ...
    //sortiere statetransitions in die, f�r die der state sich �ndert, und in die, wo objekte invalide werden
