...
public class DependencyRegister extends FunctionGroup {
   ...
   * 
   * die keys sind leere objekte ohne sets von benutzen/abh�ngigen objekten
   */
  private ConcurrentMapWithObjectRemovalSupport<Long, RemovalConcurrentMap<DependencyNode, DependencyNode>> setOfAllNodes;
  {
    ...
    //andere komponenten greifen auf dependencyregister zu, init() wurde aber noch nicht ausgef�hrt wegen abh�ngigkeiten
  ...
  static {
    try {
      // alle klassen die einen deploymenthandler definieren m�ssen hier rein! schliesslich sollen diese auch schon
      // aufgerufen werden, wenn die workflows der wfdb geladen werden. d.h. beim serverstart m�ssen die deployment
  ...
  /**
   * Entfernt eine Abh�ngikeit zwischen usedNode und noMoreUsedBy.
   * Falls noMoreUsedBy keine weiteren Abh�ngikeiten hat, wird er aus setOfAllNodes entfernt
   * @param usedNode
   * @param noMoreUsedBy
   * @return
   */
  ...
  /**
   * Entfernt einen DependencyNode und alle seine Abh�ngigkeiten
   * @param name
   * @param type
   * @param revision
   */
  ...
  void removeAllDependencies(DependencyNode toBeRemoved, Long revision) {  
    ...
    try {
      ...
      //vorw�rts und r�ckw�rts referenzen aus dep-register entfernen
  ...
  /**
   * Entfernt oldNode und �bertr�gt alle Abh�ngigkeiten auf newNode.
   * @param oldNode
   * @param newNode
   * @param revision
   */
  private void exchangeAllDependencies(DependencyNode oldNode, DependencyNode newNode, Long revision) {  
    ...
    try{
      ...
      //vorw�rts und r�ckw�rts referenzen aus dep-register �bertragen
      for (DependencyNode dependent : removed.getDependentNodes()) {
        dependent.removeUsedNode(removed);
  ...
  /**
   * Tauscht den DependencyNode des fqClassNames mit dem f�r den xmlName aus
   * @param fqClassName
   * @param xmlName
   * @param revision
   */
  ...
  /**
   * wenn sich vorw�rtsreferenzen (wen benutze ich) �ndern, will man die alten �ndern und die neuen eintragen.
   * die r�ckw�rtsreferenzen �ndern sich dadurch nicht (wer benutzt mich).
   */
  ...
  /**
   * dependencies aus additional dependencies zu using node hinzuf�gen  
   */
  ...
  /**
   * aus den used nodes der �bergebenen node werden alle nodes entfernt,
  ...
  public void updateRevisionInUsedNodes(DependencyNode dependencyNode, Set<Long> removedRevisions) {
    ...
    for (DependencyNode dn : dependencyNode.getUsedNodes()) {
      if (removedRevisions.contains(dn.getRevision())) {
        ...
        if (!dn.getRevision().equals(newRev)) { //notwendig, weil parentRevision nicht die revision sein muss, in der sich abh�ngigkeiten ge�ndert haben (sondern ein parent davon)
          if (dependencyNode.removeUsedNode(dn)) {
            dn.removeDependentNode(dependencyNode);

            if (newRev == null) {
              //unerwartet, h�tte eigtl vom aufrufer bereits erkannt werden m�ssen. jetzt kann man aber schlecht nen rollback durchf�hren. also nur loggen
