...
public class DeploymentItemBuilder {
   ...
   * damit verbleibt das problem der outputs, die mit dem output des von der inputsource referenzierten workflows �bereinstimmen m�ssen. 
   */
    ...
    public boolean resolve() {
      ...
      try {
        ...
        if (executionDestination == null || !executionDestination.getFQName().equals(fqWFName)) {
          //derzeit nicht unterst�tzt, dass man einen ordertype verwendet, der einen anderen kompatiblen workflow aufruft
          return false;
        }
  ...
  //verwendung vom ordertype ohne genaueres wissen �ber die eingangsschnittstelle des workflows
  ...
  //verwendung von ordertype und eingangsschnittstelle von workflow. name des workflows ist irrelevant (weil �ber ordertype gestartet), und ausgangsschnittstelle auch
    ...
    public boolean resolve() {
      try {
        ...
        if (ExecutionType.getByTypeString(destination.getValue().getDestinationType()) == ExecutionType.XYNA_FRACTAL_WORKFLOW) {
          ...
          return false; //nicht unterst�tzt
        }
      } catch (XPRC_DESTINATION_NOT_FOUND e) {
        return false;
      }
  ...
  private static Pair<Set<DeploymentItemInterface>, Set<DeploymentItemInterface>> buildWFInterfaces(WF wf) {
    ...
    for (AVariable thrown : wf.getAllThrownExceptions()) {
      if (thrown.isPrototype()) {
        //FIXME TypeOfUsage ist nicht korrekt, m�sste EXCEPTION sein, gibts aber nicht
    ...
    try {
      ...
      //nicht aufl�sbare variable wurden oben bereits ermittelt.
    }
        ...
        if (operations.size() > 0) {
          ...
          for (Operation operation : operations) {
            ...
            try {
              ...
              //ok, nun findet man die nicht aufl�sbaren variablen unten.
            }
  ...
  private static Collection<? extends DeploymentItemInterface> extractFunctionInvocation(StepFunction step) {
    ...
    if (sid.service.isPrototype()) {
      ...
      //TODO nachteil ist hier, dass eigtl das xml auch g�ltig bleiben w�rde, wenn man den wf l�scht und eine servicegroup anlegt, die in einem DT lebt, der genauso heisst.
      //deshalb w�re "UNKNOWN" als type dann resistenter.
...
//                                        ActiveFlagCheckedOperationInterface.of(step, sid)));
  ...
  public static void extractConstants(TypeInterface providingType, Integer stepId, AVariable variable, boolean parentWasList, Set<DeploymentItemInterface> diis) {
    if (variable.hasValue() && variable.isJavaBaseType() && variable.getJavaTypeEnum() == null) {
      //das ist der fall, dass die membervariable unbekannt ist. beim validate w�rde es einen fehler geben
  ...
  private static class AccessChainBuildingVisitor extends EmptyVisitor {
    ...
    //falls innerhalb der funktion ein roottype oder eine chain verwendet wurde, muss diese nun beseitigt werden, damit sie bei der parent-funktion (o.�,) nicht interferiert
    ...
    public void functionEnds(FunctionExpression fe) {
      ...
      if (fe instanceof DynamicResultTypExpression) {
        ...
        if (parentContext.currentChainElementHasIndexDef) {
          ...
          //roottype so lassen
          //TODO �berpr�fen, dass funktion int-r�ckgabe typ kompatibel ist, sonst wird z.b. %0%[appendlist(%1%, "bla")] nicht richtig validiert
        } else {
    ...
    public void functionSubExpressionEnds(FunctionExpression fe, int parameterIndex) {
      if (fe.getFunction().getName().equals(Functions.APPEND_TO_LIST_FUNCTION_NAME) && parameterIndex == 0) {
        //typ des ersten parameters vererbt sich auf r�ckgabe typ von funktion - deshalb diesen typ in parent-chain setzen
        ...
        localContext.chain = null; //nullen f�r den n�chsten parameter
    ...
    public void allPartsOfVariableFinished(Variable variable) {
      //TODO es fehlt eine analoge behandlung noch f�r das enden der chain hinter einem funktionsaufruf (Beispiel: f(x,y).a.b["0"].c)
