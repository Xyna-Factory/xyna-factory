...
class RelationManagementMethods {
  ...
  static abstract class UpdateBackwardRelationsTemplateMethod<F extends XMOMDatabaseEntry, B extends XMOMDatabaseEntry> {
    ...
    /**
     * m�ssen immer in der gleichen reihenfolge zur�ckgegeben werden.
      ...
      keyLoop : for (String primaryKey : primaryKeysOfObjectsFromForwardRelation) {
        ...
        for (XMOMDatabaseType xmomDatabaseType : XMOMDatabaseTypes) {
          ...
          if (!entryToProcess.getRevision().equals(rev)) {
            //r�ckw�rtsbeziehungen in andere revisions nicht eintragen, weil da fehlt uns die m�glichkeit, die revision mit zu speichern.
    ...
    protected void handleObjectNotFoundInAllArchives(ODSConnection con, List<XMOMDatabaseType> allowedXMOMDatabaseTypes, F entryToProcess,
      ...
      //falls das xml nicht existiert, soll das objekt m�glichst trotzdem in die xmomdb eingetragen werden, aber dann ohne label.
      //falls das xml existiert, dann soll es korrekt/vollst�ndig eingetragen werden
      ...
      try {
        ...
        if (rootTag != null) {
          ...
          //tr�gt evtl in datatype ein, anstatt exception...
      ...
      modifyRelations(entryToProcess, entry);
      con.persistObject(entry);
      //FIXME: was, wenn objekt bereits vorhanden war! unerwartet -> rollback und nochmal? geht nicht gut wegen der schleife �ber die objekte...
    }
