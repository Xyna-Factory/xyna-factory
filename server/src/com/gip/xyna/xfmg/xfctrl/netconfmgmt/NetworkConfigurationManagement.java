...
/**
 * verwaltung von logischen namen f�r komponenten des netzwerks, in das die fabrik eingebunden ist.
 * es gibt:
 * netzwerk
 *  - computer
 *    - interfaces
...
public class NetworkConfigurationManagement extends FunctionGroup {
  ...
  protected void init() throws XynaException {
    ...
    if (logger.isTraceEnabled()) {
      ...
      /*
       * TODO konfiguration der caches, weil ansonsten �nderungen /etc/hosts oder �hnliches nicht direkt wirksam werden.
       * dazu gibt es die java-properties:
       *  networkaddress.cache.ttl bzw (abw�rtskompatibilit�t java 1.5?): sun.net.inetaddr.ttl
       *  und
       *  networkaddress.cache.negative.ttl bzw. sun.net.inetaddr.negative.ttl
       *  
       *  mit den werten (-1=cache h�lt FOREVER, 0 = kein cache, x = x sekunden lang cachen)
       *  
       *  achtung: auf 0 setzen ist gef�hrlich, weil dann sehr viele dns requests durchgef�hrt werden. vermutlich ist ein wert im minuten bereich besser
       *  
       *  die settermethoden der werte k�nnen nicht ohne einschr�nkungen zur laufzeit verwendet werden, also entweder 
  ...
  public InternetAddressBean getInternetAddress(String id, DependencyNode user) {
    //TODO f�r den user automatisch eine abh�ngigkeit im dependencyregister anlegen, dass er die id verwendet.
