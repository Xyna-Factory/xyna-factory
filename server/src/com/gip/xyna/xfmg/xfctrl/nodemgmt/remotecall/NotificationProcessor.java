...
public class NotificationProcessor implements Runnable {
  ...
  public enum RemoteCallNotificationStatus {
    ...
    Parked(true),     //Auftrag geparkt f�r Resume
  ...
  public void run() {
    while( running ) {
      try {
        ...
        //ignorieren: Wer diesen Thread beenden m�chte, sollte stopRunning() rufen
      } catch (Throwable t) {
  ...
  public static void processRemoteCallNotification(RemoteCallNotification notification, String nodeName, FactoryNodeCaller fnc) {
    ...
    if( notification.startExecution(nodeName) ) { //zustands�bergang
      ...
      if( status == RemoteCallNotificationStatus.Execution ) {
        ...
        if( notification.changeStatusFromExecutingAndNotify(status) ) { //benachrichtigt einen auf notification.await wartenden thread (gibt es nur f�r startorder/awaitorder) 
           //ok
          // im Falle einer Failed ausf�hrung bei vorher pausiertem processor m�ssen wir jetzt pollen
          fnc.retrieveOrders(); //Zustands�bergang des RemoteProcessors auf Awake
        } else {
          //unerwartet
