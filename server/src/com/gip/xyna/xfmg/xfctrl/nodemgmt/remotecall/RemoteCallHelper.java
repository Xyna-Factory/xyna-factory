...
public class RemoteCallHelper {
   ...
   * Nach Resume muss ein weiterer startRemoteOrder ausgef�hrt werden.
   * @param resumed
  ...
  public Pair<Long,DispatchingTarget> startRemoteOrder(
    ...
    if( resumed ) {
      //Pr�fen, ob Auftrag nicht zulange gewartet hat und daher beendet werden muss
   ...
   * Entweder bis Notification eintrifft oder ein Suspend ausgef�hrt wird.
   * Nach Resume kann ein weiterer awaitOrder ausgef�hrt werden.
   * @param target
   * @param startTimeStamp
   * @param remoteOrderId
  ...
  public void awaitOrder(DispatchingTarget target, long startTimeStamp, Long remoteOrderId) throws XFMG_NodeConnectException, PersistenceLayerException {
    ...
    if( ! suspend ) {
      //wenn nicht gleich eine Suspendierung erfolgt ist nun ein Backup n�tig, 
      //damit RemoteOrderId, StartTimeStamp und FactoryNode nicht verloren gehen k�nnen
