...
public class RemoteOrderStorage extends ObjectWithRemovalSupport {
  ...
  public OrderExecutionResponse createOrderInternal(RemoteCallXynaOrderCreationParameter creationParameter, TransactionMode mode)
    ...
    //Fall k�nnte der Auftrag gecancelt werden
    //Two-Phase-Commit?
  ...
  public List<RemoteData> awaitData(long timeoutMillis) {
    ...
    //TODO evtl geht Verbindung nach den "next.finishOrder()" kaputt: Hier sind Auftr�ge dann als abgeholt markiert,
  ...
  /**
   * R�ckgabe der �bergebenen orderIds, die hier unbekannt sind 
   * @param orderIds
   * @return
   */
  ...
  public void applicationStarted(String applicationName) {
    if (System.currentTimeMillis() - lastInteraction.get() > 1000 * 60 * 10) {
      //aufrufer hat entweder keine verbindung, dann versucht er eh nochmal f�r alle applications nen start
      //oder es interessiert ihn gar nicht mehr, was hier f�r applicationchanges gesammelt sind
      synchronized (changedApplications) {
    ...
    synchronized (changedApplications) {
      if (changeNotificationRequests.remove(applicationName)) {
        ...
        //nun ist sichergestellt, dass ein remotedata f�r diese application an den client verschickt wird
      }
    }
    if (notifyNecessary) {
      if (logger.isDebugEnabled()) {
      ...
      //ein remotedata objekt in der queue gen�gt
  ...
  public void loadStoredResponses() throws PersistenceLayerException {
    //lade responses aus db und stelle sie zur verf�gung, so dass sie abgeholt werden k�nnen
    ...
    try {
      ...
      for (final Long id : l) {
        //TODO performance: f�r jede gespeicherte antwort wird im createRemoteData ein db-zugriff durchgef�hrt. 
        //     stattdessen alle auftr�ge aufeinmal in memory laden.
        //     
        remoteData.add(new RemoteDataCreation() {
  ...
  //response muss sp�ter abgeholt werden
  public void storeResponse(Long orderId, OrderExecutionResponse response, ODSConnection historyCon) throws PersistenceLayerException {
    ...
    try {
      if (historyCon.persistObject(resp)) {
        //dann soll es gel�scht werden, s.u.
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          //delete vor store -> dann insert probieren, damit das store erkennt, dass das delete durchzuf�hren ist.
          if (con.persistObject(resp)) {
            //nun ist das persist doch bereits durchgef�hrt worden
            con.deleteOneRow(resp);
          }
        }
        con.commit();
