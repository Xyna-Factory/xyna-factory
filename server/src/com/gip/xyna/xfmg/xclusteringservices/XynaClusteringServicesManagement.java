...
public class XynaClusteringServicesManagement extends Section implements XynaClusteringServicesManagementInterface {
  ...
  private class MgmtClusterStateChangeHandler implements ClusterStateChangeHandler {
    ...
    private synchronized void tryOnChange(ClusterState newState, boolean checkAgainIsReady) {
      ...
      //wenn vom clusterprovider "onChange" aufgerufen wird, sollte nicht nochmal isReadyForChange �berpr�ft werden!
      //dann ist der cluster zustands�bergang ja bereits passiert!
      if (checkAgainIsReady) {
        isReadyForChange = isReadyForChange(newState);
      }
      if( isReadyForChange && !onChangeRunning) {
        //im eigenen thread ausf�hren, damit nicht statechangehandler rekursiv aufgerufen werden.
    ...
    private class WaitForOnChange extends XynaRunnable {
      public void run() {
        try {
          ...
          //ignorieren, dann war die Wartezeit halt k�rzer
        }
        tryOnChange();
      }
    }
      ...
      public void run() {
        ...
        try {
          ...
          if( success ) {
            ...
            currentClusterState = null; //OnChange ist nicht regul�r fertig geworden, daher ist Zustand unbestimmt
          }
        }
      }
  ...
  private void initXynaClusteringServicesManagement() {
    ods = ODSImpl.getInstance();
             try {
              //achtung, die storables d�rfen nur auf xml konfiguriert sein, weil sonst die abh�ngigkeiten beim laden von odsimpl nicht passen
              ...
              synchronized (clusterInstances) {
                ...
                //aggregations-task f�r alle clusterinstances
                FutureExecution fExec = XynaFactory.getInstance().getFutureExecution();
              ...
              //die clustercomponents haben sich noch nicht registriert -> erst ausf�hren, wenn alle clustercomponents sich hier registriert haben.
              //das funktioniert so, dass die anderen components ein futureexecutiontask definieren m�ssen, mit einer before-beziehung auf diese id.
  ...
  private void registerSavedClusterComponents() throws PersistenceLayerException,
    ...
    //bereits fr�her auf cluster konfigurierte komponenten 
  ...
  /**
   * Reihenfolge der Initialisierung der ClusterComponent ist wichtig, da es gegenseitige Abh�ngigkeitem 
   ...
   * 
   * Sortierung ist: 
   * a) zugeh�riger ClusterProvider, dabei RMIClusterProvider als letzter
   ...
   * TODO Reihenfolge f�r weitere ClusterProvider konfigurierbar machen?
   */
  @Override
  ...
  public void registerClusterableComponent(Clustered clusterableComponent) throws XFMG_ClusterComponentConfigurationException {
    ...
    //falls die clusterablecomponents (fr�her) bereits mit einer clusterinstance verkn�pft wurde, aber diese
    //verkn�pfung nach diesem serverstart noch nicht stattgefunden hat, passiert dies nun.
  ...
  private void registerClusterProvider(final String clusterType, boolean persist)
    ...
    // FIXME SPS prio5: bei classloaderdispatcher registrieren, damit man mit listclassloaderinfos �ber die cli infos �ber
