...
public final class XynaFactory implements XynaFactoryBase {
  ...
  public static XynaFactoryBase getInstance() {
    //TODO f�r bestimmte aufrufer nicht erlauben  Ist dies noch n�tig?
    if (factoryInstance == null) {
      synchronized (XynaFactory.class) {
        if (createdFactoryInstance == null) {
          //hier sollte kein Aufruf innerhalb der Factory hingelangen k�nnen.  
          logger.trace( "XynaFactory instantiated as mock", new Exception("called from") );
          factoryInstance = new XynaFactory(); //FIXME diese Zeile sollte raus! nur wegen Abw�rtskompatibilit�t hier, sollte hoffentlich nie gerufen werden  
        } else {
  ...
  public static XynaFactoryBase createServerInstance() {
    //TODO Pr�fen dass nur von XynaFactoryCommandLineInterface.init aufgerufen wird
    if (factoryInstance == null) {
      synchronized (XynaFactory.class) {
        if (createdFactoryInstance == null) {
          createdFactoryInstance = new XynaFactory(); //einzige Stelle, bei der regul�r die Factory instantiiert wird
        }
        factoryInstance = createdFactoryInstance;
      }
    }
  ...
  public static void setInstance(XynaFactoryBase xfi) {
    //zum testen! //TODO �berpr�fen, dass man das darf
    factoryInstance = xfi;
  }
  
  
  /**
   * Zur�cksetzen der Instance auf die bei Serverstart angelegte Instance
   */
   ...
   */
  //ist z.B. w�hrend der Updatephase true, wenn da UpdateGeneratedClasses o.�. l�uft
  ...
  /**
   * gibt true zur�ck, wenn die fabrik instanz existiert und kein mock ist
   */
  ...
  public void init() throws XynaException {
    ...
    GenerationBase.removeFromCache = false; //f�r updates und wf-database
    isStartingUp = true;

    //bugz 11042: fehler im server.policy file fr�hzeitig entdecken
    ...
    //ConnectionPools sollen das Cleanup �ber das ShutdownHookManagement ausf�hren
    ...
    //zwei getrennte future-execution instanzen, eine f�r die tasks die bis hierhin passieren sollen
    //und eine f�r die danach
    ...
    // => dann sind sie im state cleanup, haben aber keine deploymenthandler ausgef�hrt. das muss dann noch geschehen.
    GenerationBase.clearGlobalCache();  

    // instantiate IDGenerator so that it registers itself as a late init component
    idGenerator = IDGenerator.getInstance();
    //durch ein Update wurde in XynaOrder evtl. f�r den idGenerator 'null' gechached, daher hier noch einmal explizit setzen
    ...
    //Pr�fung des MDM-Verzeichnisses
    checkDirs();
  ...
  public void shutdown() {
    ...
    try {
      ...
      XynaFactoryCommandLineInterface.shutdown(); //ruft �ber XynaFactoryCommandLineInterface-Thread shutdownComponents() auf
  ...
  public void shutdownComponents() throws XynaException {
    ...
    //2. dann erst den Cluster verlassen: dann wird anderer Knoten DISCONNECTED_MASTER und �bernimmt Auftr�ge, 
    //   die in stopGracefully hoffentlich beendet wurden.
    //3. TODO weitere Reihenfolge �berdenken: sollte CronLikeScheduler nich fr�her beendet werden?
        
    
    //damit nicht jemand das shutdownlock holen kann, w�hrend die komponenten bereits am runterfahren sind.
    //falls jemand das lock bereits h�lt, muss er es freigeben, bevor das shutdown fortsetzen darf.
    //TODO f�r den fall, dass die application irgendwie h�ngt, die das lock h�lt, sollte man hier noch einen weg 
    //erm�glichen, dass der shutdown irgendwann forciert fortgesetzt wird. timeout ist gef�hrlich. vielleicht ein cli-befehl?
    if (!shutdownLock.writeLock().tryLock()) {
      if( shutdownLockCause.equals(FACTORY_SHUTDOWN) ) {
        //Lock-�bergabe an XynaFactoryCommandLineInterface-Thread sollte innerhalb weniger Millisekunden stattfinden.
      ...
      for (Entry<Class<? extends XynaFactoryComponent>, List<XynaFactoryPath>> e : allDependenciesOfAllComponents
        ...
        if (!foundHigherDependency) {
          ...
          if (nextToBeInitialized != null) {
            ...
            //null kann nur passieren, wenn eine komponente mehrfach als abh�ngig von der gleichen komponente angegeben wurde.
            ...
            //beispiel: section rmichannelimplsessionextension ist von rmichannelimpl abgeleitet. rmichannelimpl k�nnte abh�ngigkeiten haben, dann gibt es diesen fehler.
  ...
  public static ReturnCode getStatusCodeSLESLike() {
    if (factoryInstance == null) {
      return ReturnCode.STATUS_STOPPING; // kann beim startup eine sehr sehr kurze Phase null sein und beim Shutdown eine 
                                                              // l�ngere Phase (wenn Shutdown-Hooks sehr lange brauchen zBsp)
    }
