...
//TODO XynaRunnables sollten entscheiden k�nnen, ob sie gequeued werden d�rfen
public class XynaThreadPoolExecutor extends ThreadPoolExecutor implements StatisticsReporterLegacy {
  ...
  public interface ThreadPoolUsageStrategy {
    ...
    /**
     * Pr�fung, ob XynaRunnable direkt von einem Thread ausgef�hrt werden darf
     * @param xynaRunnable
     * @return
     */
    ...
    /**
     * Vor dem Ausf�hren des XynaRunnable gerufen
     * @param xynaRunnable
     */
  ...
  /**
   * Weitere Threads �ber corePoolSize werden erst verwendet, wenn Queue voll ist
   *
   */
  ...
  public static abstract class AbstractThreadPoolSizeStrategy implements ThreadPoolSizeStrategy {
    ...
    public synchronized void adaptThreadPoolSize() {
      ...
      if( waitingTasks > 0 ) {
        //muss Pool wirklich vergr��ert werden?
    ...
    /**
     * Liefert die Anzahl der tats�chlich wartenden Tasks. (es k�nnen sich gleichzeitig Task in der Queue
     * befinden und Threads warten, dass sie etwas auslesen k�nnen)
     * @return
     */
  ...
  /**
   * Pool soweit vergr��ern, dass alle wartenden Tasks ausgef�hrt werden k�nnen:
   * Alle Tasks werden sofort neuen Thread gegeben, die Queue wird erst gef�llt, wenn die
   * maximale Gr��e des ThreadPools erreicht ist.
   */
  public static class EagerThreadPoolSizeStrategy extends AbstractThreadPoolSizeStrategy {
    ...
    protected synchronized int calculateNextPoolSize(int effectivelyWaiting) {
      //Pool soweit vergr��ern, dass alle wartenden Auftr�ge ausgef�hrt werden k�nnen
  ...
  /**
   * ThreadPool wird nur vergr��ert, wenn gen�gend Tasks in der Queue warten:
   * Gr��e des Pools = CorePoolSize + Queue-Size/Lazyness
   */
  public static class LazyThreadPoolSizeStrategy extends AbstractThreadPoolSizeStrategy {
    ...
    @Override
    protected synchronized int calculateNextPoolSize(int effectivelyWaiting) {
      //Pool soweit vergr��ern, dass ein Teil der wartenden Auftr�ge ausgef�hrt werden kann
  ...
  public static class RingBufferQueueingStrategy implements QueueingStrategy {
    ...
    /**
     * gebe runnable zur�ck, welches nicht ausgef�hrt, aber auf dem noch nicht "rejected()" ausgef�hrt wurde oder null, falls das runnable gequeued werden konnte.
     * &lt;=&gt; gebe nur runnable zur�ck, wenn das �bergebene runnable nicht gequeued werden konnte.
     */
    public Runnable exchangeRejected(Runnable r) {
      //anstelle des neuen nicht ausf�hrbaren runnables wird das �lteste vorhandene in der queue verworfen
      ...
      if( eldest == null ) {
        ...
        if( eldest instanceof XynaRunnable ) {
          ...
          return null; //Rejection wurde durchgef�hrt
        } else {
  ...
  private static BlockingQueue<Runnable> createPrioritizedQueueIfNecessary(BlockingQueue<Runnable> queue,
    ...
    if( queue instanceof PrioritizedCallerQueue ) {
      return queue; // ist bereits gew�nschte Queue 
    } else if( queue == null ) {
      //es wird eine Queue ben�tigt, daher anlegen 
  ...
  private static class WrappedRejectedExecutionHandler implements RejectedExecutionHandler {
    ...
    public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
      ...
      if( executor instanceof XynaThreadPoolExecutor ) {
        //FIXME: exchange sollte nicht das "rejected()" ausf�hren, sondern das geh�rt hier in die methode.
    ...
    if( threadPoolUsageStrategy.isExecutionPossible(xr) ) {
      ...
      if (!getQueue().offer(command)) {
        ...
        if (getActiveCount() == 0 && getQueue().size() > 0) {
          //bugz 18281: racecondition, dass executionpossible false zur�ckgegeben hat,
          //nun aber keine aktiven threads mehr existieren und deshalb das runnable nicht aus der queue entnommen wird
          //mit dem einstellen eines leeren runnables wird getriggert, dass weitere runnables aus der queue verarbeitet werden k�nnen.
  ...
  public int getCorePoolSize() {
    //muss �berschrieben werden, da interne corePoolSize ge�ndert wird
    return corePoolSize;
  }
