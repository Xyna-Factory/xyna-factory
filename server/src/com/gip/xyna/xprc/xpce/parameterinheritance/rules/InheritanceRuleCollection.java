...
public class InheritanceRuleCollection implements Serializable {
  ...
  //Cache f�r bereits ermittelte Regeln f�r eine Hierarchie von OrderTypes.
  ...
  /**
   * Liefert die Regel mit der h�chsten Precedence, die in der InheritanceRuleCollection enthalten ist
   * und die zur �bergebenen Aufrufhierarchie passt.
      ...
      for (InheritanceRule rule : inheritanceRules.values()) {
        if (rule.matches(childHierarchy)) { //Regel muss zur aktuellen Aufrufhierarchie passen
          if (comparator.compare(rule, preferredRule) > 0) {
            //Regel mit h�herer Precedence gefunden
            preferredRule = rule;
          }
        }
      }
