...
/**
 * Klasse gibt die Reihenfolge und die zugeh�rige Fehlerbehandlung f�r alle Einzel Schritte des Masterworkflows im
 * Processing nach dem Scheduling an. Alle Fehler vor der Ausf�hrung der Execution-Destination f�hren zur Ausf�hrung des
 * Cleanups. Alle Fehler w�hrend des Cleanups werden in der XynaOrder geloggt und dann der n�chste Schritt im Cleanup
 * ausgef�hrt. technisch: Die Fehlerbehandlung von Einzelschritten ist in der Methode
...
public class MasterWorkflowPostScheduler extends XynaRunnable {
  ...
  private static final Executable execution = new Executable() {
    ...
    public void execute(MasterWorkflowPostScheduler mw) throws XynaException {
      ...
      //fehler vor execution f�hren zu cleanup
  ...
  private static final Executable cleanup_batch = new Executable() {
    public void execute(MasterWorkflowPostScheduler mw) throws XynaException {
      //Falls der Auftrag ein BatchProcess-Master ist, geht der BatchProcess jetzt in den Status Cleanup �ber
  ...
  private static final Executable finish = new Executable() {
    ...
    public void execute(MasterWorkflowPostScheduler mw) throws XynaException {
      ...
      while (true) {
        ...
        try {
          //sp�ter: wenn die connections bereits vorher zugemacht werden, dann braucht man in BatchProcess.terminateMaster keinen eigenen thread.
          ...
          //FIXME es ist fragw�rdig, ob das wirklich eine so gute idee ist, hier retries zu machen.
          //wenn der auftrag das commit auf die DEFAULT connection (insbesondre orderarchive) bereits durchf�hren konnte,
          //dann funktioniert der erneute aufruf von finish_archive nicht so gut. es k�nnen dann keine auditdaten aus DEFAULT geholt werden.
          //es sollte eigtl auch wie im archive() ein fehler dazu f�hren, dass auf ALTERNATIVE ausgewichen wird, damit die daten nicht verloren gehen.
  ...
  /**
   * L�sst nur {@link ProcessSuspendedException} und die von Department.handleThrowable weitergeworfenen Errors durch,
  ...
  /**
   * L�sst nur die von Department.handleThrowable weitergeworfenen Errors durch,
   * Alles andere wird gefangen und soll als Warnung an die XynaOrder angeh�ngt werden (TODO)
  ...
  public void run() {
    try {
      if( so != null ) { //h�ufigste Ausf�hrung hat SchedulingOrder gesetzt
        extractXynaOrder();
      }
      if( xo == null ) {
        if( so != null ) {
          ...
          //keine M�glichkeit, Auftrag abbzubrechen
        }
        return;
      }
  ...
  private void runInternallyWithXynaOrder() {
    ...
    try {
      if (xo.mustDeploymentCounterBeCountDown()) {
        //xynaprocess.cleanupAbortFailedThreads z�hlt hoch
      ...
      //ausgef�hrte XynaOrder dem SuspendResumeManagement bekanntmachen, damit dort notwendige Daten zur 
    ...
    try {
      if (xo.getExecutionProcessInstance() != null) {
        if (xo.getExecutionProcessInstance().getState() == XynaProcessState.SUSPENDED
          ...
          //compensate
          xo.setAbortionException(new ProcessAbortedException(AbortionCause.UNKNOWN)); //FIXME abortionCause und orginOrderId von urspr�nglichem abort erben
  ...
  private void compensateMasterWorkflow() {
    ...
    //exceptions die an der xyna order h�ngen, sind also eindeutig der compensation zuordbar.
    final String ndc = xo.getLoggingDiagnosisContext( XynaProperty.XYNA_CREATE_LOG4J_DIAG_CONTEXT.get() );
    final boolean createLoggingContextForThis = ndc != null;
    if( createLoggingContextForThis ) {
      NDC.push(ndc);
  ...
  private boolean handleSuspension(ProcessSuspendedException processSuspendedException) {
    try {
      ...
      return true;
      //TODO sollte man hier im fehlerfall die kindauftr�ge die schon suspendiert sind killern?
      // Hier gibt es unterschiedliche Fehlerf�lle:
      // 1) DB komplett nicht mehr erreichbar. In diesem Fall wirft die Methode eine OrderDeathException. Es ist nichts
      //    weiter zu tun, weil die Factory in diesem Fall sowieso herunterf�hrt.
      // 2) Eine sonstige PersistenceLayerException: Au�er bei der internen Entwicklung sollte das nur passieren, wenn
      ...
      //    Fall Cleanup f�r die Subauftr�ge nachholen und 
    } catch (OrderDeathException ode) {
      logger.warn("Connection to database appears to be broken, stopping execution", ode);
      handleOrderDeath(this, ode);
      return true;
