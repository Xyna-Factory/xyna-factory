...
public class ConstantRateEventCreationAlgorithm extends FrequenceControlledTaskEventAlgorithm {
  ...
  //ACHTUNG: rundungsfehler in den n�chsten beiden methoden k�nnen dazu f�hren, dass
  //die sleepduration zu kurz berechnet wird, und dann gibt es hohe cpu-last durch mehrfaches durchlaufen
  //der while-schleife, ohne dass geschlafen wird und ohne dass neue auftr�ge gestartet werden k�nnen.
  ...
  private long calculateSleepDurationForRate() {
    //berechne zeit, wann n�chster event gestartet werden sein muss
  ...
  public void run() {
    try {
      // Es wird ein Thread verwendet der nach Ablauf einer ausgerechneten Wartezeit mehrere Events ausl�st. Zielrate legt dabei Wartezeit & Anzahl fest
  ...
  private void sleep(long sleepDuration) {
    //schlafen bis das n�chste task gestartet werden kann. schlafen ist bei hohen raten u.u. problematisch. deshalb kann
    //man das verhalten konfigurieren.
    //TODO f�r noch h�here genauigkeit k�nnte man die folgende fallunterscheidung einmalig im konstruktor vornehmen und hier nur
    //eine methode eines interfaces aufrufen, welches je nach fallunterscheidung anders gesetzt ist (�hnlich wie im scheduler).
    try {
      if (sleepDuration <= threadSleepMinValue) {
        //=> spezialbehandlung f�r sehr kurze sleeps!
        ...
        if (highRateThreadSleepType == HighRateThreadSleepType.NORMALSLEEP) {
           ...
           * 3. bugs die dazu f�hren, dass die systemzeit ungenau wird
           */
          int m = 10;
        ...
        // => normale sleepl�nge ben�tigt keine sonderbehandlung
        Thread.sleep(sleepDuration);
      }
