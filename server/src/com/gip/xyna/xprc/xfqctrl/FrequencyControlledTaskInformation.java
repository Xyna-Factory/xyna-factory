...
public class FrequencyControlledTaskInformation extends Storable<FrequencyControlledTaskInformation> {
  ...
  FrequencyControlledTaskInformation(FrequencyControlledTask task) {
    ...
    //RuntimeContext setzen
    if (task instanceof FrequencyControlledOrderCreationTask) {
      List<XynaOrderCreationParameter> orderCreationParameters = ((FrequencyControlledOrderCreationTask)task).getOrderCreationParameters();
      if (orderCreationParameters.size() > 0) {
        RuntimeContext rc = orderCreationParameters.get(0).getDestinationKey().getRuntimeContext();
  ...
  private static void fillByResultSet(FrequencyControlledTaskInformation info, ResultSet rs) throws SQLException {
    ...
    info.applicationname = rs.getString(COL_APPLICATIONNAME);
    info.versionname = rs.getString(COL_VERSIONNAME);
    info.workspacename = rs.getString(COL_WORKSPACENAME);
  ...
  public static class DynamicFrequencyControlledTaskInfoReader
    ...
    public FrequencyControlledTaskInformation read(ResultSet rs) throws SQLException {
      ...
      if (selectedCols.contains(FrequencyControlledTaskInfoColumn.APPLICATIONNAME)) {
        mie.setApplicationname(rs.getString(FrequencyControlledTaskInfoColumn.APPLICATIONNAME.getColumnName()));
      }
      if (selectedCols.contains(FrequencyControlledTaskInfoColumn.VERSIONNAME)) {
        mie.setVersionname(rs.getString(FrequencyControlledTaskInfoColumn.VERSIONNAME.getColumnName()));
      }
      if (selectedCols.contains(FrequencyControlledTaskInfoColumn.WORKSPACENAME)) {
        mie.setWorkspacename(rs.getString(FrequencyControlledTaskInfoColumn.WORKSPACENAME.getColumnName()));
      }
      return mie;
    }
  }
  ...
  public FrequencyControlledTaskInformation createCopyWithReducedStatisticsInformation(String[] selectedStatistics) {
    ...
    result.versionname = this.versionname;
    result.workspacename = this.workspacename;
    
    result.statistics = new HashMap<String, FrequencyControlledTaskInformation.StatisticsInformation>();
