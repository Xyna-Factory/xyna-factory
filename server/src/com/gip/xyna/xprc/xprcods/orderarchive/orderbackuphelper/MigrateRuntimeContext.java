...
public class MigrateRuntimeContext {
   ...
   * !force = abbrechen, falls laufende objekte gefunden werden die voraussichtlich nicht migriert werden k�nnen 
   *          (TODO genauere analyse durchf�hren - es wird zu h�ufig abgebrochen)
   */
  public static MigrationContext migrateRuntimeContext(RuntimeDependencyContext from, RuntimeDependencyContext to, Collection<MigrationTargets> targets, boolean force, MigrateRuntimeContextAccessContext accessCtx)
    ...
    try {
      ...
      try {
        ...
        // Laufende Auftr�ge (Suspension & heldAtCheckpoints)
 ...
 /*   XynaFactory.getInstance().getFactoryManagement().getXynaFactoryControl().getRevisionManagement().getRevision(runtimeContext)
    ...
    try {
      ...
      if (affectedCrons.size() > 0) {
        for (CronLikeOrder clo : affectedCrons) {
           ...
           *     Root-Types, weil Membervariablen abgeleitete Typen haben k�nnten.
  ...
  private static class ObjectCompatibility {
    ...
    public Set<Long> getRevisionsBetween(Long sourceRev, Long targetRev) {
      ...
      Set<Long> check = new HashSet<>(); //enth�lt nur revs, die noch nicht in result sind, und die != targetrev sind
      check.add(sourceRev);
      while (check.size() > 0) {
    ...
    private boolean objectChangesAfterRevisionChange(GeneralXynaObject gxo, Set<Long> reachableRevisionsUntilChangedRevision) {
      /*
       * rekursion pro GXO: falls typ in quell und ziel-RTC-hierarchie auf den gleichen typ aufl�st, muss nichts ber�cksichtigt werden.
       * - rekursion nur auf non-null-members
       * - f�r jede membervariable checken, dass sie noch vom gleichen typ ist
       * 
       * achtung, wenn man das wiederverwenden m�chte f�r xynaobjekte, die in workflows/audits referenziert sind, muss man die oldVersions von
      ...
      if (revisionsReachableFromToRevision.contains(revisionLoadingObj) || reachableRevisionsUntilChangedRevision.contains(revisionLoadingObj)) {
        //objektdefinition �ndert sich durch migration nicht. komplexwertige membervariablen trotzdem rekursiv checken!
      } else {
        ...
        if (compatible != null) {
          ...
          try {
            ...
            //objekt nicht aufl�sbar oder sowas.
            if (logger.isDebugEnabled()) {
      ...
      try {
        ...
        while (batch != null && batch.size() > 0) {
          for (CronLikeOrder clo : batch) {
            //inputparameter k�nnte abgeleitete typen enthalten, die aus parent-revision stammen? => nein, die m�ssen von der revision erreichbar sein, die der cron hat.
  ...
  private static void migrateOrderBackup(MigrationContext context) throws PersistenceLayerException {
    ...
    for (int i = 0; i < orderBackupIds.size(); ++i) {
      ...
      if (isRevisionReachable(context.getToRevision(), p.getSecond())) { 
        // TODO man erwischt damit leider auch auftr�ge, die vor der migration bereits die torevision verwendet hatten und deshalb nicht migriert werden m�ssten...
  ...
  private static OrderInstanceBackup loadBackupSavely(ODSConnection con, final Long orderId, Long fromRevision,
     ...
     * 
     * folgende m�glichkeiten, wo sich auftr�ge im orderbackup einordnen:
     * 
     ...
     * 7) root-auftrag in parentrevisions, current orderbackup in usedrevisions (=> ordertype muss nicht in der gleichen revision von torevision aus aufl�sbar sein)
     * 
     ...
     *  => orderbackup.revision: fromrevision in torevision um�ndern sollte immer ok sein. - ggfs auch ordertype aufl�sen
     * 7) => da die ganze auftragshierarchie migriert wird, muss auch dieses orderbackup migriert werden
     *  => orderbackup.revision: neue revision m�sste eigtl durch neues aufl�sen des ordertypes bestimmt werden. 
     *
     */
  ...
  private static void migrateDestinationKey(XynaOrderServerExtension xo, Long fromRevision, Long toRevision)
    ...
    if (xo.getDestinationKey().getRuntimeContext().equals(fromRc)) { //FIXME eigtl m�sste man hier den ordertype erneut nachschlagen. der muss ja nicht notwendigerweise in der torevision leben
