...
public class OrderbackupWithoutParentAndWithoutXynaOrder extends AbandonedOrderDetectionRule<AbandonedOrderDetails> {
  ...
  // FIXME: ... where rootorderid = id ... k�nnen dies auch nicht DB-PLs verstehen?
  // BG: Nope, java.lang.NumberFormatException: For input string: "id" f�r Mem-PL
      ...
      if (results.size() > 0) {
        for (OrderInstanceBackup oib : results) {
          ...
          if (oib.getDetails() == null) {
            ...
            // order is part of a series and thus not broken FIXME nur f�r alte Huckepack-Auftr�ge 
            broken = false;
          } else {
