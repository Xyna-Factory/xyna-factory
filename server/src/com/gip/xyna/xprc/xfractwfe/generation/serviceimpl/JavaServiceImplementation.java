...
/**
 * wie sieht der generierte code aus, der f�r die eclipse projekte generiert wird
 */
public class JavaServiceImplementation {
  ...
  public String createDelegationImplCode() {
    ...
    if (getOperations(false).length > 0 && dom.hasJavaImpl(true, null)) {
      ...
      for (InterfaceVersion version : dom.getVersionsOfOperations(true)) {
        if (!dom.hasSuperTypeWithJavaImpl(true, version)) {
          //impl ist hier definiert, wurde aber evtl in einem sub-typ instanziiert und �ber die map hierher delegiert
    ...
    if (hasStaticOperationImpl) {
      //FIXME falls abstrakt, werden die nie aufgerufen. das ist f�r den eclipse user verwirrend
    ...
    if (hasImplInstanceVar && !dom.isAbstract()) {
      ...
      cb.addLine("return new ", getDelegationImplSimpleClassName(), "(implclone)"); //TODO das ist aber unsch�n, diesen konstruktor aufzurufen, der steht oben als "nur aus abw�rtskompatibilit�t" markiert.
  ...
  private void appendOperationsForProjectImpl(CodeBuffer cb, boolean staticOps, Set<String> importedClassesFullyQualified) {
    for (OperationInformation operationInformation : getOperations(staticOps)) {
      ...
      cb.add(" {").addLB();
      //methode kann in den f�llen, wo sie als workflow oder codesnippet implementiert ist, nicht weggelassen oder abstrakt gemacht werden
  ...
  /**
   * gibt zur�ck, ob die beiden versionen gleich behandelt werden sollen. typischerfall der nicht-identit�t ist, dass
  ...
  public String createProjectNonStaticImplCode() {
    //auch erzeugen, wenn es keine operations gibt! z.b. leitet sshnetconfconnection von sshconnection nur deshalb ab, um nicht modellierte operations zu �berschreiben
  ...
  public String createSuperProxyCode() {
    ...
    if (dom.hasSuperTypeWithInstanceMethods(version)) {
      //TODO variable nicht generieren, wenn sie nicht ben�tigt werden (also es gibt nur final und abstract methoden im supertype)
      ...
      for (OperationInformation operationInformation : dom.getSuperClassGenerationObject().collectOperationsOfDOMHierarchy(false)) {
        ...
        if (operationInformation.isAbstract()) {
          ...
          if (operationInformation.isNotAbstractInJavaServiceImpl()) {
            //super delegation. nur, wenn es auch eine javaimplementierung obendr�ber gibt
      ...
      //clone methode �berschreiben, falls abstrakt in supertype
    ...
    if (dom.hasSuperTypeWithInstanceMethods(version)) {
      for (OperationInformation operationInformation : dom.getSuperClassGenerationObject().collectOperationsOfDOMHierarchy(false)) {
        ...
        //delegation f�r finale methoden einfach an die instanz-Variable, da muss man nichts beachten, weil sie lokal nicht �berschrieben sein k�nnen
        if (operationInformation.isFinal()) {
          cb.add("public final ");
          operationInformation.getOperation().createMethodSignature(cb, false, importedClassesFullyQualified,
                                                                    operationInformation.getOperation().getNameWithoutVersion());
          ...
          //-> methode muss �ber spezielle delegation den super-aufruf machen, damit evtl eine workflow implementierung aufgerufen werden kann
          ...
          operationInformation.getOperation().createMethodSignature(cb, false, importedClassesFullyQualified,
                                                                    operationInformation.getOperation().getNameWithoutVersion());
          cb.add(" {").addLB();
          //reflection, weil methode im datentyp private ist (sichtbar w�re bl�d, weil verwirrend f�r den benutzer). 
          //Au�erdem ist eine h�here Sichtbarkeit schlecht, weil die Methoden dann nicht gezielt aufgerufen werden k�nnen (wegen dynamic dispatch)
          ...
          //TODO mehr unterst�tzung von special purpose inputvars, falls man die mit dieser technologie erweitern m�chte
          //     vgl auch unten die parameter �bergabe ans invoke
          //spezial parameter:
          ...
          cb.add(")").addLB();
          //TODO mehr informationen bei den runtimeexceptions �bergeben
  ...
  private void returnNothing(CodeBuffer cb, List<AVariable> outputVars) {
    ...
    if( out.isJavaBaseType() && ! out.getJavaTypeEnum().isObject() ) {
      //kein Object, daher kein null m�glich
  ...
  private String createInterfaceCodeInternally(boolean staticOps) {
    ...
    cb.addLB(2);
    cb.addLine("/**");
    //FIXME kommentare m�ssen als ein ganzer string zum codebuffer geaddet werden, damit keine semikolons generiert werden
    ...
    for (OperationInformation operationInformation : getOperations(true)) {
      ...
      //TODO javadoc f�r die variablen
      cb.addLine(" */");
      cb.add("public ");
      operation.createMethodSignature(cb, false, importedClassesFullyQualified, operation.getNameWithoutVersion());
      cb.addLB(2);
  ...
  /**
   * falls lokal keine dokumentation definiert ist, die aus oberklasse �bernehmen
   */
  ...
  /**
   * erstellt tempor�ren superklassen code falls vorhanden, um nicht abh�ngig von kram im deployten superklassen code zu sein
   * (z.b. abstract methoden)
   * 
   * code ist anders aufgebaut, als der echte superklassen code, er enth�lt z.b. keine informationen �ber weitere superklassen.
   * ben�tigt werden nur konstruktor und korrekte methodensignaturen
   */
   ...
   * 
   * code ist anders aufgebaut, als der echte superklassen code, er enth�lt z.b. keine informationen �ber weitere superklassen.
   * ben�tigt werden nur konstruktoren
   */
  public String createSuperDelegationImplCode() {
    ...
    cb.addLine("}").addLB();
    //methoden werden nicht ben�tigt, weil es keine super-aufrufe in der delegationimpl klasse gibt
    cb.addLine("}");
