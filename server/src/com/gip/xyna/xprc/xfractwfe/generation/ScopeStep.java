...
public class ScopeStep extends Step {
  ...
  protected long calculateSerialVersionUID() {
    ...
    for (Step s : allLocalSteps) {
      ...
      if (!(s instanceof ScopeStep)) {
        //typ des steps mit beachten - weil ein ausgetauschter step den gleichen namen hat, aber f�r die serialisierung nicht mehr kompatibel sein wird
  ...
  /**
   * alle steps, die die scope als membervars enth�lt. also ohne this und ohne steps, die unterhalb von weiteren scopes
   * liegen.
   */
  ...
  protected void generateJavaInternally(CodeBuffer cb, HashSet<String> importedClassesFqStrings)
    ...
    for (Step s : allLocalSteps) {
      if (!(s instanceof ScopeStep)) {
        ...
        if (s instanceof StepChoice) {
          //F�r Alias-F�lle in Choices einen zus�tzlichen Step generieren. Diese sind vom Typ des referenzierten Steps.
          //Die Steps vom eigenen Typ werden bei Alias-F�llen nicht mehr verwendet, werden aber wegen
          //Abw�rtskompatibilit�t weiter generiert.
    ...
    // initialize vars f�r wiederverwendung durch workflowpools
    ...
    //TODO performance: falls liste f�r for-each konstant belegt ist, k�nnte man das auch hardgecodet erzeugen.
    if (containsForeachOrScopeSubStep()) {
      cb.addLine("public " + FractalProcessStep.class.getSimpleName() + "[] ", METHODNAME_GET_ALL_STEPS, "() {");
      cb.addLine("List<", FractalProcessStep.class.getSimpleName(), "> l = new ArrayList<", FractalProcessStep.class.getSimpleName(), ">()"); //TODO performance: liste mit korrekter l�nge initialisieren
  ...
  private void generateInitializeMemberVars(CodeBuffer cb, HashSet<String> importedClassesFqStrings, List<Step> allLocalSteps) {
    ...
    for (Step s : allLocalSteps) {
      if (!(s instanceof ScopeStep)) {
        ...
        if (s instanceof StepChoice) {
          //f�r Choices die Alias-Steps zu allSteps hinzuf�gen
          StepChoice choice = (StepChoice)s;
  ...
  //�berschrieben von wf, weil man dort die methoden nicht braucht
