...
public class StepFunction extends Step implements Catchable, HasDocumentation {
  ...
  public String getUniqueStepId() {
    ...
    //gibt keine foreach-indizes, und retry ist hier nicht n�tig
    return getIdx() + ".0";
  }
  ...
  private boolean compensationDelegatesToSubworkflow() {
    ...
    /*
     * manchmal k�nnte man hier auch false zur�ckgeben. n�mlich wenn man wei�, dass im subworkflow in den compensationschritten (rekursiv) nichts passiert.
     ...
     *  das nicht unbedingt aktuell gehalten wird, falls sich der subworkflow (oder rekursiv sub-sub-sub-workflow) �ndert
     *  
     * achtung: eine schlauere ermittlung f�hrt zu h�ufigeren archivierungsvorg�ngen, und damit mit auditdaten zu h�ufigeren nicht-gebatchten
     * orderarchive-commits. das kann in der summe zu schlechter performance f�hren. leider ist ein batching der vorgezogenen commits nicht so einfach
  ...
  private void createCodeForLazyCreateSubWf(CodeBuffer cb, Service s, List<VariableIdentification> inputVarsForWFCall,
    ...
    if (!s.isDOMRef()) {
      ...
      // set possible mi responses: nur die hier definierten sind f�r den MI bearbeiter erlaubt (retry geht nur, wenn man sich in einem catchblock mit retry-feature befindet)
      ...
      //subauftrag-revision bestimmen anhand von aktueller revision und destinationkey. destinationkey hat in einigen f�llen aber noch keinen runtimecontext zugewiesen.
      ...
      cb.addLine("if (!compensationRecursive()) {"); //wenn man hier bereits wei�, dass compensation eh rekursiv ausgef�hrt wird, stepcoords nicht setzen. dann wird beim archivieren schnell erkannt, dass man nichts aufr�umen kann.
      ...
      cb.addLine("subworkflow.setNewOrderContext()");
      /*
       TODO Soll ein detached gestarteter Workflow den LoggingDiagnosisContext vom startenden Workflow (entspricht Parent) �bernehmen?
      if (isExecutionDetached()) {
  ...
  private void generateJavaForDOMRef(ServiceIdentification service, CodeBuffer cb,
    ...
    if (isExecutionDetached()) {
      ...
      if (compensateStep != null) {
        ...
        //ntbd: remote compensate wird derzeit nicht unterst�tzt
      } else {
        if (!ops.isStatic()) {
          /*
           * kann in abgeleitetem objekt �berschrieben sein. also zur laufzeit checken, was f�r ein typ die operation hat!
           * wf soll nicht neu generiert werden m�ssen, wenn neue subklassen erzeugt werden
           * => man muss zur laufzeit herausfinden k�nnen, wie methode x in klasse y implementiert ist.
           * => das findet man heraus, indem man im datentyp nachschaut. xml ist ineffizient, deshalb 
           *    entweder beim ondeployment direkt merken, oder sp�ter lazy nachladen
  ...
  protected void appendExecuteInternallyForDOMRef(ServiceIdentification service, CodeBuffer cb,
    ...
    if (isExecutionDetached()) {
      ...
      if (isRemoteCall()) {
        ...
        //kein remoteCall, alle Parameter normal �bergeben
        boolean skipFirstVar = !ops.isStatic() && service.service.getVariable() == null;
      ...
      if (isExecutionDetached()) {
        ...
        // ggfs container auseinanderklam�sern
        if (ops instanceof WorkflowCallServiceReference) {
          if (compensateStep == null) {
  ...
  private void appendResultSetter(CodeBuffer cb, HashSet<String> importedClassesFqStrings, boolean instanceOfWorkflowCallServiceReference)
    ...
    if (receiveVarIds.length > 0) {
      ...
      for (int i = 0; i < receiveVarIds.length; i++) {
        cb.add("case " + i, " : "); //TODO unsch�ner code, um zu verhindern, dass da ein semikolon zuviel ist
  ...
  protected List<GenerationBase> getDependencies() {
    //normalerweise muss man hier nicht den aufgerufenen workflow zur�ckgeben, weil er in einer servicereference auf der n�chsth�heren stepserial-ebene referenziert wird
    //es gab aber offenbar mal einen bug, wo die servicereference-objekte im xml nur innerhalb des function-objekts lagen. 
    //es schadet auch nichts, die workflows hier auch zur�ckzugeben
