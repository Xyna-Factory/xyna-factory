...
/**
 * Klasse f�r Parsing, Typisierung und Codegenerierung von Formel-Ausdr�cken, wie sie in 
 * Conditional Choices und Mappings modelliert und verwendet werden k�nnen.
 * 
 * Das Parsing geschieht im Parser, die Typisierung und Codegenerierung passiert haupts�chlich �ber
 * ein Visitor-Pattern.
 * 
 * Formel-Ausdr�cke haben folgenden Syntax
 * <pre>
 * Expr := ValueExpr | ValueExpr mit AND/OR und klammerungen verkn�pft
 ...
 *   step.getOutputIds werden in dieser Reihenfolge durchgez�hlt)
...
public class ModelledExpression {
  ...
  /**
   * visitor f�r die erzeugung des xfl strings aus den javaobjekten
   */
          ...
          if (nullComparison) {
            ...
            if (expression.getVar1().getOriginalType().isUnknown()) {
              if (expression.getVar2().getOriginalType().isUnknown()) {
                ...
                //unknown �bernimmt typ vom anderen
      ...
      try {
        ...
        if (!variable.getTargetType().isUnknown()) {
          if (!variable.getTypeOfExpression().equals(variable.getTargetType(), true)) {
            //TODO inkompatible typen erkennen und fehler werfen. passiert aber ansosnten sp�ter beim compile
          }
        } else {
  ...
  public class JavaCodeGeneratorVisitor extends ContextAwareVisitor {
     ...
     * grund f�r die teilung: dazwischen werden andere expressions vom visitor besucht. 
     */
    private String[] transformation(String value, TypeInfo fromType, TypeInfo toType) {
      if (fromType.equals(toType, true)) {
        ...
        //wenn beide listenwertig sind, wurde es obendr�ber abgefangen.
        ...
        //FIXME hier die information verarbeiten, welchen typ die listenelemente haben m�ssen.       
    ...
    public void variablePartStarts(VariableAccessPart part) {
      if (part.getIndexDef() != null && 
        ...
        if (ctx instanceof Variable) {
          ...
          //TODO offenbar sollten castexpression und variable ein gemeinsames interface implementieren, wenn die verwendung hier so �hnlich ist!
          ...
          //new k�nnte theoretisch auch indexdef haben, macht aber wenig sinn
  ...
  /**
   * bestimmt die typen aller teil-ausdr�cke und bef�llt den maptree mit informationen zu den variablen-zugriffen des
   * ziel ausdrucks.
   * damit variablen-teile nicht �berschrieben werden, muss f�r alle an einem mapping beteiligten einzelmappings 
   * die gleiche maptree an diese methode �bergeben werden.
   * 
   * erl�uterung: damit beim bef�llen des ziel types keine NPEs zur laufzeit passieren, wird der code generiert,
   ...
   * @param varPathsAlreadyCreated enth�lt informationen zu allen variablen-teilen des target-types, und ob diese
   *   bei der codegenerierung noch erstellt werden m�ssen, oder ob sie bereits vorhanden sind.
   */
  ...
  private void checkCreationOfVariable(MapTree varPathsAlreadyCreated, FollowableType sve) throws XPRC_ParsingModelledExpressionException {
    ...
    //um auf index-def von basisvariable zugreifen zu k�nnen, m�ssen cast-funktionen ausgepackt werden 
  ...
  public void writeAssignmentExpressionToCodeBuffer(CodeBuffer cb, Set<String> importedClassNames, long uniqueId,
    ...
    if (var != null && var.map.size() > 0) {
      ...
      //default konstruktor gen�gt
    }

    // for typeless mappings
    boolean needSetterInvocation = true;
    
    //tempor�re variable anlegen, die sp�ter �ber den setter dem target zugewiesen wird
    ...
    if (type.isList()) {
      ...
      if (parsedSourceExpression.getOriginalType().isNull()) {
        ...
        if (cloneObjects) {
          //eigtl m�sste man f�r o hier die typinformation der liste in code nehmen, da kommt man aber nicht gut dran. so funktioniert es auch...
      ...
      if (parsedSourceExpression == null) {
        ...
        VariableInfoPathMap vipm = (VariableInfoPathMap) sve.getFollowedVariable();
        cb.add(vipm.getJavaCodeForVariableAccess());
        //TODO mehr als eine value- variable unterst�tzen?
   ...
   * @param sve
   * @param pathDepth
   * @param varPathsForVariables kinder davon sind kinder von der zugeh�rigen variable (entsprechend dem path),
   * d.h. bei depth == -1, wird hier die teilmap der basisvariable �bergeben
   * bei depth == 0 wird hier die teilmap %0% bzw %0%[x] (falls listenwertig) �bergeben.
   */
  private void createEmptyChildObjects(CodeBuffer cb, FollowableType sve, int pathDepth,
    ...
    if (varInfo.getTypeInfo(false).isList()) {
      ...
      //per default ist liste leer -> mit nulls (oder richtigen objekten) bef�llen 
      int maxIdx = -1;
    ...
    if (!varPathsForVariables.generated) {
      //TODO falls dynamischer listenindex verwendet wird, muss hier darauf zugegriffen werden k�nnen. leider wird 
      //     hier vorausgesetzt, dass der name der gleiche ist, wie bei der sp�teren codeerzeugung f�r die eigentliche zuweisung
