...
/*
 * unterst�tzt dynamische output verzeichnisse f�r die classfiles, indem man im javasourcefromstring target das outputdir angibt.
...
class CompilationFileManager extends ForwardingJavaFileManager<JavaFileManager> {
  ...
  public JavaFileObject getJavaFileForInput(Location location, String className, Kind kind) throws IOException {
    //TODO m�sste man hier eigtl den source auch zur�ckgeben, wenn className eine innere klasse davon ist?! bei tests wurde dieser methode allerdings nie aufgerufen!
      ...
      if (jfo != null) {
        if (jfo instanceof JavaSourceFromString) {
          ...
          if (idx == -1) {
            ...
            //entferne classfiles von inneren klassen von fr�heren compiles             
            if (ret == null) {
  ...
  public Iterable<JavaFileObject> list(Location location, String packageName, Set<Kind> kinds, boolean recurse) throws IOException {
    Iterable<JavaFileObject> normal = super.list(location, packageName, kinds, recurse); //-> gibt liste von classfiles zur�ck - inkl innerer klassen
    normal = appendAdditionalFiles(normal, packageName, recurse);
    if (!kinds.contains(Kind.SOURCE)) {
      return normal;
    }

    //idee: falls objekt als class gefunden wird, trotzdem nur als source zur�ckgeben, wenn es ein compilationtarget ist. 
