...
public class DOM extends DomOrExceptionGenerationBase {
  ...
  //false = serialisierte objektinstanzen der alten version k�nnen von der neuen version problemlos deserialisiert werden und alle methoden, die es in der alten
  //        version gab, k�nnen immer noch aufgerufen werden.
  @Override
  public boolean compareImplementation(GenerationBase oldVersion) {
    ...
    //jede membervariable von old muss immer noch vorhanden sein, ansonsten true zur�ckgeben
  ...
  private void addDependenciesOfVariable(AVariable var, Set<GenerationBase> set) {
    ...
    if (var.isList()) {
      for (AVariable child : var.getChildren()) {
        //kinder k�nnten von abgeleitetem typ sein (z.b. konstante vorbelegung oder beim fromXML)
  ...
  /**
   * getter der form .getX().getY() f�r den relativen pfad von fullPath relativ zu basePath. 
   */
  ...
  protected void parseXmlInternally(Element rootElement) throws XPRC_InconsistentFileNameAndContentException,
    ...
    if (!isEmpty(rootElement.getAttribute(GenerationBase.ATT.BASETYPENAME))) {
      // FIXME hier m�sste man eigentlich das super class dom auch parsen, um sicherzugehen, dass das nicht ein Exceptiontyp
      //       ist, den man hier zur�ckerh�lt...
      superClassDom =
    ...
    //pktype ist ggf aus supertype und wird erst sp�ter bef�llt (validate)
  ...
  /**
   * f�gt ohne rekursion abh�ngige jars dazu
   */
    ...
    if (XynaFactory.isFactoryServer()) {
      ...
      if (withSharedLibs) {
        for (String s : sharedLibs) {
          // FIXME abh�ngigkeit von sharedlibclassloader entfernen.
  ...
  /**
   * rekursion �ber andere davon abh�ngige doms, nicht �ber WFs
   */
  ...
  protected void validateInternally() throws XPRC_InvalidVariableNameException, XPRC_DuplicateVariableNamesException,
    ...
    if (getSuperClassGenerationObject() != null) {
      for (Operation o : getOperations()) {
        ...
        if (!o.isStatic()) {
          //finde die superclass, die diese operation definiert. falls �berschreiben nicht ok -> fehler.
          ...
          while (nextSuperClassDOM != null) {
            ...
            if (found) {
              break; //n�chste operation checken
            }
    ...
    if (!isAbstract() && !foundAbstractMethod) {
      //nach abstrakten methoden in den oberklassen suchen, die hier nicht �berschrieben werden
    ...
    if (isInheritedFromStorable() && 
      ...
      if (xmomStorableFlatteningMode.get() != FlatteningMode.NONE && (mode == DeploymentMode.codeChanged || mode == DeploymentMode.codeNew)) {
        ...
        if (pathsToAutoFlat.size() > 0 ||
          ...
          //TODO achtung! bei xsd �nderungen sind die xml anpassungen evtl auch anzupassen
          try {
  ...
  /**
   * alle variablennamen m�ssen auch in lowercase eindeutig sein. auch f�r die verwendeten datentypen
   */
  ...
  public static class OperationInformation {
    ...
    /**
     * ~ muss man also �berschreiben, wenn man das objekt instanziieren will 
     */
    public boolean isAbstractInBaseType() {
  ...
  /**
   * sind lokal methoden definiert, die in einem subtyp �berschrieben werden k�nnen
  ...
  public boolean hasJavaImpl(boolean onlyCountInstanceMethods, InterfaceVersion versionFilter) {
    ...
    //bei den methoden zu checken ist nicht redundant (sp�ter wird auf das impl-jar gecheckt), weil die library unter umst�nden nicht da ist
    ...
    //unterst�tzung von javalib an einer instanz, die die lib nicht selbst verwendet (aber f�r subklassen zur verf�gung stellt)
    if (libraryExists()) {
  ...
  /**
   * f�r statische operations nur die des aktuellen types, nicht die der hierarchy!<br>
   * nach name sortiert
   */
  public OperationInformation[] collectOperationsOfDOMHierarchy(boolean includeStaticMethods) {
    ...
    for (DOM dom : getDOMHierarchy()) {
      for (Operation op : dom.getOperations()) {
        if (opMap.containsKey(op.getName())) { //�berschrieben
          continue;
        } else {
  ...
  public Set<String> getImports() {
    ...
    for (OperationInformation operation : collectOperationsOfDOMHierarchy(true)) {
      if (operation.getOperation().isStatic() && !operation.isImplementedHere()) {
        //ben�tigt keine imports lokal
        continue;
      }
      if (!operation.getOperation().isStatic() && operation.isAbstractInBaseType() && !operation.isImplementedHere()) {
        //imports von geerbten methoden sind notwendig f�r die super-aufruf-proxys
        continue;
      }
  ...
  //FIXME mehrere operations mit dem gleichen namen unterst�tzen
  ...
  public InterfaceVersion[] getVersionsOfOperations(boolean onlyOneCurrentVersion) {
    ...
    versions.add(InterfaceVersion.BASE); //base immer, damit es auch ohne modellierte methoden m�glich ist, code zu schreiben
  ...
  //FIXME soll das wirklich festgelegt werden, dass impl klassen so heissen m�ssen. abgesehen von der template-generierung ist das die einzige stelle wo man 
  // sich darauf verl�sst. dann kann man auch die code-snippets verstecken und sagen, dass man die eh immer generiert.
  ...
  protected void fillVarsInternally() throws XPRC_MEMBER_DATA_NOT_IDENTIFIED, XPRC_InvalidXMLMissingListValueException,
    ...
    //persistenceinformation fertig bef�llen falls n�tig. kann in parsexml nicht gemacht werden, weil noch das geparsten super-objekt fehlt.
  ...
  public static void addLibraryTagAndCodeSnippetInXML(Document doc, DOM dom, boolean libraryTagOnly, boolean adjustDomAccordingly)
    ...
    if (ss.size() == 0) {
      ...
      // kann aber passieren, wenn oberklasse methoden hat, und man im impl nur interne methoden �berschreiben m�chte
  ...
  public static class TemplateGenerationResult {
    ...
    /**
     * die files, die im eclipse projekt implementiert werden m�ssen 
     */
    ...
    /**
     * f�rs compile notwendige oberklasse-files, die man nicht aus den deploy-ten jars verwenden will.
   ...
   * diese m�ssen n�mlich auch neu deployed werden, wenn this sich �ndert (weil die storable-klassen sich �ndern m�ssen).
   */
  public Set<GenerationBase> getRootXMOMStorablesUsingThis(GenerationBaseCache parseAdditionalCache, Set<GenerationBase> visited) {
    ...
    //FIXME performance!!!! eigtl w�re es hier sch�n, wenn man die xmomdatabase verwenden k�nnte. man ben�tigt aber die informationen aus dem deployed ordner
   ...
   * oder transienten variablen), wird dort das �bergebene dom irgendwo verwendet??
   * ber�cksichtigt nicht, dass z.b. das dom in einer additionaldependency eines verwendeten datentyps steht oder
