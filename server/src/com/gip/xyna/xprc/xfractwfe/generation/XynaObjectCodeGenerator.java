...
public class XynaObjectCodeGenerator {
  ...
  private void generateJavaClassMembers(CodeBuffer cb) {
    ...
    for (AVariable v : memberVars) {
      ...
      if (v.isList()) {
        ...
        //dieses feature wird in projekten genutzt. bei �nderungen also bitte aufpassen
  ...
  public static void appendGetter(CodeBuffer cb, AVariable v, Set<String> importedClassNames, boolean useListExtendedGenerics) {
    ...
    if (v.isList()) {
      //wenn listen herausgegeben werden, k�nnen diese ge�ndert werden und m�ssen dann von der versionierung beachtet werden -> also wrappen
      cb.addLine("if (supportsObjectVersioning()) {");
  ...
  public static void appendUseVersioningMethod(CodeBuffer cb) {
    ...
    switch (XynaProperty.useVersioningConfig.get()) {
      ...
      case 4 :
      case 3 :
        //ausserhalb der factory kein versioning verwenden, weil keine versionsnummern �ber idgenerator gefunden werden k�nnen.
  ...
  private boolean generateJavaPathMapMembersMap(CodeBuffer cb) {
    ...
    //TODO hier k�nnte man auch sch�ner das objekt nicht hinzuf�gen, wenn es bereits in der map enthalten ist, und dann nur den value umsetzen
  ...
  //usecase: das datenmodell kennt metainformationen, die im pathmap objekt automatisch eingetragen werden sollen, wenn der pfad entsprechend gew�hlt ist
      ...
      if (dom.getPathMapInformation() != null) {
        ...
        if (dom.getPathMapInformation().getInheritFromDataModel(childPath) != null) {
          if (!vv.isJavaBaseType() && !vv.isList()) {
            //TODO mehr als eine automatisch erstellte membervariable unterst�tzen
  ...
  private void generateJavaConstructors(CodeBuffer cb) {
    ...
    if (memberVars.size() > 0 && calcLengthOfParameters(memberVars) < 255) {
      ...
      //TODO javadoc f�r alle membervars mit doku aus xml?
      cb.addLine("*/");
  ...
  public static void appendObjectVersionClassGeneration(CodeBuffer cb, List<AVariable> memberVars, Set<String> imports,
    ...
    if (hasSuperObjectVersionClass) {
      superClassName = fqSuperClassName + ".ObjectVersion"; //TODO klassenname k�nnte bei nicht-generiertem code anders sein?!
    } else {
    ...
    if (memberVars.size() > 0) {
      ...
      for (AVariable v : memberVars) {
        ...
        if (v.isJavaBaseType()) {
          ...
          //FIXME zyklencheck: �hnlich wie bei hashcode berechnung stack von einem der objekte durchreichen
    ...
    if (memberVars.size() > 0) {
      ...
      for (AVariable v : memberVars) {
        ...
        if (v.isJavaBaseType()) {
          ...
          //TODO zyklen check: stack beachten. falls stack objekt enth�lt, dann den abstand im stack zu dem eigenen objekt als f�r den hash relevante zahl verwenden.
  ...
  private void generateJavaMethods(CodeBuffer cb) {
    ...
    if (instanceOperations.length > 0) {
      ...
      if (hasImplInstanceVar) {
        ...
        //volatile f�r die lazy initialisierung
      ...
      if (hasJavaImpl && isAbstract) {
        if (dom.isFirstTypeOfHierarchyWithJavaImpl(true, null)) {
           ...
           * A hat methode
           * B ist abstrakt
           * C wird instanziiert und ruft methode auf. dann darf in B und C init() nicht �berschrieben sein
           */
      ...
      } else if (hasJavaImpl //�quivalent zu: (hasImpl && !libraryExists()) || (!hasImpl && !abstract && hasOnlyAbstractBla())
          || (!isAbstract && hasOnlyAbstractSuperTypesUntilFirstTypeWithJavaImpl())) {
        //init wird lokal nicht ben�tigt, aber weil die methode abstrakt ist, muss sie definiert sein

        //kann zustand beim ersten speichern von der gui aus sein -> soll kompilieren. TODO �ber irgendein flag steuern, ob das
        ...
        //keine impl -> kein getter ben�tigt
      }
  ...
  private void generateJavaReadWriteObject(CodeBuffer cb, Set<String> currentImportsFqClasses,
    ...
    //f�r die instanzen der impls muss man auch serializableclassloaded object verwenden, weil die impl-klassen dem classloader, der die serialisierung
    //gestartet hat (vgl. ContainerClass), nicht bekannt sein m�ssen.

    //TODO sortierung derart, dass eine umbenennung einer variablen nicht zur inkompatibilit�t mit dem alten stand f�hrt.
