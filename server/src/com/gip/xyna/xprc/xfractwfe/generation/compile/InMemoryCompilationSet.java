...
public class InMemoryCompilationSet implements CompilationSet {
   ...
   * wirkt sich nur auf sources aus, die nach dem setzen des classdirs zum compilationset hinzugef�gt werden.
   * 
   * alternativ kann f�r jede javasource ein eigenes classdir angegeben werden @link{JavaSourceFromString#setClassOutputLocation(String)}
   * @param classDir
   */
  ...
  public CompilationResult compile() throws XPRC_CompileError {
   ...
   // options.add("-verbose");
    ...
    try {
      ...
      synchronized (GenerationBase.class) { //keine compiles gleichzeitig ausf�hren (synchronisiert mit Main.compile())
        if (!task.call()) {
  ...
  /**
   * reihenfolge in der files hier hinzugef�gt werden, ist f�rs kompilieren relevant
   */
