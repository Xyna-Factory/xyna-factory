...
/**
 * Thread-Beschr�nkung mit blockierten Tasks: Solange der Task suspendiert ist, steht dem ParallelExecutor ein 
 * Thread weniger zu Verf�gung.
 * 
 * Dies ist beispielsweise sinnvoll, wenn zuwenig Capacities zu Verf�gung stehen, so dass weitere Threads 
 * mit ihrem synchronen Subworkflow-Aufruf durch das Warten auf die Capacity blockiert w�rden. Dann sollen diese 
 * Threads besser erst gar nicht nicht gestartet werden.
 * 
 * �ber {@link #awaitLimitationNotZero()} wartet der FractalWorkflowParallelExecutor blockierend solange, bis 
 * ein Thread ausgef�hrt werden darf.
 * 
 */
public abstract class AbstractBlockingParallelismLimitation implements ParallelismLimitation<FractalProcessStep<?>> {
  ...
  protected AtomicInteger threadLimit = new AtomicInteger(); //tats�chliches ThreadLimit unter Ber�cksichtigung der blockierten Lanes
  ...
  public void addTaskToParallelExecutor(SuspendableParallelTask<FractalProcessStep<?>> taskToResume) {
    ...
    if (isBlocking) {
      ...
      changeThreadLimit(1); //ThreadLimit wieder erh�hen
      if (logger.isDebugEnabled()) {
  ...
  private void changeThreadLimit(int delta) {
    ...
    synchronized (this) { 
      //synchronized, damit zwischen dem lesen und schreiben nichts passieren kann. ansonstne k�nnte die reihenfolge sein
      //thread1 liest threadlimit
      //thread2 �ndert threadlimit
  ...
  public boolean awaitLimitationNotZero() {
    if( threadLimit.get() > 0 ) {
      ...
      //TODO sp�ter: wenn z.b. die kardinalit�t der kapazit�t vergr��ert wird = maxthreadlimit vergr��ert => suspendierten auftrag aufwecken!
