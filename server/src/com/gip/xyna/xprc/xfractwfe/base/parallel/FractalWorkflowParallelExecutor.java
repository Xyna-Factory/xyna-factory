...
public class FractalWorkflowParallelExecutor<S extends Step> implements ResumableParallelExecutor, Serializable {
  ...
  public interface ParallelismLimitation<S extends Step> {
    ...
    /**
     * �bergabe des ParallelExecutor zur Verwendung im ParallelismLimitationAlgorithm
    ...
    /**
     * Wartet, bis Limitation != 0 ist (es d�rfen wieder Tasks ausgef�hrt werden)
  ...
  public void execute() throws XynaException {
    ...
    //Starten des ParallelExecutor und auf vollst�ndige Bearbeitung warten
    do {
    ...
    //1) //Evtl. konnten normale Auftr�ge nicht bearbeitet werden, weil keine Threads mehr laufen durften. 
    //Die zugeh�rigen LaneIds werden nun f�r den n�chsten Start des ParallelExecutors gespeichert 
    ...
    do {
      ...
      if( parallelExecutor.isExecuting() ) {
        ...
        if( parallelExecutor.size() != 0 && parallelExecutor.hasExecutableTasks() ) {
          //es gibt noch Tasks, die ausgef�hrt werden m�ssen
          if( logger.isDebugEnabled() ) {
  ...
  private void handleSuspensions() {
    ...
    //Zuerst m�ssen Suspendierungen behandelt werden: Falls Suspendierungen vorliegen, wird eine SuspendedException
    //geworfen. Die gefangenen Exception bleiben gespeichert, bis alle Lanes komplett ausgef�hrt worden sind
    if( data.hasSuspendedTasks() ) {
  ...
  private class ResumeHandler {
    ...
    private ResumeState resumeTaskInternally(String laneId) {
      ...
      try {
        ...
        if( parallelExecutor.isExecuting() ) {
          //ignorieren, der Task wird noch ausgef�hrt werden
        } else {
  ...
  /**
   * Abbrechen der weiteren Task-ausf�hrung, nur Resumes sind noch zul�ssig
   */
  public void cancel() {
  ...
  /**
   * Mitteilung, welche threads h�ngen.
   * @param hangingThreads
   * @return mainthread, falls mainthread h�ngt
   */
