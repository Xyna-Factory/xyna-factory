...
public class Parser {
   ...
   *        falls nicht, kann nun alles m�gliche kommen:
   *        - assignment
        ...
        switch (c) {
          case ',' :
            if (state == FunctionParsingState.OPEN_BRACE || state == FunctionParsingState.POSSIBLY_PARAMETER_END) {
              ...
              //klammern sind optional. das komma geh�rt zu einer �bergeordneten funktion oder sowas
              return new FunctionExpression(startIdx, idx - 1, f, subExpressions, null);
            } else {
              throw new XPRC_ParsingModelledExpressionException(expression, idx + 1,
          ...
          case ')' :
            if (state == FunctionParsingState.OPEN_BRACE || state == FunctionParsingState.POSSIBLY_PARAMETER_END) {
              ...
              //klammern sind optional. die klammer geh�rt zu einer �bergeordneten funktion oder sowas
              return new FunctionExpression(startIdx, idx - 1, f, subExpressions, null);
            } else {
              throw new XPRC_ParsingModelledExpressionException(expression, idx + 1);
            }
      ...
      if (c == '\\') { // ein backslash gefunden
        ...
        switch (c) {
          ...
          case '\\' :
            ...
            break;
          case 'u' :
            // "\ u <zahl>" erstmal nicht korrekt unterst�tzt
            // fall through
          default :
            ...
            d = c;
            forJava = String.valueOf(c); // escape von anderen zeichen enth�lt im value kein backslash, sondern nur c. -> in string: "asd'c'asd" -> value <asd'c'asd>
        }
        value.append(d);
  ...
  public Expression parseSecondPartOfExpressionIfExisting(final int startIdx, Expression firstPart)
      throws XPRC_ParsingModelledExpressionException {
    boolean firstPartPlusSecondPartMustBeBoolean = false; //TODO das k�nnte man wahrscheinlich immer ganz gut checken - z.b. wissen funktionen, ob sie vom typ boolean sind
    int idx = startIdx;
    /*
     * entweder 
