...
public class XynaFractalWorkflowEngine extends Section implements WorkflowEngine, IPropertyChangeListener {
  ...
  public void init() throws XynaException {
    ...
    XynaProcess.addGlobalPreCompensationHandler(new LogHandler( "Starting compensate", true, false,
                                                                new StartCompensationHandler() ) ); //FIXME ist das hier n�tig? besser MasterWorkflowPostScheduler?
    ...
    if (jars != null && jars.size() > 0 && revisionManagement.isWorkspaceRevision(revision)) {
      ...
      for (Entry<String, InputStream> entry : jars.entrySet()) {
        ...
        if (entry.getKey().endsWith("zip")) {
          ...
          //ans target dir nur den file-namen (ohne pfad) anh�ngen
  ...
  /**
   * Gibt aus der �bergebenen Liste alle WORKFLOWs, EXCEPTIONs und DATATYPEs zur�ck, die bereits in der Revision vorliegen
   * @param xmomObjects
   * @param revision
   * @return
   */
  ...
  /**
   * Kopiert aus der �bergebenen Liste alle WORKFLOWs, EXCEPTIONs und DATATYPEs aus einer Revision in die andere und deploy anschlie�end
   * @param xmomObjects
   * @param fromRevision
   * @param toRevision
  ...
  private class XynaDispatcherDeploymentHandler implements DeploymentHandler {
        ...
        for (Step s : detachedSteps) {
          if (s instanceof StepFunction) {
            ...
            try {
              ...
              //wurde bereits beim parsen �berpr�ft.
              throw new RuntimeException(e);
            }
            if (service.isDOMRef()) {
    ...
    private void registerServiceDestination(String orderType, Long revisionOfDestination, String serviceFqXmlName, String serviceName,
      ...
      try {
        ...
        throw new RuntimeException(e); //dann h�tte es vorher schon probleme beim parsen geben m�ssen
      }
        ...
        for (Step s : detachedSteps) {
          if (s instanceof StepFunction) {
            ...
            if (service.isDOMRef()) {
              ...
              //nicht deregistrieren, weil es noch andere detached remotecalls geben k�nnte
