...
public class TimeConstraintManagement extends FunctionGroup implements TimeConstraintManagementInterface, Clustered {
  ...
  public static enum TimeConstraintProblemReaction {
    Wait,         //Auftrag wartet im Scheduler, bis TimeWindow-Problem gel�st ist
    ...
    public static String documentation(DocumentationLanguage lang) {
      switch( lang ) {
        case DE:
          return "'Wait': Auftrag wartet im Scheduler, bis TimeWindow-Problem gel�st ist; "
  ...
  private void initClusterStatusKnown() {
    ...
    //K�nnen Zeitfenster komplett initialisiert werden?
    ...
    if( ! rmiClusterContext.getClusterState().isStable() ) {
      //Zeitfenster m�ssen nach �bergang in einen stabilen ClusterState erneut initialisiert werden
      if( XynaProperty.SCHEDULER_WAIT_FOR_STABLE_TIME_WINDOWS.get() ) {
        //Zeitfenster werden gesperrt angelegt, damit Auftr�ge im Scheduler nicht wegen fehlender 
        hasToWaitForStableTimeWindows = true;
      } else {
        //Zeitfenster werden angelegt, k�nnten bei Verwendung aber veraltete Definition haben
      }
    }
  ...
  public boolean hasToWaitForStartTime(SchedulingData schedulingData) {
    ...
    if( ! tcd.isConfigured() ) {
      return false; //keinerlei zeitliche Einschr�nkung
    }
    ...
    if( windowName != null ) {
      //TimeWindow fehlt: gleich in Scheduler eintragen, der soll sich dann um das Problem k�mmern
  ...
  public void addWaitingOrder(SchedulingOrder so, boolean wasNeverScheduled) {
    ...
    if( wasNeverScheduled) {
      ...
      //TODO ist es hier nicht sinnvoll, auftr�ge aktiv abzubrechen (wie beim cancel), falls bereits ein timeout erreicht wurde?
      // vorteil: kein unn�tiges scheduling
      
      if( so.isWaitingFor(WaitingCause.StartTime) ) {
        //Normales Scheduling hier, um SchedulingOrder in TimeConstraintExecutor einzutragen bzw. zum Melden, dass Auftrag lauff�hig ist. 
        TimeConstraintResult tcr = checkTimeConstraint(so);
        if (tcr.isExecutable()) {
          //Auftrag ist entweder sofort lauff�hig oder hat Fehler, der zum Abbruch f�hren muss
  ...
  public TimeConstraintResult checkTimeConstraint(SchedulingOrder so) {
    ...
    //4. Muss doch noch l�nger auf Startzeit gewartet werden?
    if( tcd.hasToWaitForStartTime(now) ) {
      //Auftrag muss noch l�nger warten
    ...
    String windowName = tcd.getTimeWindowName();
    if( windowName == null ) {
      //Keine Zeitfenster-Beschr�nkung, d.h startbar
      return TimeConstraintResult.SUCCESS;
    } else {
      //Zeitfenster holen, gesperrt gegen gleichzeitige �nderungen
  ...
  private TimeConstraintResult checkTimeConstraintForTimeWindow(SchedulingOrder so, TimeConstraintData tcd,
    ...
    //nun um die TimeConstraints k�mmern, die innerhalb des Zeitfensteres gelten:
    tcd.recalculateInWindow(window);
     
    //3. Im Zeitfenster k�nnte es Timeout geben
    if( tcd.hasTimeout(now) ) {
    ...
    //4. Im Zeitfenster k�nnte es eine neue Wartezeit geben
    if( tcd.hasToWaitForStartTime(now) ) {
      //Auftrag muss noch l�nger warten
      return waitAgain( so, null, tcd.getStartTimestamp(), now, false );
    }

    //5. Auftrag ist startf�hig
  ...
  /**
   * Eintragen der SchedulingOrder in die SchedulingTimeout-�berwachung
   * @param so
   * @param tcd
   */
  ...
  private TimeConstraintResult waitAgain(SchedulingOrder so, TimeConstraintWindow window, long startTime, long now, boolean schedulingOrderIsAlreadyLocked) {
    ...
    if( hasToWait < XynaProperty.SCHEDULER_CLOSED_TIMEWINDOW_REMOVE_TIME_OFFSET.getMillis() ) {
      //Auftrag ist in wenigen Millisekunden lauff�hig, daher ist es nicht n�tig, ihn aus 
   ...
   * SchedulingOrder befindet sich unter Umst�nden nicht im Scheduler, weil das alte Zeitfenster 
   * geschlossen ist oder der Startzeitpunkt noch nicht erreicht ist. 
   * Daher muss beim Wechsel des TimeConstraints daf�r gesorgt werden, dass diese Methode 
  ...
  /**
   * Fr�hzeitiger Timeout aller Auftr�ge, die ihren regul�ren Timeout innerhalb der n�chsten timeout ms h�tten
   * @param timeout
   */
  ...
  public void addTimeWindow(TimeConstraintWindowDefinition definition) throws XPRC_DuplicateTimeWindowNameException, PersistenceLayerException, XPRC_TimeWindowNotFoundInDatabaseException, XPRC_TimeWindowRemoteManagementException {
    ...
    //gegen konkurrierende �nderungen sch�tzen
    ...
    try {
      if( allTimeConstraintWindows.hasTimeWindow(name) ) {
        ...
        try {
          ...
          //addTimeWindow ist erfolgreich, keine Compensation n�tig
  ...
  public void removeTimeWindow(String name, boolean force) throws PersistenceLayerException, XPRC_TimeWindowStillUsedException {
    ...
    try {
      ...
      try {
        ...
        //zuletzt lokal aus DB l�schen
        removeTimeWindow.remove(name);
        //removeTimeWindow ist erfolgreich, keine Compensation n�tig
  ...
  public void changeTimeWindow(TimeConstraintWindowDefinition definition) throws PersistenceLayerException, XPRC_TimeWindowNotFoundInDatabaseException, XPRC_TimeWindowRemoteManagementException {
    ...
    try {
      ...
      try {
        ...
        //changeTimeWindow ist erfolgreich, keine Compensation n�tig
