 ...
 * komplexere Zeitfenster zu erm�glichen.
 * Das MultiTimeWindow gilt als offen, wenn mindestens eines der Zeitfenster offen ist.
 * Da sich die zusammengefassten Zeitfenster �berlappen k�nnen, sind die berechnete Zeitpunkte 
 * des n�chsten �ffnens und Schlie�ens evtl. nicht richtig. In diesem Fall wird in TimeWindowData
...
public class MultiTimeWindow extends TimeWindow {
  ...
  protected TimeWindowData recalculateInternal(long now) {
    ...
    if( isOpen ) {
      //derzeit offen -> nextClose muss gesch�tzt werden, daraus dann nextOpen berechnen
      ...
      //derzeit geschlossen -> nextOpen ist einfach ermittelbar, daraus dann nextClose sch�tzen
      nextOpen = getMinNextOpen(data);
  ...
  /**
   * Achtung: wenn nextClose nur gesch�tzt wird, ist Ausgabe negativ
   * @param now
   * @return
   */
  private long getNextClose(long now) {
    //Zeitfenster ist nur geschlossen, wenn alle Sub-Zeitfenster geschlossen sind. 
    //Aufgrund von �berlappungen der Offen-Phasen muss hier iterativ nach dem 
    //n�chsten Schlie�en gesucht werden
  ...
  /**
   * Achtung: wenn since nur gesch�tzt wird, ist Ausgabe negativ
   * @param now
   * @return
   */
  private long getOpenSince(long now) {
    ...
    for( int i=0; i<10; ++i ) { //FIXME Grenze konfigurieren, hilft gegen Endlosschleife
      ...
      if( openSince == estimatedOpenSince ) {
        ...
        //zu diesem Zeitpunkt gibt es bereits offene Zeitfenster: nach fr�herem OpenSince weitersuchen
  ...
  public static class MultiTimeWindowDefinition extends TimeWindowDefinition {
    ...
    public MultiTimeWindowDefinition(ArrayList<TimeWindow> timeWindows) {
      ...
      for( TimeWindow tw : timeWindows ) {
        definitions.add(tw.getDefinition()); //FIXME pr�fen, dass kein MultiTimeWindow
      }
    }
    ...
    //FIXME pr�fen, dass kein MultiTimeWindowDefinition in timeWindowDefinitions enthalten
