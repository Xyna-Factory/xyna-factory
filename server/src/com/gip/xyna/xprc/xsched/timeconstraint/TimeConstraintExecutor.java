 ...
 * zeitabh�ngigen Aufgaben k�mmert.
 * Diese Aufgaben sind
 * <ul>
...
public class TimeConstraintExecutor implements Executor<TimeConstraintExecutor.TimeConstraintTask> {
  ...
  /**
   * Entfernen des SchedulingTimeout f�r den wartenden Auftrag
   * @param orderId
   * @return
   */
  ...
  /**
   * Basisklasse f�r alle hier verwendeten {@link TimedTasks}. 
   * Die vollst�ndige Implementation dieser Klasse ist sowohl der TimedTasks-Task als auch 
   * �ber die execute-Methode die eigentliche {@link TimedTasks.Executor}-Implementierung.
   * (Die �bliche Trennung im TimedTasks zwischen einem Executor und simplen Task-Objekten 
   * musste hier aufgegeben werden, damit die derzeit drei verschiedenen Task-Typen in einem 
   * TimedTasks laufen k�nnen.
   */
  ...
  /**
   * Abbrechen der Auftr�ge mit Timeout
   * Achtung, diese TimeConstraintTasks werden derzeit nicht aus der TaskListe entfernt, da 
   * das Suchen in der TaskListe aufw�ndig ist, die �berfl�ssige Arbeit dagegen gering ist 
   * und die unn�tige Referenz auf die SchedulingOrder nur wenig Speicher ben�tigt.
   */
  ...
  /**
   * Auftr�ge, die auf ihren Startzeitpunkt warten, werden hier�ber wieder in den Scheduler eingestellt.
   *
   */
  ...
  /**
   * TimeWindows werden �ber diesen TimeConstraintTask ge�ffnet und geschlossen.
   *
   */
  public static class TimeWindowChanger extends TimeConstraintTask {
    ...
    @Override
    public void execute(TimeConstraintManagement timeConstraintManagement, AllOrdersList allOrders) {
      timeWindow.lock(); //gegen gleichzeitige Verwendung im Scheduler sch�tzen
      try {
        this.timestamp = timeWindow.getNextChange(System.currentTimeMillis());
