...
/**
 * Speicherung aller TimeConstraintWindows, �berwachung aller Add-, Remove- und Replace-Vorg�nge. 
 *
 */
public class AllTimeConstraintWindows {
  ...
  public void replaceTimeWindow(TimeConstraintWindow timeWindow) {
    ...
    existing.lock(); //Sicherstellen, dass Scheduler nicht gleichzeitig schedulen m�chte
    try {
      //kein richtiges replace hier: Scheduler k�nnte bereits TimeConstraintWindow existing haben
      ...
      //wartende Auftr�ge wieder in den Scheduler einstellen
      if( existing.isOpen() ) {
  ...
  public void removeTimeWindow(String name) {
    ...
    timeWindow.lock(); //gegen Verwendung im Scheduler sch�tzen
    try {
      timeWindow.setRemoved(); //kann nun nicht mehr verwendet werden
      //timeConstraintExecutor muss sich nicht mehr um dieses Zeitfenster k�mmern
