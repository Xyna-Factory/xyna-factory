 ...
 public static final String CLO_ALREADY_EXECUTED_OR_CHANGED = 
  ...
  private final class CronLikeOrderAck extends AbstractConnectionAwareAck {
        ...
        public Boolean executeAndCommit(ODSConnection con) throws PersistenceLayerException {
          try {
            if (!internally(xose, con)) {
              con.rollback();  //TODO sch�digt evtl. �u�ere Transaktion
              return false;
            }
            return true;
          } catch (Error e) {
            updateCLOonError(con); //TODO sch�digt evtl. �u�ere Transaktion
            throw e;
    ...
    private boolean internally(XynaOrderServerExtension cronTargetOrder, ODSConnection con)
      ...
      // CLO neu aus DB laden und dabei locken. Gibt die Sicherheit, dass die CLO nicht gleichzeitig ausgef�hrt wird
      // oder jetzt vor dem Ausf�hren ge�ndert wird.
      ...
      try {
        ...
        // Auftrag muss schon ausgef�hrt wurden sein.
      ...
      if(!cronOrderDB.getNextExecution().equals(order.getNextExecution())) {
        // Ausf�hrungszeit wurde ge�ndert -> Abbruch der Ausf�hrung
        // U.U. wurde der Auftrag schon ausgef�hrt.
