...
public class CronLikeTimer extends ManagedPlainThreadFromPausableRunnable {
  ...
  protected void runOnce() throws Exception {
    ...
    synchronized (cronQueue) {
      ...
      if (empty) {
        ...
        //so werden diese wieder verdr�ngt, wenn nicht gen�gend Platz ist und der
        //Ausf�hrungszeitpunkt zu weit in der Zukunft liegt. Kann ein Cron nicht eingef�gt werden,
    ...
    synchronized (cronQueue) {
      ...
      if (next != null) {   
        ...
        if (next.isExecutionTimeReached()) {
          ...
          if (logger.isDebugEnabled()) {
            //auftrag ist aber nicht mehr notwendigerweise in der queue - evtl wurde er bereits gel�scht.
  ...
  private void startUnderlyingOrder(final CronLikeOrder next) throws InterruptedException, XNWH_RetryTransactionException {
    ...
    while (!started && runToggle) {
      try {
        ...
        cnt++;
        cronQueue.wait(100); //gibt das lock frei, damit andere threads nicht warten m�ssen
      }
    }
  }
  ...
  /**
   * Order wird nicht zur Scheduling-Queue hinzugef�gt. Wenn sie schon drin sind, wird sie dort entfernt. 
   */
  ...
  /**
   * Queue komplett leeren und neu aus DB bef�llen ...
   */
  ...
  /**
   * Order wird nicht zur Scheduling-Queue hinzugef�gt. Wenn sie schon drin sind, ist es zu sp�t und sie werden dennoch ausgef�hrt. 
   */
    ...
    synchronized (cronQueue) { // stopps the execution to add a new CLO into the queue. otherwise the CLO may be running
      ...
      while(iter.hasNext()) {
        ...
        if (order.getNextExecution() <= cronQueue.getLatestExecutionTimeInQueue()) {
          //muss fr�her gestartet werden als der auftrag mit der l�ngsten wartezeit in der queue
      ...
      if(cronQueue.size() > maxQueueLengthUpperBound) {
        // Queue k�rzen ... letztes Element l�schen
  ...
  public boolean tryAddNewOrderWithoutUnmarkOrder(CronLikeOrder order, boolean addFromPersistenceLayer) {
    synchronized (cronQueue) { // stopps the execution to add a new CLO into the queue. otherwise the CLO may be running
      ...
      if(added) {
        ...
        //wenn die Queue leer ist, m�ssen neue Crons aus der DB gelesen werden
