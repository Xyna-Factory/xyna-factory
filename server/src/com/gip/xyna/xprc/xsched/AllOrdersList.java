...
/**
 * AllOrdersList ist die Liste aller Auftr�ge im XynaScheduler. 
 * 
 * Der SchedulerAlgorithm fragt regelm��ig nach den neuen Auftr�gen und pflegt diese in einer 
 * eigenen Liste. Die eigene Liste ist auf Grund der anderen Sortierung n�tig und hilfreich, 
 ...
 * jeder geschedulte Auftrage �ber {@link #removeOrder(SchedulingOrder)} aus der Liste aller 
 * Auftr�ge entfernt.<br> 
 * 
 * Da die interne Speicherung in AllOrdersList eine ConcurrentHashMap ist, k�nnen viele Operationen 
 * ohne Lock auskommen. Allerdings hat die ConcurrentHashMap den Nachteil, dass sie nicht mehr 
 * schrumpft, wenn sie zuviele Auftr�ge speichern musste.<br>
 * 
 * Zus�tzlich zur Liste aller Auftr�ge wird noch eine weitere, kleinere Liste von SchedulingOrders 
 ...
 * �ber diese Liste werden XynaOrders in der SchedulingOrder gebackupt und restoret.<br> 
 * Dadurch, dass nur alte XynaOrders gebackupt und aus dem Speicher entfernt werden und wieder 
 * nachgeladene XynaOrders als neu gelten, ist gew�hrleistet, dass der Scheduler selten XynaOrders 
 * mehrfach nachladen muss. Mehrfaches Nachladen sollte nur passieren, wenn Auftr�ge langdauernd 
 ...
 * Parent-Auftr�ge hat, siehe {@link com.gip.xyna.xprc.xsched.scheduling.SchedulingOrder#isRelevantForOOMProtection() SchedulingOrder.isRelevantForOOMProtection()}.<br> 
 *  
 * <br><br>
 * 
 * TODO Was w�re noch gut?<br> 
 * OOMProtectionList sollte keine LinkedList sein, sondern HashMap bzw LinkedHashMap, 
 * damit die remove-Operationen g�nstig durchgef�hrt werden k�nnen. 
 * Mit Umsetzung von Bug 13829 k�nnte OOMProtectionList eine einfache Erweiterung von LRUCache sein
 * 
 * 
 */
public class AllOrdersList {
  ...
  private OOMProtectionList oomOrders;           //f�r OOM-Schutz relevante SchedulingOrder
  
  private CanceledOrders canceledOrders;         //f�r gleichzeitiges Add- und CancelOrder: Cancel darf vor Add gerufen werden
  
  private Scheduler<SchedulingOrder,SchedulerInformationBean> scheduler;
  
  /**
   * Liste aller SchedulingOrders, die wegen OOM �berwacht werden m�ssen.
   * 
   * {@link #add(SchedulingOrder) } �berwacht, dass nicht mehr als maxOrders Auftr�ge in der Liste stehen.
   * �berz�hlige werden gebackupt und aus der Liste entfernt. Dabei werden die �ltesten Aufr�ge (vorne) 
   * in der Liste entfernt, neue Auftr�ge werden hinten angeh�ngt.
   * 
   * Geschedulte oder gecancelte Auftr�ge werden nicht direkt nach dem Schedulen oder beim Cancel aus der
   * Liste entfernt, daher w�chst die OOMProtectionList immer auf maxOrders an, bevor sie wieder stark 
   * zusammenschrumpft, wenn im addOrder alle geschedulten oder gecancelten Auftr�ge entfernt werden.
   * 
   * Grund daf�r ist die h�here Effizienz einer vollst�ndigen Iteration �ber die OOMProtectionList im 
   * addOrder, welche nur alle maxOrders/10 bis maxOrders (also typischerweise 1000-10000) eingestellten 
   * Auftr�ge auftritt im Vergleich zu einer Iteration bei jedem Cancel oder nach jedem Schedule-Vorgang.
   *
   */
  private static class OOMProtectionList {
    ...
    /**
     * Aufnahme der SchedulingOrder in die OOMProtectionList, wenn sie f�r den OOM-Schutz relevant ist
     * @param so
     */
    ...
    /**
     * Liste ist zu lang geworden: Zuerst Aufr�umen, danach Entfernen einiger Eintr�ge 
     * aus der OOMProtectionList, damit OOM-Schutz gew�hrleistet ist. XynaOrders werden 
    ...
    private void decreaseListSize() {
      ...
      while (cnt < ordersToBackup && i++ < oomOrders.size()) {
        SchedulingOrder so = oomOrders.remove();//holt �ltesten Eintrag (vorne)
        synchronized (so) {
          if( so.isLocked() ) {
        ...
        try {
          if( so.canBeRemovedFromOOMProtection() ) {
            //Auftrag l�uft nun anscheinend doch schon. (Im Scheduler wird 
            //...OrderInstanceStatus auf Running gesetzt)
            cnt++;
          } else {
            if( so.backup() ) { //TODO batch-verarbeitung f�r die backups
  ...
  private static class CanceledOrders implements Executor<CanceledOrders.Cancel> {
    ...
    /**
     * falls kein listener existiert, wird false zur�ckgegeben.
     * 
     * ansonsten signalisiert der r�ckgabewert, ob der aufrufer den auftrag canceln muss.
     * beispiele:
  ...
  /**
   * gibt den gecancelten auftrag zur�ck, falls er aus der liste entfernt wurde
   */
    ...
    try {
      ...
      switch( removeState ) {
        ...
        case ResumeOrCompensate:
          ...
          //in beiden f�llen soll kein listener registriert werden.
          if (listener != null) {
            listener.cancelFailed();
          }
          return null;
        case Aborted:
          // auftrag konnte nicht entfernt werden, aber abgebrochen. das ist nur f�r den fall:
  ...
  public enum RemoveState {
    ...
    AlreadyScheduled(true,false),  //Auftrag wurde bereits ausgef�hrt
    ResumeOrCompensate(true,false),//Resume- oder Compensate-Auftrag wird nicht abgebrochen
    Aborted(true,true),            //Auftrag w�hrend eines Resumes; wurde nun abgebrochen, um nicht auf Caps oder Vetos zu warten
   ...
   * 
   * gibt die xynaorder zur�ck, falls abort erfolgreich, und true/false ob der auftrag entfernt wurde.
   */
  public Pair<XynaOrderServerExtension, RemoveState> removeOrder(long orderId, boolean removeCompensatingAndResumes, boolean ignoreResourcesWhenResuming) {
    ...
    try { 
      ...
      //Order nur entfernen, wenn man sicher ist, dass man das will. weil man hinterher nicht gut zur�ck kann
      //Daher nun erst einige Pr�fungen
      ...
      if( process != null ) {
        ...
        if (!ignoreResourcesWhenResuming && isResuming ) {
          ...
          if ( schedulingData.needsCapacities() || ! schedulingData.getVetos().isEmpty() ) {
            //auftrag ben�tigt capacities oder vetos und diese sollen beim resume nicht ignoriert werden
  ...
  /**
   * AllOrders ist daf�r veranwortlich, die XynaOrders f�rs Scheduling zu liefern
   * @param so
  ...
  public void timeoutOrder(SchedulingOrder so) {
    synchronized(so) {
      so.markAsTimedout();
      //Reschedule tr�gt Auftrag evtl wieder in Scheduler ein, wenn SchedulingOrder au�erhalb aufbewahrt wird
  ...
  private boolean tryScheduleOrder(SchedulingOrder so, String msg, boolean addToScheduler, boolean recalculateUrgency, boolean reschedule) {
    ...
    if( so.isMarkedAsNew() ) {
      ...
      if( so.isLocked() ) {
        //SchedulingOrder ist gesperrt: daher keine weiteren �nderungen vornehmen sondern f�rs Unlock vormerken.
        if( addToScheduler ) {
      ...
      if( so.isAlreadyScheduled() ) {
        ...
        //Auftrag kann nicht regul�r geschedult werden, deswegen Scheduler zum Aufr�umen geben
        rescheduleOrder(so);
      }
      return false;
    } else {
  ...
  private void addOrderToScheduler(SchedulingOrder so) {
    ...
    //.. Backup w�hrend newOrdersLock gelesen werden muss
   ...
   * @param so
   * @param orderInstanceStatus null ist erlaubt -&gt; keine �nderung
   */
  ...
  public void deploymentFinished(Long orderId) {
    SchedulingOrder so = allOrders.get(orderId);
    if( so == null ) {
      return; //Order nicht gefunden, sollte nicht auftreten k�nnen
    }
    synchronized (so) {
      so.waitIfLocked();
  ...
  public int changeCapacityRequirement(DestinationKey destinationKey, boolean add, String capName, int cardinality) {
    ...
    for( SchedulingOrder so : affected ) {
      synchronized (so) { //gegen gleichzeitiges Schedulen gesch�tzt
        so.waitIfLocked();
        if( !so.canBeScheduled() ) {
          continue; //zu sp�t, Auftrag kann nicht mehr geschedult werden
        }
