 ...
 * SeriesInformationStorable remote durchf�hrt.<br>
 * <br>
 * Algorithmus f�r update{Successor/Predecessor}<br>
 * 1) Ist <code>clustered==true</code><br>
 * 1.1) Ja: Remote-Aufruf �ber RMIClusterProviderTools<br>
 * 1.2) Nein: Aufruf update{Successor/Predecessor}Later<br>
 * <br>
 * Algorithmus f�r update{Successor/Predecessor}Later<br>
...
public class OSMRemoteProxyImpl implements OSMRemoteInterface {
  ...
  private Result updateSuccessorLater(int binding, String successorCorrId, String predecessorCorrId,
    ...
    //Binding �berpr�fen, da sonst Endlosschleife auftritt, falls Knoten mit dem binding <code>binding</code> nicht wiederkommt
  ...
  private Result executeLater(OSMTask osmTask) {
    //Mit Verz�gerung einstellen zur Lastbegrenzung
    //2 Anwendungsf�lle:
    //a) kurzfristiger Ausfall der RMI-Verbindung
    //b) Ordermigration: Predecessor ist bereits migriert, und m�chte nun updateSuccessor ausf�hren,
  ...
  private Result singleResult(List<Result> res) {
    ...
    if( later ) {
      ...
      return Result.NotFound; //Result.NotFound war einzige R�ckmeldung
    }
  }
