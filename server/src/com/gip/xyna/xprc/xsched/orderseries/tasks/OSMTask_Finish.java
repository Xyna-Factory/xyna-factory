 ...
 * wird hier nicht �berpr�ft, der Update hier �berschreibt sogar die bisherigen �nderungen in der DB.
 * Wichtig ist er aber zur Konsistenzwahrung im Fehlerfall, wenn dieser Task nicht mehr vollst�ndig 
 ...
 * 3.1) Falls der Auftrag gecancelt wurde, muss entschieden werden, ob die Successoren laufen d�rfen.
 * 3.1.1) Es sind noch nicht alle Predecessoren fertig: nur OrderStatus auf CANCELING setzen und Task beenden
 * 3.1.2) Auftrag war regul�r lauff�hig: OrderStatus auf CANCELED setzen, weiter mit 4)
 * 4) Nachfolger muss gecancelt werden, wenn OrderStatus.isError=true und AutoCancel=true
 * 5) Schleife �ber alle SuccessorCorrIds
 * 5.1) Zugriff auf Predecessor-Baum sucTree zu SuccessorCorrId
 * 5.1.1) Falls sucTree nicht existiert, Neubau.
 * 5.2) Auswertung des Ergebnis des Aufrufs updateSuccessorInternal(sucTree), 3 F�lle
 ...
 * 1.2) sucTree hat eigenes Binding: R�ckgabe Aufruf 
 *                                   OSMLocalImpl.updateSuccessor (siehe dort)
 * 1.3) sucTree hat fremdes Binding: R�ckgabe Remote-Aufruf 
...
public class OSMTask_Finish extends OSMTask {
  ...
  protected void executeInternal() {
    ...
    try {
      ...
      //Auftrag wurde gecancelt. Nun entscheiden, ob Successoren starten d�rfen
      if( orderStatus == OrderStatus.CANCELING ) {
        if( sis.getPredecessorCorrIds().isEmpty() ) {
          //Auftrag war regul�r lauff�hig, d.h. der Auftrag ist fertig und Nachfolger d�rfen starten
          orderStatus = OrderStatus.CANCELED;
        } else {
          //nur den Status CANCELING speichern. Successoren d�rfen noch nicht benachrichtigt werden
      ...
      while( iter.hasNext() ) {
        ...
        if( sucTree == null ) {
          //hier muss kein gro�er Baum gebaut werden, da hier nur die Informationen des TreeNode 
          //selbst gebraucht werden. Falls doch irgendwann der vollst�ndige Baum in 
          //OSMTask_Preschedule gebraucht wird, wird er dort erg�nzt werden.
          sucTree = predecessorTrees.buildShortTree(successorCorrId); 
        } else {
          if( ! sucTree.hasData() ) {
            //Daten erg�nzen, da Successor ja benachrichtigt werden muss 
            sucTree = predecessorTrees.buildShortTree(successorCorrId);
          }
        }
        ...
        switch( result ) {
          case NotFound:
            ...
            break;
          case Later:
            //Successor kann derzeit nicht bearbeitet werden, Operation wird sp�ter wiederholt
