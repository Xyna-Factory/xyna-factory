 ...
 * OSMTask_Resume:
 * <br>
 * Im OrderBackupHelperProcessAbstract gerufen, um f�r gebackupte Sereinauftr�ge erneut zu 
 * untersuchen, ob diese starten k�nnen. In der Zwischenzeit (w�hrend des Backups) k�nnten 
 * unter Umst�nden Predecessoren gelaufen sein, ohne dass diese ihren Successor starten konnten.
 ...
 * allerdings wird hier zus�tzlich vorher f�r jeden bekannten Predecessor �berpr�ft, ob dieser nicht
 * mittlerweile fertig ist. Dies hilft bei dem Fehler, dass der Server gecrasht ist und nicht alle
 * Daten �ber fertige Auftr�ge in SeriesInformationStorable eintragen konnte.
 * Leider ist h�ufig das OrderArchive so konfiguriert, dass diese Information nicht mehr nachtr�glich 
...
public class OSMTask_Resume extends OSMTask {
      ...
      try {
        ...
        if( sisPre != null ) {
          ...
          if( ! sisPre.getOrderStatus().isFinished() ) {
            ...
            //dies jetzt hier gepr�ft werden. Es besteht n�mlich die M�glichkeit, dass der Update 
  ...
  private void checkFinished(SeriesInformationStorable sisPre) {
    ...
    if( mws == null ) {
      ...
      //die Nachfolge-Auftr�ge nie automatisch starten und daher manuell gestartet werden m�ssen.
    } else {
      switch( mws ) {
        case CANCELED:
          if( sisPre.getOrderStatus() != OrderStatus.CANCELING ) {
            ...
            //hier k�nnte selten ein Fehler entstehen, wenn OrderStatus CANCELING ist, der
            //Auftrag bereits gecancelt ist, alle Vorg�nger bereits fertig sind und diese 
