 ...
 * <br>
 * Wird von OSMLocalImpl gerufen, wenn f�r den Aufruf updatePredecessor das n�tige Lock nicht erhalten wurde.
 * <br>
 ...
 * 2.2) Later:   Successor wird sp�ter benachrichtigt werden, nichts mehr weiter zu tun
 * 3.3) Running:  Predecessor ist noch nicht gelaufen, nichts mehr weiter zu tun
 * 3.4) Cancel, Success: Predecessor ist beendet. Dar�ber muss nun der Successor informiert werden
 * 3.4.1) Suche der TreeNodes zu Predecessor und Successor
 * 3.4.2) Auswertung des Ergebnis des Aufrufs updateSuccessorInternal(sucTree), 3 F�lle
...
public class OSMTask_UpdatePredecessor extends OSMTask {
  ...
  protected void executeInternal() {
    ...
    switch( result ) {
      case Later: 
        //ok, nichts mehr weiter zu tun, Successor wird sp�ter benachrichtigt werden
      case NotFound: 
    ...
    if( informSuccessor ) {
      ...
      if( sucTree == null ) {
        ...
        if( ! sucTree.hasData() ) {
          //Daten m�ssen existieren, da Successor ja vorhanden ist
          sucTree = predecessorTrees.buildShortTree(successorCorrId);
        }
      }
