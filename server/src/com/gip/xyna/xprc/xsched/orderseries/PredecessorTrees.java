...
/**
 * PredecessorTrees verwaltet alle bekannten Predecessor-Abh�ngigkeiten.
...
public class PredecessorTrees {
  ...
  public static class TreeNode extends Tree<String,SisData> {
    ...
    public TreeNode getBranch( String correlationId ) { //wandelt ReturnType f�r einfachere Verwendung
      return (TreeNode) branches.get(correlationId);
    }

    /**
     * Liefert das Binding (Achtung: vorher mit hasData() pr�fen, sonst NPE!)
     * @return 
     */
    public int getBinding() {
    ...
    /**
     * Liefert die OrderId (Achtung: vorher mit hasData() pr�fen, sonst NPE!)
     * @return
     */
    public long getOrderId() {
  ...
  private static class TreeBuilder implements Tree.TreeBuilder<String, SisData, TreeNode> {
    ...
    public TreeNode createTree(String correlationId) {
      ...
      if( cachedTree.getValue() == null ) { //(orderId,binding) erg�nzen
    ...
    public Collection<String> getBranchKeys(String correlationId) {
      ...
      if( sis == null ) {
        ...
        //Hier wird auf sis zugegriffen, ohne dass ein Lock sch�tzt: nur sichere Operationen verwenden
        //sis.getPredecessorCorrIds().isEmpty() wird als sicher angesehen
        if( sis.getPredecessorCorrIds().isEmpty() ) {
          //h�ufiger Spezialfall: alle Predecessoren sind bereits gelaufen
          ...
          if( osmCache.tryLock( correlationId ) ) { //es wurden bereits Locks geholt
            ...
            //es ist unwahrscheinlich, dass gerade die Daten ver�ndert werden.
     ...
     * (Lokaler Cache ist n�tig, da meist die beiden Aufrufe createTree und getBranchKeys
     * direkt nacheinander kommen und vor allem die teure Suche fehlender Eintr�ge im OSMCache
   ...
   * @param correlationId
   * @return true, falls eine zyklische Abh�ngigkeit besteht
   */
  ...
  /**
   * Aussortieren aller PredecessorTrees, die nicht mehr ben�tigt werden
   * Ben�tigt werden alle PredecessorTrees mit eigenem Binding
   * Ben�tigt werden alle von ben�tigten PredecessorTrees referenzierten PredecessorTrees. 
