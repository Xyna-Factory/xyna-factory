...
/**
 * OSMLocalImpl:
 * Implementierung des Interfaces OSMRemoteInterface, die die notwendigen �nderungen an den 
 * SeriesInformationStorable lokal durchf�hrt.  
 *
 * Algorithmus f�r update{Successor/Predecessor}
 * <pre>
 * 1) tryLock der corrId
 * 1.1) Lock erhalten
 * 1.1.1) R�ckgabe des Ergebnis von update{Successor/Predecessor}WithoutLock
 * 1.1.2) Unlock
 * 1.2) Lock nicht erhalten
 * 1.2.1) Einstellen des Tasks OSMTask_Update{Successor/Predecessor} in die Task-Queue
 * 1.2.2) R�ckgabe von Result.Later
 * </pre>
 * Algorithmus f�r updateSuccessorWithoutLock
 * <pre>
 ...
 * 3) Enth�lt <code>sisSuc</code> <code>predecessorCorrId</code>?
 * 3.1) Ja: <code>predecessorOrderId</code> eintragen, alle Vorkommen von <code>predecessorCorrId</code> entfernen
 * 3.2) Nein: Enth�lt <code>sisSuc</code> <code>predecessorOrderId</code> bereits?
 * 3.2.1) Ja: Ok
 ...
 * 6) Update <code>sisSuc</code>, falls sich �nderungen ergeben haben
 ...
 * 4) Enth�lt <code>sisPre</code> <code>successorCorrId</code>?
 * 4.1) Ja: <code>successorOrderId</code> eintragen, alle weiteren Vorkommen von <code>successorCorrId</code> entfernen
 * 4.2) Nein: Enth�lt <code>sisPre</code> <code>successorOrderId</code> bereits?
 * 4.2.1) Ja: Ok
 * 4.2.2) Nein: Predecessor und Successor passen nicht zusammen, Warnung ins Log, <code>successorOrderId</code> eintragen
 * 5) Update <code>sisPre</code>, falls sich �nderungen ergeben haben
 * 6) Enth�lt <code>sisPre</code> <code>successorCorrId</code>s?
 * 6.1) Ja: Da kein Successor mehr den Predecessor ben�tigt, kann er aus PredecessorTrees und OsmCache entfernt werden
 * 7) Wenn <code>sisPre</code> <code>hadError==true</code> und <code>autoCancel==true</code> hat:
 * 7.1) Ja: R�ckgabe <code>Result.Cancel</code>
 * 7.2) Nein: R�ckgabe <code>Result.Success</code>
 * </pre>
 */
...
public class OSMLocalImpl implements OSMRemoteInterface {
  ...
  /**
   * Warten mit tryLock, falls Lock nicht erhalten wird: Auftrag in Queue einstellen und sp�ter probieren
  ...
  public Result updateSuccessor(int binding, String successorCorrId, String predecessorCorrId, long predecessorOrderId,
    ...
    if( osmCache.tryLock(successorCorrId) ) {
      ...
      //wird dieser Update in einen OSMTask verpackt, der dann sp�ter nochmal vom 
  ...
  /**
   * Eigentlicher Update des Successor ohne Ber�cksichtigung der Locks
   * (Auftragsende)
   * @param successorCorrId
   * @param predecessorCorrId
   * @param predecessorOrderId
  ...
  private Result updateSuccessorWithoutLock(String successorCorrId, String predecessorCorrId, long predecessorOrderId,
    ...
    if( sisSuc.getPredecessorCorrIds().remove(predecessorCorrId) ) {
      ...
      if( sisSuc.getPredecessorOrderIds().contains(predecessorOrderId) ) {
        ...
        //predecessorOrderId trotzdem nachtr�glich in Successor eintragen
        sisSuc.getPredecessorOrderIds().add( predecessorOrderId );
      }
    }
    if( cancel ) {
    ...
    if( sisSuc.getOrderStatus() == OrderStatus.CANCELING ) {
      if( sisSuc.getPredecessorCorrIds().isEmpty() ) {
        //keine weiteren Vorg�nger vorhanden, daher nun mit einem OSMTask_Finish den
    ...
    if( sisSuc.getOrderStatus().isFinished() 
      ...
      //sisSuc sollte von niemandem mehr ben�tigt werden, daher aus dem Cache entfernen
  ...
  /**
   * Eigentlicher Update des Predecessor ohne Ber�cksichtigung der Locks
   * (Auftragseingang)
   * @param predecessorCorrId
   * @param successorCorrId
   * @param successorOrderId
  ...
  private Result updatePredecessorWithoutLock(String predecessorCorrId, String successorCorrId, long successorOrderId) {
    ...
    if( sisPre.getSuccessorCorrIds().remove(successorCorrId) ) {
      ...
      if( sisPre.getSuccessorOrderIds().contains(successorOrderId) ) {
        ...
        //successorOrderId trotzdem nachtr�glich in Predecessor eintragen
        sisPre.getSuccessorOrderIds().add( successorOrderId );
      }
    }
    if( changed ) {
