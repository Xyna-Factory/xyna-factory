...
public class OrderSeriesSeparator {
  ...
  /**
   * Liegt die SeriesInformation im alten Format vor und m�ssen deshalb die darin enthaltenen 
   ...
   * <ul>
   * <li> Rekursive Sammlung aller Auftr�ge, die in den SeriesInformation gespeichert
   ...
   * <li> F�r alle XynaOrder au�er der �bergebenen: Erzeugung der 
   *      XynaOrderServerExtension, F�llen des OrderContext und Speichern in Liste, 
   *      die �ber {@link getSeries()} erh�ltlich ist
   * </ul>
   * @param xo
   */
  public void separate(XynaOrderServerExtension xo) {
    ...
    //nun erh�lt jeder Auftrag eine neue SeriesInformation
    ...
    if( hasToBeAcknowledged ) {
      ...
      //Sichern des urspr�ngliches Acks, da es sonst aus dem gemeinsamen OrderContext verschwindet
      originalAck = xo.getOrderContext().getAcknowledgableObject();
    }
  }
  
  /**
   * Liefert Liste aller Serien-Auftr�ge ohne den Basis-Auftrag
   * @return
   */
  ...
  /**
   * Extraktion aller SeriesInformation->XynaOrder aus der �bergeben XynaOrder
   * @param allOrders
   * @param xo
   */
  @SuppressWarnings("deprecation")
  ...
  public void prepareAcknowledge(XynaOrderServerExtension xose) {
    //BackupAck jedes mal neu setzen, da es aus dem gemeinsamen OrderContext nach jedem acknowledge gel�scht wird
  ...
  public void restoreAcknowledge(XynaOrderServerExtension xo) {
    if( hasToBeAcknowledged ) {
      //urspr�ngliches Ack wiederherstellen
