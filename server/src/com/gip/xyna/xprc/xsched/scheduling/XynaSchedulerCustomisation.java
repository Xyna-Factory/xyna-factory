...
public class XynaSchedulerCustomisation implements SchedulerCustomisation<SchedulingOrder,SchedulerInformationBean> {
  ...
  private TryScheduleResult tryScheduleOrder(SchedulingOrder so) { 
    synchronized (so) { //SchedulingOrder kann nicht mehr fremd durch AllOrders.remove etc. ver�ndert werden
      if( so.isLocked() ) {
        return TryScheduleResult.RETRY_NEXT; //Auftrag �berspringen und Scheduling wiederholen
      }
      if( so.isWaitingOrLocked() ) { //locked sollte hier nicht auftreten k�nnen
        return TryScheduleResult.CONTINUE; //Auftrag �berspringen
      }
      ...
      if( so.isAlreadyScheduled() ) {
        //XynaOrder wurde bereits geschedult, aber nicht aus der Liste gel�scht. 
  ...
  private ScheduleResult switchTryScheduleResult( TryScheduleResult result, Urgency<SchedulingOrder> uo ) {
    SchedulingOrder so = uo.getOrder();
    so.setTag(null); //bisherige Tags l�schen
    switch (result.getType()) {
      ...
      case CAPACITY :
        ...
        if( maxDemandTried ) {
          ...
          //die diese Capacity ebenfalls ben�tigen. (SchedulingOrders m�ssen bereits getaggt sein) 
          return new TagScheduleResult(capName, true);
        } else {
          //obwohl bekannt ist, dass die Capacity nicht mehr verf�gbar ist, m�ssen trotzdem weitere
          //Auftr�ge geschedult werden, um dem anderen Knoten den Bedarf melden zu k�nnen
      ...
      case CONTINUE :
        //Auftrag soll aus irgendeinem Grund �bersprungen werden
        break;
      case RETRY_NEXT :
        notifyScheduler(); //sicherstellen, dass es einen n�chsten Schedulerlauf gibt
        return ScheduleResult.Continue;
      case REORDER :
  ...
  public void postparation() {
    //Caps fuer andere Knoten reservieren, eigenen Bedarf melden und Capacities �bertragen
