...
public abstract class TryScheduleAbstract implements TrySchedule {
  ...
  private final XynaPropertyBuilds<Execution> regularExecutionPipeline = 
    ...
    .setDefaultDocumentation(DocumentationLanguage.DE, "Wenn mit 'true' belegt wird ein laufendes Backup nicht dazu f�hren das weniger dringliche Auftr�ge welche die gleichen Ressourcen ben�tigen vorgezogen werden.");
  ...
  /**
   * K�mmert sich um ein einfaches uns sicheres Ausf�hren von Capacity- und Veto-Allozieren 
   * und dem anschlie�enden Schedulen, so dass im Fehlerfall ordentlich aufger�umt wird.
   * Das Aufr�umen geschieht im finally-Block, so das es sicher gegen Exceptions ist
   *
    ...
    public TryScheduleResult tryExecute(SchedulingOrder so ) {
      ...
      try {
        if( so.canBeScheduled() || subExecution == null ) {
          //nur ausf�hren, wenn Order geschedult werden kann, letzte Execution (subExecution == null) muss trotzdem ausf�hren
          tsr = execute(so);
        }
        if( tsr == null ) {
          //es ist kein Fehler aufgetreten, daher subExecution ausf�hren
        ...
        try {
          if( tsr == null ) {
            ...
            //oder ein regul�rer Fehler mit gesetztem tsr
            undo(so);
          } else {
  ...
  private class VetoAllocation extends Execution {
    ...
    private boolean needsVetos; //TODO unsch�n, dass in einer nicht-auftragsabh�ngigen Klasse tempor�r Daten 
    ...
    protected TryScheduleResult execute(SchedulingOrder so) {
      ...
      if( needsVetos ) {
        needsVetos = so.getSchedulingData().isNeedsToAcquireVetosOnNextScheduling(); //TODO n�tig?
      }
      if( needsVetos ) {
  ...
  private class CapacityAllocation extends Execution {
    ...
    private boolean needsCapacities; //TODO unsch�n, dass in einer nicht-auftragsabh�ngigen Klasse tempor�r Daten 
    //zu einem Auftrag gespeichert werden. Ist aber derzeit kein Problem, da nur der Scheduler seriell eine Instanz
    //dieser Klasse verwendet.
    ...
    @Override
    protected TryScheduleResult execute(SchedulingOrder so) {
      needsCapacities = so.getSchedulingData().isNeedsToAcquireCapacitiesOnNextScheduling(); //TODO n�tig?
      if( needsCapacities ) { 
  ...
  private class TimeConstraint extends Execution {
    ...
    private boolean needsToCheckTimeConstraint; //TODO unsch�n, dass in einer nicht-auftragsabh�ngigen Klasse tempor�r Daten 
    //zu einem Auftrag gespeichert werden. Ist aber derzeit kein Problem, da nur der Scheduler seriell eine Instanz
    //dieser Klasse verwendet.
    ...
    protected TryScheduleResult execute(SchedulingOrder so) {
      ...
      if( needsToCheckTimeConstraint ) {
        ...
        if( tr.isExecutable() ) {
           ...
           * - falls "soll sp�ter gescheduled werden" -> return "TimeConstraint + Remove" (oder TimeConstraint + Continue (falls einfach nochmal probiert werden soll))
  ...
  private class Batch extends Execution {
    ...
    protected TryScheduleResult execute(SchedulingOrder so) {
      ...
      if(marker == null){
        return null; //einfach �berspringen, sollte so eh nicht aufgerufen werden
      }
  ...
  private class Schedule extends Execution {
    ...
    @Override
    protected void undo(SchedulingOrder so) {
      //hier ist leider kein undo m�glich
    }
