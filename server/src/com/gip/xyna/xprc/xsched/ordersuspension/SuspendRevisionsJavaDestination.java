...
public class SuspendRevisionsJavaDestination extends JavaDestination {

  private static final long serialVersionUID = -1735590502272607246L;
  private static Logger logger = CentralFactoryLogging.getLogger(SuspendRevisionsJavaDestination.class);
  
  public static final String SUSPEND_REVISIONS_DESTINATION = "com.gip.xyna.SuspendRevisions";

  public SuspendRevisionsJavaDestination() {
    super(SUSPEND_REVISIONS_DESTINATION);
  }

  @Override
  public GeneralXynaObject exec(XynaOrderServerExtension xose, GeneralXynaObject input) throws XynaException {
    if (!(input instanceof SuspendRevisionsBean)) {
      throw new XPRC_INVALID_INPUT_PARAMETER_TYPE("1", SuspendRevisionsBean.class.getName(), input.getClass().getName());
    }

    final SuspendRevisionsBean bean = (SuspendRevisionsBean) input;
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("receiving suspendrevisions for revisions: " + bean.getRevisionsToSuspend() + " forcingInterrupt: "
          + bean.isInterruptingStuckOrders());
    }
    
    Collection<? extends XynaOrderServerExtension> relevantOrders = fep.getRunningProcessesFrom(bean.getRevisionsToSuspend());
    
    suspend( bean, relevantOrders );
     
    //TODO nochmal kontrollieren, dass fep.getRunningProcessesFrom( bean.getRevisionsToSuspend() ); nun leer ist?
    
    return bean;
  }

  private void suspend(SuspendRevisionsBean bean, Collection<? extends XynaOrderServerExtension> relevantOrders ) {
