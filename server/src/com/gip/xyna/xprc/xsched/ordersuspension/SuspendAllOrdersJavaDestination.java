...
public class SuspendAllOrdersJavaDestination extends JavaDestination {
  ...
  private Pair<Integer,Integer> suspendAll(boolean suspendForShutdown, long deploymentId) throws PersistenceLayerException, XNWH_OBJECT_NOT_FOUND_FOR_PRIMARY_KEY, XDEV_UNSUPPORTED_FEATURE {
    ...
    //warten bis Auftr�ge sicher den Scheduler verlassen haben und im ExecutionProcessor angekommen sind
    ...
    for( XynaOrderServerExtension xo : executionProcessor.getOrdersOfRunningProcesses() ) {
      if (suspendForShutdown) {
        //set a flag for it
        xo.setSuspendedOnShutdown(true); //FIXME wof�r?
      }
    ...
    if( suspendForShutdown ) {
      ...
      //TODO wie k�nnte das gebraucht werden?
  ...
  private void waitForUnreachableOrders(long deploymentId) {
    //Unreachable Orders sind hier eigentlich nur die Auftr�ge, die den Scheduler verlassen haben,
    //aber noch nicht im ExecutionProcessor angekommen sind. Diese Auftr�ge w�rden nicht entdeckt 
    //werden und daher w�hrend ihrer Ausf�hrung abgebrochen werden, ihr OrderBackup aber w�re noch 
    //auf dem vorigen Stand. Diese Auftr�ge w�rden dann nach dem Start der Factroy oder nach der 
    //OrderMigration teilweise doppelt ausgef�hrt werden.  
    //Hier wird nun der Auftragsz�hler aus dem DeploymentManagement daf�r zweckentfremdet.
    //Dieser Auftragsz�hler z�hlt auch noch Auftr�ge an anderen Stellen mit, dies sollte jedoch
    //eher positive Auswirkungen haben: So werden z.B. JavaDestinations noch komplett ausgef�hrt.
    
    //eigener Auftrag SuspendAllOrders wird mitgez�hlt, dieser soll jetzt jedoch ignoriert werden
    DeploymentManagement.getInstance().countDownOrderThatKnowsAboutDeployment(deploymentId);
    try {
      
      //DeploymentId erh�hen, damit waitForUnreachableOrders() bisherige Auftr�ge als alt ansieht und darauf wartet
      DeploymentManagement.getInstance().propagateDeployment(); //TODO anderes waitForUnreachableOrders
      try {
        //auf alle versteckten Auftr�ge warten
      ...
      //eigenen Auftrag SuspendAllOrders wieder mitz�hlen
