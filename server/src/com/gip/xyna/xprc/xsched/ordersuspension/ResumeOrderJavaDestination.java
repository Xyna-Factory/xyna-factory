...
public class ResumeOrderJavaDestination extends JavaDestination {
   ...
   *
   * optimierung von
   * - gleichzeitiges halten von ressourcen m�glichst kurz
   * - und m�glichst die ressourcen k�rzer, die viel contention haben
   * 
   * die notwendigkeit, mehrere datenbank-operationen in der gleichen transaktion durchzuf�hren, ist hier nicht gegeben.
   * es muss nur schutz vor folgenden problemen gegeben werden:
   * a) falls resume fehlschl�gt, muss resume retry machen
   * b) falls resume erfolgreich, muss resumter auftrag sicheren ort erreichen, damit der resumeauftrag ansich verschwinden kann
   * beides muss nicht gekoppelt werden - zuviele resumes sind nicht sch�dlich 
   * 
   */
  ...
  public GeneralXynaObject exec(final XynaOrderServerExtension xose, GeneralXynaObject input)
    ...
    try {
       ...
       * Wenn also das Binding �ber das OrderBackup nicht ermittelt werden kann, sollte man noch nach einem SRInformation-Eintrag oder einem Memory-Backup schauen.
       * Falls beides nicht gefunden wird, dann auf dem anderen Knoten delegieren.
       * Flag �bergeben, so dass auf dem anderen Knoten nicht erneut zur�ckdelegiert werden kann (Nur f�r den Fall eines Bugs)
       * Es gibt dabei noch die M�glichkeit einer Racecondition, wenn das Suspend gleichzeitig mit den Checks passiert. 
       * => Deshalb beim Check nach SRInformation und Memory-Backup aufpassen.
       * 
       * => Der Check wird dadurch durchgef�hrt, dass man einfach lokal das Resume probiert.
       * Wenn es fehlschl�gt, weil der Auftrag nicht im Backup gefunden wird (dies inkludiert B1a und B2a), dann auf den anderen Knoten delegieren
       */
  ...
  private Integer getForeignBinding(final ResumeOrderBean bean) throws PersistenceLayerException {
    //TODO fastpath f�r "nicht im cluster, muss nichts �berpr�fen". das kann dann gecached werden. achtung: wann cacherefresh?
            ...
            if (oib == null) {
              ...
              if (oib.getBinding() == ownBinding) {
                ...
                try {
                  ...
                  throw new OrderDeathException(e); //resumeorder bleibt im orderbackup und wird beim n�chsten serverstart erneut ausgef�hrt
                }
              }
            }
          }
  ...
  /**
   * Falls eine Migration gerade l�uft, wird gepr�ft, ob diese schon eine DB-Connection hat. Zur Vermeidung von Deadlocks
    ...
    do {
      ...
      if (waitAgain) {
        try {
          ...
          // die ignorieren wir mal ... ansonsten w�rde der Resumeauftrag verloren gehen und der zugeh�rige Auftrag w�r f�r immer verloren        
        }
      }
    } while (waitAgain);
  }
  ...
  private static class ResumeOrder {
        ...
        case Failed :
          if (SuspendResumeManagement.FAILED_ORDERBACKUP_NOT_FOUND.equals(result.getSecond())) {
            if (bean.mayDelegateToOtherNodeIfOrderIsNotFound() && new OrderInstanceBackup().isClustered(ODSConnectionType.DEFAULT)) {
              ...
              //unerwarteter fall. hier kann man den auftrag killen/aufr�umen, er kann ja nicht wieder resumed werden. 
    ...
    /**
     * Resume konnte nicht ausgef�hrt werden, daher mit etwas Verz�gerung nochmal probieren, 
