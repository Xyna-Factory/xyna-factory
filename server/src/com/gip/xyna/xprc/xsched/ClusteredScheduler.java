...
public class ClusteredScheduler extends XynaScheduler implements Clustered, ClusteredSchedulerRemoteInterface {
  ...
  //FIXME eigtl sollte die abh�ngigkeit zu XynaScheduler. siehe xynafactory-FIXME
  static {
  ...
  private class RMIClusterStateChangeHandler implements ClusterStateChangeHandler {
    ...
    private void changeToDisconnected(ClusterState lastState, ClusterState newState ) {
        //abh�ngigkeit zu capacities und vetos tabelle: diese sollen jetzt
        //nicht mehr geclustered vergeben werden, d.h. ein knoten soll in godmode �bergehen
        ...
        if( lastState == ClusterState.STARTING ) {
          ...
          nscp.run(); //sofortigen �bergang veranlassen
        }

        if (newState.in(ClusterState.DISCONNECTED_SLAVE)) {
          //in diesem Zustand sollen keine Auftr�ge mehr laufen, damit jedoch notwendige Auftr�ge
          //vor dem Shutdown ausgef�hrt werden, wird das Schdeuling nicht komplett unterbunden
    ...
    private void changeToConnected(ClusterState lastState, ClusterState newState) {
      if( lastState.in(ClusterState.DISCONNECTED_SLAVE)) {
        //R�ckkehr zum normalen Scheduling
        resumeScheduling();
      }
    }
  ...
  public void notifyRemoteScheduler() {
    //TODO den lazyalgoexecutor noch verbessern, damit erst der n�chste remoterequest gemacht wird, wenn der
    //remote scheduler tats�chlich nicht mehr l�uft. => "lazyremotealgorithmexecutor"
      ...
      if( ple == null ) {
        if( asyncFailedException == null ) {
          if( result != null && result.getFirst() == ResumeResult.Unresumeable && SuspendResumeManagement.UNRESUMABLE_LOCKED.equals(result.getSecond()) ) {
            //Dies ist kein ungew�hnlicher Zustand: z.B. Resume kam recht schnell, bevor Suspend fertig ist.
    ...
    if (clusterState == ClusterState.CONNECTED) {
      try {
        ...
        for(Boolean result : results) {
          if(result) {
            return true; //ein anderer Knoten hat das Resume ausf�hren k�nnen
          }
        }
