...
public class OrderSeriesManagement extends FunctionGroup implements OSMInterface, Clustered {
  ...
  private ArrayBlockingQueue<OSMTask> externalQueue; //externe Queue f�r Task, die neu ins OSM eingestellt 
                                                     //werden. Blockierend als �berlastschutz
  private Queue<OSMTask> internalQueue; //interne Queue f�r Tasks, die w�hrend der Taskbearbeitung anfallen
  ...
  private Map<Long,Triple<String,OrderState,List<String>>> readyOrders; //Auftr�ge, die sofort in den Scheduler eingestellt werden k�nnen 
  ...
  private void initClusterStatusKnown() {
    //ClusterStatus ist nun bekannt, daher k�nnen die restlichen Initialisierungen durchgef�hrt werden.
    ...
    if( rmiClustered && storableClustered ) {
      //OrderSeriesManagement ist geclustert: jetzt bereits starten, keinen �bergang nach Connected abwarten
    } else {
      if( !rmiClustered && !storableClustered ) {
        //OrderSeriesManagement ist nicht geclustert
      } else {
        //nur einer der beiden ist geclustert; dies ist ein schwerer Fehler der zum Abbruch f�hren muss.
  ...
  /**
   * Auftragseingang: Mehrere Threads k�nnen gleichzeitig Auftr�ge einstellen und 
   * diese Methode aufrufen.<br>
   * Durch das Einstellen in die Queue hier wird die Parallelit�t im OSM verringert.
   * Der Insert und Commit des SeriesInformationStorable erfolgt noch in dem akktuellen Thread.
   * Dieser erh�lt daher auch die dabei m�glichen Fehler.
   * 
   * @param xo
  ...
  /**
   * Einstellen nach der OrderMigration: OSMTask_Resume muss ausgef�hrt werden
   * Achtung: Task darf erst nach dem Commit auf der �bergebenen Connection laufen,
   * da sonst ben�tigte Daten aus dem OrderArchive nicht sichtbar sind. 
   * @param xo
   * @param con 
   */
  ...
  /**
   * Auftragsausgang: Mehrere Threads bearbeiten die Auftr�ge und k�nnen diese
  ...
  public void finishOrder(XynaOrderServerExtension xo) {
    if (xo.isTransientFlagSet(TransientFlags.DuplicateSeriesCorrelationId)) {
      ...
      //finish nicht ausgef�hrt werden, da sonst die SeriesInformation eines anderen
      //Auftrags �berschrieben wird
  ...
  public void abortOrder(long xynaOrderId) {
    ...
    switch(result.getType()) {
      ...
      case Found:
        ...
        break ;
      case OtherBinding:
        // FIXME anderen Knoten benachrichtigen ... keine gute L�sung!!!
        putInExternalQueue( OSMTask.abort(result.getCorrelationId()) );
   ...
   * OrderState f�r {@link #addWaitingOrder(SchedulingOrder) addOrder }.
  ...
  public void readyToRun(String correlationId, long id, OrderState orderState, List<String> cycle ) {
    ...
    try {
      ...
      if( so != null ) {
        //Auftrag stand in waitingOrders, nun Orderstate pr�fen
  ...
  /**
   * Eintragen der neuen SchedulingOrder in die Liste aller Auftr�ge. Falls kein OrderState in readyOrders
   * bekannt ist, wird die SchedulingOrder f�r {@link #readyToRun(String, long, com.gip.xyna.xprc.xsched.orderseries.OSMInterface.OrderState, List) readyToRun} gespeichert.
   * @param so
   */
  public void addWaitingOrder(SchedulingOrder so) {
    ...
    try {
      ...
      if( triple != null ) {
        //Auftrag stand in readyOrders, nun OrderState pr�fen
  ...
  private boolean canBeStarted(SchedulingOrder so, String correlationId, OrderState orderState, List<String> cycle) {
    ...
    switch( orderState ) {
      ...
      case HasCyclicDependencies:
        //Auftrag muss wegen zyklischer Predecessor-Abh�ngigkeit abgebrochen werden
      ...
      case HasToBeCanceled:
        ...
        return true;
      case NotFound:
        //sollte hier nicht m�glich sein
      ...
      case AlreadyFinished: 
        //Eigentlich ist hier nichts zu tun. //TODO aber es k�nnte versucht werden, das AlreadyFinished
        //hier nachzuvollziehen: Eintr�ge aus Listen entfernen
    }
    return false;
  }
    ...
    public WaitingOrder transform(XynaOrderInfo from) {
      ...
      if( mode == Mode.Predecessors ) {
        ...
        if( preTree != null ) {
          ...
          boolean set = false;
          while( ! set ) {
            try { //ungesch�tzter Zugriff auf nicht-synchronisierte Map. Gleichzeitiger Zugriff ist unwahrscheinlich
   ...
   * Sollte nur von OSMTaks_reschedule in Notf�llen gerufen werden.
   * @param orderId
   */
  ...
  /**
   * Zerlegen der OrderSerien, wenn diese im alten Format (Huckepack-Auftr�ge) vorliegen 
   * @param xo
   * @param backupCon
  ...
  public void separateSeries(XynaOrderServerExtension xo, ODSConnection backupCon, boolean hasToBeAcknowledged) {    
    ...
    //Einstellen der Serien-Auftr�ge in XynaProcessCtrlExecution: 
    //Durchf�hrung von Planning und Prescheduling, bis XynaScheduler.addOrder fertig ist.
    ...
    if( ! successful ) {
      //Auftr�ge sind nicht erfolgreich durch das Planning gelaufen
      //evtl. loggen
    }
    
    //f�r den Basis-Auftrag muss das alte Acknowledge verwendet werden
  ...
  public RescheduleSeriesOrderInformation rescheduleSeriesOrder(long orderId, boolean force) {
    if( orderId == 0 ) {
      ...
      try {
        ...
        //ignorieren, Info ist dann halt evtl. nur teilweise gef�llt 
      }
      return task.getInfo();
    }
    ...
    if( task.canBeStarted() ) { //vom Suchergebnis ist abh�ngig, ob ein Task sinnvoll gestartet werden kann
      putInExternalQueue( task );
      try {
        task.await(); //auf Bearbeitung des Tasks warten
      } catch( InterruptedException e ) {
