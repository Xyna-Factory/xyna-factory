...
@XynaObjectAnnotation(fqXmlName = "base.date.Date")
public abstract class Date extends XynaObject {

  private static final long serialVersionUID = -1L;
  private static final Logger logger = CentralFactoryLogging.getLogger(Date.class);


  protected static class InternalBuilder<_GEN_DOM_TYPE extends Date, _GEN_BUILDER_TYPE extends InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>>  {

    protected _GEN_DOM_TYPE instance;

    protected InternalBuilder(Date instance) {
  ...
  protected void fillVars(Date source, boolean deep) {
  }

  public abstract Date clone();

  public abstract Date clone(boolean deep);
  ...
  public String toXml(String varName, boolean onlyContent, long version, com.gip.xyna.xdev.xfractmod.xmdm.GeneralXynaObject.XMLReferenceCache cache) {
    ...
    if (!onlyContent) {
      ...
      XMLHelper.beginType(xml, varName, "Date", "base.date", objectId, refId);
    } else {
      objectId = -1;
    }
    if (!onlyContent) {
