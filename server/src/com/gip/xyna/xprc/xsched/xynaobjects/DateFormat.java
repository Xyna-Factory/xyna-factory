...
@XynaObjectAnnotation(fqXmlName = "base.date.DateFormat")
public abstract class DateFormat extends XynaObject {

  private static final long serialVersionUID = -1L;
  private static final Logger logger = CentralFactoryLogging.getLogger(DateFormat.class);


  protected static class InternalBuilder<_GEN_DOM_TYPE extends DateFormat, _GEN_BUILDER_TYPE extends InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>>  {

    protected _GEN_DOM_TYPE instance;

    protected InternalBuilder(DateFormat instance) {
      this.instance = (_GEN_DOM_TYPE) instance;
    }

    public DateFormat instance() {
  ...
  protected void fillVars(DateFormat source, boolean deep) {
  }

  public abstract DateFormat clone();

  public abstract DateFormat clone(boolean deep);
  ...
  public String toXml(String varName, boolean onlyContent, long version, com.gip.xyna.xdev.xfractmod.xmdm.GeneralXynaObject.XMLReferenceCache cache) {
    ...
    if (!onlyContent) {
      ...
      XMLHelper.beginType(xml, varName, "DateFormat", "base.date", objectId, refId);
    } else {
      objectId = -1;
    }
    if (!onlyContent) {
