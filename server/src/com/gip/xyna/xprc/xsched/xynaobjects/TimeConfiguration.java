...
@XynaObjectAnnotation(fqXmlName = "xprc.xsched.TimeConfiguration")
public abstract class TimeConfiguration extends XynaObject {

  private static final long serialVersionUID = 1L;
  private static final Logger logger = CentralFactoryLogging.getLogger(TimeConfiguration.class);


  protected static class InternalBuilder<_GEN_DOM_TYPE extends TimeConfiguration, _GEN_BUILDER_TYPE extends InternalBuilder<_GEN_DOM_TYPE, _GEN_BUILDER_TYPE>>  {

    protected _GEN_DOM_TYPE instance;

    protected InternalBuilder(TimeConfiguration instance) {
      this.instance = (_GEN_DOM_TYPE) instance;
    }

    public TimeConfiguration instance() {
      return (TimeConfiguration) instance;
    }
  ...
  protected void fillVars(TimeConfiguration source, boolean deep) {
  }

  public abstract TimeConfiguration clone();

  public abstract TimeConfiguration clone(boolean deep);
  ...
  public String toXml(String varName, boolean onlyContent, long version, com.gip.xyna.xdev.xfractmod.xmdm.GeneralXynaObject.XMLReferenceCache cache) {
    ...
    if (!onlyContent) {
      ...
      XMLHelper.beginType(xml, varName, "TimeConfiguration", "xprc.xsched", objectId, refId);
    } else {
      objectId = -1;
    }
    if (!onlyContent) {
  ...
  */
  ...
  public abstract AbsRelTime toAbsRelTime();

  public static TimeConfiguration fromAbsRelTime(AbsRelTime absRelTime) {
    if( absRelTime == null ) {
      return null;
    }
    if( absRelTime.isAbsolute() ) {
      return AbsoluteTimeConfiguration.fromMillis(absRelTime.getTime());
    } else {
      return RelativeTimeConfiguration.fromMillis(absRelTime.getTime(), true);
    }
  }
}
