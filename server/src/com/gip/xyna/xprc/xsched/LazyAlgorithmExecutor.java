...
/**
 * F�hrt einen eventuell langlaufenden Algorithmus in einem eigenen Thread solange nacheinander aus, bis
 * keine Requests mehr kommen, dass er ausgef�hrt werden soll. Dann schl�ft der Thread, bis neue Requests
 * kommen.
 * Der Algorithmus wird nicht so oft ausgef�hrt wie die Anzahl der Requests, sondern die Ausf�hrung ist zeitlich 
 * an die Requests gebunden.
 * D.h. nach jedem Request l�uft der Algorithmus mindestens einmal vollst�ndig durch, aber nicht unbedingt zweimal.
 * 
 * Der Algorithmus kann zur Laufzeit ausgetauscht werden, was dazu f�hrt, dass baldm�glichst zum anderen Algorithmus gewechselt wird.
 * 
 * Man kann konfigurieren, dass der Algorithmus periodisch aufgerufen wird, auch wenn kein normaler Request ankommt.
 * 
 * Es wird nicht garantiert, dass der Algorithm nicht l�uft, wenn keine Requests kommen (vgl spurious wakeups. k�nnte man �ndern...)
 */
public class LazyAlgorithmExecutor<ALG extends Algorithm> implements Runnable {
  ...
  /**
   * stoppt thread gracefully, d.h. er l�uft seine jetzige schleife zuende und beendet sich danach.
   */
  ...
  /**
   * thread schl�ft solange bis er durch unPause aufgeweckt wird
   */
  ...
  /**
   * ausf�hrung fordern. ausf�hrung sofern nicht bereits laufend.
   * @return true, falls ausf�hrung direkt beginnt. false falls nicht, weil z.B. noch laufend.
   */
  ...
  /**
   * Achtung: kehrt evtl. nach einer bereits laufenden Ausf�hrung zur�ck
   ...
   * kann nur einmal gestartet werden. falls der thread noch existiert, macht dieser aufruf nichts.
   * falls algorithm null ist, wird versucht der alte algorithm zu benutzen. falls keiner vorhanden, wird ein fehler geworfen.
   * Setzt Runnable initialization, der dirket nach dem Start des Thread einmal ausgef�hrt wird
  ...
  /**
   * falls der alte algorithmus noch am laufen ist, wird er noch fertig ausgef�hrt und erst der n�chste durchlauf
    ...
    if (algorithm != this.algorithm) {
      ...
      synchronized (sleepLock) {
        if (threadIsAsleep && !isPaused) {
          ...
          //entweder am laufen => ok, wird unterbrochen und l�uft dann mit neuem gleich nochmal
          //oder kurz vorher => l�uft mit neuem algorithmus los und wird dann unterbrochen, geht aber nicht schlafen und l�uft dann nochmal
          //oder danach => l�uft direkt nochmal mit neuem algorithmus, wird unterbrochen, und geht dann nochmal nicht schlafen
  ...
  private void loopAsLongAsRequested() {
    ...
    while( notifiedCount.getAndSet(0) > 0 && !algorithmChangeRequested ) {
      ...
      notifiedCount.getAndDecrement(); //runtersetzen, damit keine weitere Ausf�hrung passiert ausser requestExecution() gerufen
    }
