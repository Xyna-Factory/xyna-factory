...
public class CMClustered extends CMAbstract implements ClusteredCapacityManagementInterface, CapacityManagementReservationInterface {
  ...
  public static CMClustered createCMClustered(ODS ods, CapacityCache cache, int ownBinding, 
    ...
    if (!XynaFactory.getInstance().isStartingUp()) {
      scheduler.notifyScheduler(); //noch ein notify, damit Scheduler auf jedem Fall einmal mit neuen CapacityReservation l�uft
    }
    
    //Wie ist es mit Remote-Aufrufen, wenn CapacityReservation noch nicht im Scheduler ausgetauscht wurde?
    //Nur communicateLocalDemand(...) relevant, alle anderen RMI-Methoden �ndern nur Capacities
    //communicateLocalDemand tr�gt bereits in neue CapacityReservation ein, so dass die Demands nicht verloren gehen.
    return cmc;
  }
  ...
  /**
   * Schlie�en der RMI-Verbindung: Abmelden der RMIInterface-Implementierung
   */
  ...
  protected int transportReservedCaps(List<Reservations> reservations) {
    //Die Reservierungen wurden ohne ManagementLock ermittelt und an diese Methode �bergeben.
    //Daher kann es sein, dass in der Zwischenzeit �nderungen in der DB erfolgt sind, die zu
    //den �bergebenen Reservierungen nicht kompatibel sind
  ...
  protected void increaseCaps(ODSConnection defCon, CapacityStorables allCs,
    ...
    //addCard Capacities werden hinzugef�gt, dies ist immer m�glich.
    ...
    //c) gleichm��ig vergeben -> am einfachsten, deswegen verwendet
  ...
  protected void decreaseCaps(ODSConnection defCon, CapacityStorables allCs,
    ...
    //removeCard Capacities sollen entfernt werden, dies ist unter Umst�nden nicht m�glich
    //hier werden 3 F�lle unterschieden, alle erfordern einen anderen Algorithmus
  ...
  private void decreaseCapsActive(ODSConnection defCon, CapacityStorables allCs,
    ...
    //Alle Knoten m�ssen alle ihre Capacities des Typs capName abgeben, dann wird dies lokal ausgeglichen
    //defCon.persistCollection(allCs); //SelectForUpdate-Lock zur�ckgeben
    if( logger.isDebugEnabled() ) {
      logger.debug("current capacities: "+allCs);
    }
    defCon.rollback(); //SelectForUpdate-Lock zur�ckgeben
    ...
    try {
      ...
      //In jedem Fall die Capacity-Verteilung aktualisieren, da sonst Capacities im Cache fehlen w�rden
    ...
    if( free >= removeCard ) {
      //ok, es sind gen�gend freigeworden
  ...
  public void moveAllFreeCapacitiesToBinding(String capName, int binding) throws RemoteException {
    //alle freien Capacities m�ssen abgegeben werden
    cacheLock.lock();
    try {
  ...
  private void decreaseCapsDisabled(ODSConnection defCon, CapacityStorables allCs,
    ...
    //ok, es gibt gen�gend unbenutzte Capacities, diese nun entfernen
    ...
    while( toRemove > 0 ) {
      for( int r =0; r < size; ++r ) {
        if( maxFree <= free[r] ) { //Vergabeschwelle ber�cksichtigen
          if( remCard[r] < free[r] ) { //sind noch Caps frei?
            ++remCard[r]; //Cap verringern
            --toRemove;
            if( toRemove == 0 ) {
              break; //gen�gend Caps gefunden
            }
          }
        }
      }
  ...
  public CapacityInformation getCapacityInformation(String capacityName) throws XPRC_ClusterStateChangedException {
    ...
    try {
      ...
      managementLock.unlock(); //throws AlreadyUnlockedException ok, da Aufruf �ber CLI
    }
  }
