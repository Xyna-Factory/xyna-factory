...
public class XynaOrder implements Serializable {
  ...
  private SeriesInformation seriesInformation; 
    ...
    //m�ssen. Dies ist derzeit noch der Fall beim Lesen aus OrderInstanceBackup
  ...
  /**
   * bedeutung der priorit�t siehe {@link Thread#MIN_PRIORITY}, {@link Thread#MAX_PRIORITY}
   * @return
   */
  ...
  //FIXME in monitoringlevel umbenennen wenn mal abw�rtskompatibilit�t nicht so wichtig ist
  ...
  /**
   * absoluten zeitstempel setzen, bei �berschreitung dessen der auftrag fehlerhaft beendet wird FIXME: umbenennen in
   * setAbsoluteSchedulingTimeout, wenn wir mal auf abw�rtskompatibilit�t nicht soviel wert legen.
  ...
  /**
   * dieser auftrag soll status �nderungs listener benachrichtigen. dieser parameter wird gesetzt, falls ein listener
   * f�r diesen ordertype gesetzt ist.
   */
  ...
  /**
   * vorg�nger und nachfolger auftr�ge.
   */
  ...
  private void readObject(java.io.ObjectInputStream s) throws IOException, ClassNotFoundException {
    ...
    try {
      ...
      if (schedulingData == null) {
        //nur f�r abw�rtskompatibilit�t. alte auftragsobjekte haben kein schedulingData
  ...
  public String getLoggingDiagnosisContext(boolean logOrderId) {
    if( getOrderContext() == null || getOrderContext().getLoggingDiagnosisContext() == null ) {
      if( logOrderId ) {
        ...
        return null; //kein Log4jDiagContext gew�nscht
      }
    } else {
      if( logOrderId ) {
