...
public class BatchProcessScheduling {
  ...
  private SchedulingData slaveSchedulingData; //SchedulingData. mit denen der Master die Capacities f�r die Slaves besorgt
  ...
  public enum SchedulingState {
   ...
   Init,
     ...
     * auf die Scheduling Data der Slaves gewechselt und der Status geht nach ScheduleSlaves �ber.
     */
    ScheduleMaster,
     ...
     * belegten Kapazit�ten abgeholt haben
     */
    Ignore,
    
    /**
     * Der Batch Process soll beim n�chsten Scheduling aus dem Scheduler entfernt werden.
  ...
  private enum For {
    Slave,    //Master->Slave
    Refresh,  //Slave->Slave, prio,caps,timeconstraint k�nnen sich �ndern
    FairnessOrRate; //Slave->Slave, timeconstraint kann sich �ndern, entranceTimestamp �ndert sich auf jeden fall
  }
   ...
   * zur�ckgewechselt und der Master wird aus dem Scheduler entfernt, wenn noch nicht alle
   * Slaves beendet sind.
   * @param so
   * @param isMaster
   * @return 
  ...
  public TryScheduleResult tryScheduleBatch(SchedulingOrder so, boolean isMaster) {
    if( schedulingState.get() == SchedulingState.ReInit ) {
      //es ist zu fr�h, der BatchProcess ist noch nicht wiederhergestellt
    ...
    if (isMaster) {
      ...
      if (schedulingState.get() == SchedulingState.Remove) {
        ...
        if (allSlavesFinished && schedulingState.compareAndSet(SchedulingState.Remove, SchedulingState.ScheduleForExecution)){
          ...
          return TryScheduleResult.CONTINUE;
        //k�nnte inzwischen schon den Status ScheduleForExecution haben, dann nicht entfernen
      ...
      //evtl. m�ssen die SchedulingData der Slaves aktualisiert werden
      ...
      //Slave geht nach Scheduling �ber
      batchProcess.setSlaveState(so.getOrderId(), OrderInstanceStatus.SCHEDULING);
    }
    if (logger.isTraceEnabled()) {
  ...
  public TryScheduleResult trySchedule(SchedulingOrder so, boolean isCancelled) {
    if (so.getBatchProcessMarker().isBatchProcessMaster()) {
      if (!isCancelled && schedulingState.compareAndSet(SchedulingState.ScheduleMaster, SchedulingState.ScheduleSlaves)) {
        //Slaves m�ssen mit eigenen SchedulingData gescheduled werden 
      ...
      if (schedulingState.get() == SchedulingState.ScheduleSlaves) {
        //es m�ssen noch weitere Slaves gestartet werden
    ...
    while (true) {
      ...
      switch (state) {
        ...
        case Remove :
          ...
          //beim n�chsten schedulingdurchlauf geht es nach "ExecuteMaster" (vgl tryScheduleBatch())
          
          if (masterSchedulingOrder.isMarkedAsTimedout()) {
            batchProcess.setTimeoutTransiently();
          }
          schedulerMasterWith(masterSchedulingData);
          //master unabh�ngig von zeitfenster starten
  ...
  private SchedulingData slaveSchedulingData(For reason) {
    ...
    //Eventuelle �nderungen aus slaveOrderTypeInfo nachtragen
    ...
    switch( reason ) {
      case Slave:
        ...
        break;
      case Refresh:
        //obige �nderungen reichen
        break;
      case FairnessOrRate:
        slaveSchedulingData.setTimeConstraint( createSlaveTimeConstraintFor(reason) );
        //EntranceTimestamp �ndern, so dass auch bei TimeConstraint.immediately() die StartTime ge�ndert wird.
  ...
  /**
   * Legt die SchedulingData f�r einen Slave an. Die SchedulingData f�r den Master
   * m�ssen schon gesetzt sein, da hieraus Daten �bernommen werden.
   * @return
   */
  ...
  /**
   * Bestimmt die Kapazit�ten, die die Slaves ben�tigen.
   * @return
  ...
  private MultiAllocationCapacities getMultiAllocationCapacities() {
    ...
    //es sollen maximal so viele Kapazit�ten allokiert werden, wie Slaves parallel
    //gestartet werden d�rfen
  ...
  /**
   * Gibt alle transferierbaren Kapazit�ten frei
   */
  ...
  public boolean canBeCanceled() {
    return ! schedulingState.isIn( SchedulingState.ScheduleForExecution, //wartet noch auf letztes Scheduling
                                   SchedulingState.ExecuteMaster ); //Master wird bereits ausgef�hrt
  }
  ...
  public boolean cancel() {
    if (schedulingState.compareAndSet(SchedulingState.ScheduleMaster, SchedulingState.ExecuteMaster)) {
      //Wenn der BatchProcess noch nicht dabei ist Slaves zu starten, soll er direkt in die Execution-Phase �bergehen
      new AllSlavesStartedRunnable().run(); //so tun, als ob alle Slaves gestartet worden w�ren
      if (logger.isTraceEnabled()) {
      ...
      //es k�nnte laufende Slaves geben, dies pr�fen
  ...
  public void transferCapacities(XynaOrderServerExtension xo, TransferCapacities transferCapacities) {
    ...
    try {
      if (!getXynaScheduler().getCapacityManagement().transferCapacities(xo, transferCapacities)) {
        //unexpected. kann das auftreten, wenn jemand im richtigen moment kapazit�tskardinalit�ten �ndert?
    ...
    if (!inputGeneratorHasNext) { //es gibt keine Input-Daten mehr
      ...
      if( schedulingState.compareAndSet(SchedulingState.ScheduleSlaves, SchedulingState.Ignore) ) {
        ...
        if (batchProcess.getStarted() == 0) {
          //es sind �berhaupt keine Input-Daten vorhanden, dadurch werden keine Slaves
  ...
  /**
   * Dieses Runnable wird ausgef�hrt, wenn alle Slaves gestartet wurden
   */
  private class AllSlavesStartedRunnable implements Runnable {
    public void run() {
      //wenn alle Slaves gestartet wurden, m�ssen die vom Master bereits belegten Kapazit�ten
      //wieder freigeben werden und der BatchProcess aus dem Scheduler entfernt werden    
      
      //es kann bereits rescheduleMaster (durch SlaveResponseListener) aufgerufen worden sein, dann passiert hier kein zustands�bergang
  ...
  /**
   * Gibt die restlichen f�r Slaves reservierten Capacities frei und stellt den Master wieder
   ...
   * @param executeMaster true, wenn der Master in die Execution-Phase �bergehen soll
   */
  public void rescheduleMaster(boolean executeMaster, boolean ignoreTimeConstraintsOfMaster) {
    ...
    /*
     * falls Master bereits ausgetimed ist, ist ScheduleMaster am sinnvollsten, damit er dann im tryScheduleMasterFailed den Timeout l�scht
     *   (falls man direkt nach ExecuteMaster gehen w�rde, w�rde der Timeout durchschlagen und damit der Master-Workflow nicht mehr durchgef�hrt werden)
     * falls Master gecancelt ist, ist wird oben im tryschedule bemerkt, dass nicht wieder slaves gescheduled werden
     * falls Master keinen Fehler hat, geht er wieder automatisch �ber ScheduleSlave korrekt weiter
     */
  ...
  /**
   * Setzt das Flag, das beim n�chsten Scheduling die Slaves neue SchedulingData brauchen
  ...
  public TimeConstraint changeMasterTimeConstraint(TimeConstraint plannedTC, TimeWindowDefinition newTWD ) {
    ...
    if( oldTC.toString().equals(newTC.toString()) ) {
      ...
      //im XynaScheduler reschedulen, damit �nderungen wirksam werden, wenn Auftrag bereits wartet
  ...
  public TimeConstraint createTimeWindow(Long batchProcessId, TimeConstraint timeContraint, TimeWindowDefinition timeWindowDefinition) {
    ...
    if( timeWindowDefinition == null ) {
      //nichts zu tun; TimeConstraint bleibt unver�ndert
      if( timeContraint == null ) {
        return TimeConstraint.immediately(); //ist �quivalent
      } else {
    ...
    try {
      ...
      //wird nicht remote ben�tigt, daher nicht schlimm
  ...
  public void removeTimeWindow(Long batchProcessId) {
    ...
    try {
      ...
      //unerwartet, da TimeWindows f�r BatchProcesse nicht gespeichert werden
  ...
  public void createParallelExecutorAndCheckLimitation(SlaveOrderTypeInfo slaveOrderTypeInfo, SlaveExecutionPeriod slaveExecutionPeriod, 
    ...
    //Limitierung pr�fen und speichern
  ...
  private SlaveExecutionPeriod checkLimitation(Long batchProcessId, SlaveOrderTypeInfo slaveOrderTypeInfo, SlaveExecutionPeriod slaveExecutionPeriod, Integer total) {
    ...
    if( hasCapacities || hasSlaveExecutionPeriod || hasSmallTotal ) {
      ...
      //BatchProcess ist nicht limitiert, dies kann zu Problemen f�hren (zu viel Last, BatchProcess kann abbrechen)
