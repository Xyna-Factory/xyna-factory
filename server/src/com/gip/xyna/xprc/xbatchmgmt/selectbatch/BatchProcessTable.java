...
public enum BatchProcessTable {
  //abgeschlossene Auftr�ge aus der Tabelle BatchProcessArchive
  Archive(BatchProcessArchiveStorable.TABLE_NAME, BatchProcessArchiveStorable.COL_ORDER_ID, BatchProcessArchiveStorable.idReader) {
    ...
    public String convertColumnName(String columnName) {
      if (columnName.equals(BatchProcessRuntimeInformationStorable.COL_RUNNING)) {
        return "0";  //running gibt es im Archive nicht als Spalte, aber f�r abgeschlossene w�re sie immer "0"
      }
  ...
  //laufende Auftr�ge aus der Tabelle BatchProcessArchive
  ...
  /**
   * Konvertiert den �bergebenen columName in den entsprechenden Spaltennamen der Tabelle
   * @param columnName
   * @return
   */
  public String convertColumnName(String columnName) {
