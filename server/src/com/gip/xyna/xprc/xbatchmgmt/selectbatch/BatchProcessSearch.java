...
public class BatchProcessSearch {
  ...
  /**
   * Sucht alle Batch Prozesse, die die �bergebenen Filterkriterien erf�llen und
  ...
  public BatchProcessSearchResult searchBatchProcesses(BatchProcessSelectImpl select, int maxRows) throws PersistenceLayerException {
    ...
    try {
      //die Ids der Batch Prozesse suchen, die die Filterkriterien erf�llen
      ...
      //auf maximale Anzahl beschr�nken
      ...
      //zu den BatchProcessIds die zugeh�rigen BatchProcessInformations holen
  ...
  /**
   * Liefert die Ids der Batch Prozesse, die die �bergebenen Filterkriterien erf�llen
   * @param con
   * @param select Filterkriterien
   * @return
   * @throws PersistenceLayerException
  ...
  private Set<Long> searchBatchProcessIds(ODSConnection con, BatchProcessSelectImpl select) throws PersistenceLayerException {
    ...
    //zun�chst werden alle laufenden Prozesse aus dem Archive gesucht, die die Filter-Kriterien erf�llen
    ...
    //zu den gefundenen laufenden Prozessen m�ssen nun noch die abgeschlossenen hinzugef�gt werden
    ...
    //gebildet werden, um die Filter-Kriterien aus der RestartInformation zu ber�cksichtigen
  ...
  /**
   * Liefert die BatchProcessInformations zu den �bergebenen BatchProcessIds (f�r laufende
   * aus dem Memory, f�r abgeschlossenen aus dem Warehouse)
   * @param con
  ...
  private List<BatchProcessInformation> getBatchProcessInformations(ODSConnection con, List<Long> batchProcessIds, BatchProcessSelectImpl select) throws PersistenceLayerException {
    ...
    for (Long batchProcessId : batchProcessIds) {
      ...
      if (batchProcess != null) {
        //Batch Process l�uft noch
        bpi = batchProcess.getBatchProcessInformation();
      } else {
        //der Process l�uft nicht mehr, daher Archive und RestartInformation
        ...
        try {
          ...
          // sollte eigentlich nicht auftreten, da aus dem Archive keine Daten gel�scht werden
