...
public class BatchProcessCustomizationStorable extends Storable<BatchProcessCustomizationStorable> {
  ...
  public static final String COL_COUNTER0 = "counter0"; //Erstes Custom-Feld, f�r die freie Verwendung durch Komponenten.
  ...
  private Double counter0; //Erstes Counter-Feld, f�r die freie Verwendung durch Komponenten.
    ...
    Add() {
      public Double exec(Double old, Double value) {
        if( old == null ) {
          ...
          if( value == null ) {
            return old; //keine �nderung: (1 + null => 1);
          } else {
            return new Double(old.doubleValue()+value.doubleValue()); //�nderung: (1 + 2 => 3)
          }
        }
      }
    };
