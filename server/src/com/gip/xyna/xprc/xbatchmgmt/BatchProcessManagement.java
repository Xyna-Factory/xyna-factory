...
public class BatchProcessManagement extends Section {
  ...
  public enum MissingLimitationReaction {
    SlowDown,     //Slaves werden �ber eine SlaveExecutionPeriod verz�gert
    ...
    public static String documentation(DocumentationLanguage lang) {
      switch( lang ) {
        case DE:
          return "'SlowDown': Slaves werden �ber eine SlaveExecutionPeriod verz�gert; "
  ...
  private class OnTheFlyInputGeneratorCacheClearer implements DeploymentHandler {
    ...
    public void finish(boolean success) throws XPRC_DeploymentHandlerException {
      //TODO das muss nicht f�r alle batchprocesses gemacht werden. optimierungsm�glichkeit
  ...
  /**
   * Schlie�t die Auftragseingangsschnittstelle der Batch Prozesse f�r eine Application,
  ...
  /**
   * �berpr�ft, ob die Auftragseingangsschnittstelle der Batch Prozesse f�r eine Application
  ...
  /**
   * Aktiviert die Auftragseingansschnittstelle der Batch Prozesse f�r eine Revision wieder,
   * indem alle Prozesse wieder fortgesetzt werden, die durch das Schlie�en der
   ...
   * Anschlie�end geht der Master in die Execution-Phase �ber.
   * @param batchProcessId
   * @param cancelMode
  ...
  public boolean cancelBatchProcess (Long batchProcessId, CancelMode cancelMode, long callerOrderId) throws PersistenceLayerException {
    ...
    //TODO falls true �bergabe weitergegeben werden soll, sollte man die schnittstelle hier nochmal �berdenken,
  ...
  /**
   * �ndert die Werte eines BatchProcesses, die im �bergebenen Input ungleich null sind.
   * @param batchProcessId
   * @param input
   * @throws PersistenceLayerException 
   */
  ...
  public boolean refreshBatchProcess (Long batchProcessId) {
    ...
    //TODO die Priorit�t des ParallelExecutors muss angepasst werden
  ...
  /**
   * Falls BatchProcess noch nicht angelegt wurde, m�ssen die Slaves, deren BatchProcess
   * fehlt, warten:
   * entweder bis der Master geschedult wurde und der BatchProcess angelegt wurde,
   * oder bis bekannt ist, dass der andere Knoten den BatchProcessMaster ausf�hrt. //FIXME nicht implementiert!
   ...
   * entweder Slaves erzeugen und TryScheduleResult zur�ckgeben
   * oder null zur�ckgeben, so dass Master in die Execution gelangt
   * @param so
   * @return
   */
   ...
   * au�erhalb des SchedulerThread durchgef�hrt werden kann. Der BatchProcess wird aber gleich sichtbar im
   * BatchProcessManagement, jedoch gesch�tzt durch das ManagementOperationLock.
   */
  public static class BatchProcessRestarter extends XynaRunnable {
    ...
    public BatchProcessRestarter(Long batchProcessId, SchedulingOrder master) {
      ...
      this.priority = master.getSchedulingData().getPriority(); //k�nnte Prio vom Slave sein
    }
  ...
  /**
   * F�hrt die Aufr�umarbeiten aus:
   * 1) Austragen aus laufenden BatchProcessen
   * 2) L�schen der Runtime-Storables und archivieren des BatchProcessArchive
   * 3) Entfernen des zugeh�rigen Zeitfensters
   * @param batchProcessId
   * @throws PersistenceLayerException
   */
  ...
  public void terminateSlaves(Long batchProcessId, long orderId, boolean success) {
    BatchProcess batchProcess = getBatchProcess(batchProcessId);
    if (batchProcess != null) { //BatchProcess k�nnte inzwischen abgebrochen worden sein
      try {
  ...
  /*
   * falls master noch am slave-scheduling ist, dieses unterbrechen und TryScheduleResult.ReSchedule zur�ckgeben
   * 
   * ansonsten null zur�ckgeben (-> master wird abgebrochen)
   */
