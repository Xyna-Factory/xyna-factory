...
public class XynaProcessing extends XynaProcessingBase {
  ...
  //TODO w�re sinnvoll? //public static final String FUTUREEXECUTIONID_ORDER_ENTRANCE = "OrderEntrance";
  ...
  public Long startOrder(XynaOrderCreationParameter xocp) {
    try {
      ...
      //wegen abw�rtskompatibilit�t wird eine derartige exception in eine runtimeexception gewrapped
  ...
  public CancelBean cancelOrder(Long id, Long timeout, boolean waitForTimeout) throws XPRC_CancelFailedException {
    ...
    // falls l�nger warten als die uhr z�hlen kann, warte f�r bis 5 sek vor uhr-ende
  ...
  private void startPersistedOrders() {
    // lade gespeicherte+pausierte Auftr�ge im eigenen Thread
  ...
  /**
   * 1. cronls anhalten 2. trigger, rmi anhalten 3. mi bearbeitung deaktivieren 4. scheduler anhalten (pause-auftr�ge
   * m�ssen noch laufen d�rfen!) 5. auftr�ge die bald einen timeout haben mit fehler beantworten 6. 7. pause signal an
   * laufende auftr�ge senden (timeout) 8. warten bis keine auftr�ge mehr laufen 9. warten bis von triggern keine
   * auftr�ge mehr im planning sind oder dahin kommen k�nnen 10. �berpr�fen, dass responselistener-anzahl = wartender +
   * pausierte auftr�ge 11. responselistener deregistrieren 12. pausierte und wartende auftr�ge persistieren 13.
  ...
  public void stopGracefully() {
    ...
    // 3. pause signal an laufende auftr�ge senden und auf timeouts warten
    try {
    ...
    // warten bis von triggern keine auftr�ge mehr im planning sind oder dahin kommen k�nnen
    waitForActiveThreads( false, XynaProperty.TIMEOUT_SHUTDOWN_ORDERS_IN_PLANNING, "planning");

    // warten bis keine Auftr�ge mehr in Execution oder Planning sind
    ...
    if (ordersForPersistence.size() > 0) {
      ...
      try {
        ...
        while (iter.hasNext()) {
          ...
          // FIXME counter hochz�hlen und alle 1000 (?) Auftr�ge die IDs rausschreiben, sonst wird der StringBuilder zu gro�
        }
    ...
    // backup f�r orderinstancedetails erstellen
