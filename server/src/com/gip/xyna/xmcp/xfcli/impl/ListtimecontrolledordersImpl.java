...
import com.gip.xyna.xmcp.xfcli.generated.Listtimecontrolledorders;
...
public class ListtimecontrolledordersImpl extends XynaCommandImplementation<Listtimecontrolledorders> {

  public void execute(OutputStream statusOutputStream, Listtimecontrolledorders payload) throws XynaException {
    if (payload.getOnlymemory()) {
      ...
      if (ordersInMemory.size() == 0) {
        writeLineToCommandLine(statusOutputStream, "No time controlled orders in memory.");
      } else {
        writeLineToCommandLine(statusOutputStream, ordersInMemory.size(), " time controlled orders in memory:");
    ...
    if (iter.hasNext()) {
      writeLineToCommandLine(statusOutputStream, "Found information on ", cronLikeOrders.size(), " time controlled order"
        ...
        writeLineToCommandLine(statusOutputStream,
                               String
                                   .format("%10s  %30s  %30s  %20s  %25s  %20s  %25s  %12s  %7s  %26s  %28s  %12s  %12s  %12s",
                                           ...
                                           "behaviour in case of error", "status", "application", "versionName", "workspaceName"));
      }
    } else {
      writeLineToCommandLine(statusOutputStream, "No time controlled orders found.");
    }
    ...
    while (iter.hasNext()) {
      ...
      if (payload.getAsTable()) {
        ...
        if (next.getValue().getStatus().equals(CronLikeOrderStatus.ERROR)) {
          output =
              String.format("%10d  %30s  %30s  %20s  %25s  %20s  %25s  %12b  %7b  %26s  %28s (%s)  %12s  %12s  %12s", next
                                ...
                                .getValue().isSingleExecution()) ? "single execution" : "calendar definition "
                                + next.getValue().getCalendarDefinition(),
        ...
        //id
        StringBuilder output = new StringBuilder("Time controlled order found with ID ").append(next.getKey()).append(": ");
        //orderType
        ...
        if (next.getValue().getInterval() != null) {
          if (next.getValue().isSingleExecution()) {
            output.append("', single execution '");
          } else {
            output.append("', calendar definition  '").append(next.getValue().getCalendarDefinition());
          }
        }
