...
public class ListThreadInfo implements CommandExecution {
  ...
  public void execute(AllArgs allArgs, CommandLineWriter clw) {
    ...
    for (ThreadInfo ti : tis) {
      if (ti != null) { //null if thread is not active any more
        ...
        if (showStackSize) {
          //stacksize von threads sind typischerweise immer gleich (vgl listsysteminfo) - ausser wenn man sie bei der threaderstellung spezifisch ab�ndert!
  ...
  private String getThreadInfo(ThreadInfo ti) {
    //FIXME code aus java6 ThreadInfo.toString() geklaut. in java5 ist toString() sehr d�rftig.
    //leider ist in java6 toString die gr��e des ausgegebenen stacks auf 8 begrenzt.
    StringBuilder sb =
