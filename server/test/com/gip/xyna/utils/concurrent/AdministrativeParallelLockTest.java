...
public class AdministrativeParallelLockTest extends TestCase {
  ...
  public void testWaitForHashCollision() throws Exception {
    ...
    ParallelLockTestHelper.sleep(100);
    long start = System.currentTimeMillis();
    assertEquals(false, apl.tryLock(new A(1, 2))); //hashCollision! kommt sofort zur�ck
    ParallelLockTestHelper.assertSimilar(System.currentTimeMillis(), start, 5);

    assertEquals(false, apl.tryLock(new A(1, 1))); //kommt sofort zur�ck, weil identisches objekt bereits gelockt ist
    ParallelLockTestHelper.assertSimilar(System.currentTimeMillis(), start, 5);
    ...
    ParallelLockTestHelper.sleep(100);
    ParallelLockTestHelper.assertSimilar(lockTime2.get(), start + 200, 5); //nach den beiden unlocks wird das lock f�r den anderen thraed frei
