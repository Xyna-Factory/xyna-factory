...
import com.gip.xyna.xact.triggerv6.tlvdecoding.dhcpv6.decoders.DUIDLLDHCPv6TlvDecoder;





public final class DUIDLLDHCPTlvDecoderFactoryTest {

    @Test
    public void testSuccessfulNormalUse() {
        DHCPv6TlvDecoderFactory decoderFactory = new DUIDLLDHCPv6TlvDecoderFactory();
        assertEquals("DUIDLL", decoderFactory.getDataTypeName());
        assertEquals(decoderFactory.getDataTypeName(), DUIDLLDHCPv6TlvDecoderFactory.DATA_TYPE_NAME);
        DHCPv6TlvDecoder decoder = decoderFactory.create(0, "Foo", new ArrayList<DHCPv6TlvDecoder>(),null);
        assertTrue(decoder instanceof DUIDLLDHCPv6TlvDecoder);
        ...
        subTlvDecoders.add(new DUIDLLDHCPv6TlvDecoder(56, "Qux", new ArrayList<DHCPv6TlvDecoder>()));
    ...
    public void testFailsCreateWithTypeEncodingTooLow() {
        new DUIDLLDHCPv6TlvDecoderFactory().create(-1, "Foo", new ArrayList<DHCPv6TlvDecoder>(),null);
    }

//    @Test (expected = IllegalArgumentException.class)
//    public void testFailsCreateWithTypeEncodingTooHigh() {
//        new DUIDLLDHCPv6TlvDecoderFactory().create(70000, "Foo", new ArrayList<DHCPv6TlvDecoder>(),null);
//    }

    @Test (expected = IllegalArgumentException.class)
    public void testFailsCreateWithTypeNameNull() {
        new DUIDLLDHCPv6TlvDecoderFactory().create(123, null, new ArrayList<DHCPv6TlvDecoder>(),null);
    }

    @Test (expected = IllegalArgumentException.class)
    public void testFailsCreateWithSubTlvDecodersNull() {
        new DUIDLLDHCPv6TlvDecoderFactory().create(123, "Foo", null,null);
    }
}
