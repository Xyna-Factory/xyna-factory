...
public final class DoNothingTlvEncoder extends AbstractTypeOnlyTlvEncoder {
    ...
    public DoNothingTlvEncoder(int typeEncoding) {
        super(typeEncoding);
    }

    @Override
    protected void writeTypeOnlyNode(TypeOnlyNode node, OutputStream target) throws IOException {
        if (node.getSubNodes().size() > 0) {
            throw new IllegalArgumentException("Expected no sub nodes for node: <" + node + ">.");
        }
    }
}
