...
public final class ProvServerTlvEncoderFactory extends AbstractTypeWithValueTlvEncoderFactory {

    @Override
    protected TlvEncoder createTypeWithValueTlvEncoder(final int typeEncoding, final Map<String, String> arguments) {
      if (arguments.size() > 0) {
        throw new IllegalArgumentException("Expected no arguments when creating <ProvServer> encoder, but got: <"
                + arguments + ">.");
      }
        return new ProvServerTlvEncoder(typeEncoding);
    }
