 ...
 */
public final class DHCPv6ValueOnlyReader implements TlvReader {

  private static Logger logger = Logger.getLogger(DHCPv6ValueOnlyReader.class);
  ...
  public DHCPv6ValueOnlyReader(final InputStream inputStream, final Set<Integer> paddingTypeEncodings) {
    if (inputStream == null) {
...
//                                           "Padding type encodings contains an invalid type encoding: <" + typeEncoding + ">.");
  ...
  private int readLength() throws TlvReaderException {
    int length = readByteValue() * 256 + readByteValue(); // L�ngenfeld 2 Byte lang
