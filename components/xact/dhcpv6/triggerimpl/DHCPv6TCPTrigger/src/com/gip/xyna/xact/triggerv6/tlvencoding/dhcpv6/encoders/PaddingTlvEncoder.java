 ...
 * Padding TLV encoder.
 */
public final class PaddingTlvEncoder extends AbstractTypeOnlyTlvEncoder {

    public PaddingTlvEncoder(final int typeEncoding) {
        super(typeEncoding);
    }
    ...
    protected void writeTypeOnlyNode(final TypeOnlyNode node, final OutputStream target) throws IOException {
        if (node.getSubNodes().size() > 0) {
            throw new IllegalArgumentException("Expected no sub nodes for type <Padding>, but got: "
                + node.getSubNodes() + ">.");
        }
