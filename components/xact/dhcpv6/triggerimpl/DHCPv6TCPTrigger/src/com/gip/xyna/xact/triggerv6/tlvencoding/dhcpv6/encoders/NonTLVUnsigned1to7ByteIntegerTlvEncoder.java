 ...
 */
public final class NonTLVUnsigned1to7ByteIntegerTlvEncoder extends AbstractTypeWithValueTlvEncoder {

  private static final Pattern VALUE_MATCHES_NUMBER_FORMAT_PATTERN = Pattern.compile("([1-9][0-9]*)?[0-9]");
  ...
  private final int nrBytes;
    ...
    public NonTLVUnsigned1to7ByteIntegerTlvEncoder(final int typeEncoding, final int nrBytes, final Long minValue,
  ...
  protected void writeTypeWithValueNode(final TypeWithValueNode node, final OutputStream target) throws IOException {
    ...
    if (!VALUE_MATCHES_NUMBER_FORMAT_PATTERN.matcher(value).matches()) {
      throw new IllegalArgumentException("Invalid unsigned integer value: <" + value + ">, for node: <" + node + ">.");
    }
    long unsignedInt;
    try {
      ...
      throw new IllegalArgumentException("Invalid unsigned integer value (out of range): <" + value + ">.", e);
    }
    ...
    // target.write(this.nrBytes); // Kein TLV f�r DHCPv6
