 ...
 */
public final class PaddingTlvEncoderFactory extends AbstractTypeOnlyTlvEncoderFactory {
    ...
    protected TlvEncoder createTypeOnlyTlvEncoder(final int typeEncoding, final Map<String, String> arguments,
        ...
        if (arguments.size() > 0) {
            throw new IllegalArgumentException("Expected no arguments, but got: " + arguments + ">.");
        } else if (subNodeEncodings.size() > 0) {
            throw new IllegalArgumentException("Expected no sub node encodings, but got: "
                    + subNodeEncodings + ">.");
        }
        return new PaddingTlvEncoder(typeEncoding);
    }
}
