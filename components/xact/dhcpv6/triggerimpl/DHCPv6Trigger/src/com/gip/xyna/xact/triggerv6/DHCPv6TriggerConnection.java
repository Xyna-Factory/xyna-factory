...
public class DHCPv6TriggerConnection extends TriggerConnection {

  private static final long serialVersionUID = -4844846661721673746L;

  private static Logger logger = CentralFactoryLogging.getLogger(DHCPv6TriggerConnection.class);
...
*/
  public DHCPv6TriggerConnection(DatagramPacket packet, DHCPv6ConfigurationDecoder d, DHCPv6ConfigurationEncoder e, int r, int lqr, DatagramSocket t, String mac) {
    this.rawPacket = packet;
    this.dec = d;
    this.enc = e;
    this.replyport=r;
  ...
  public void sendUDP(String targetaddress, byte[] payload, boolean leasequery)
  {
    try
    {
      InetAddress ia; 
      ...
      {
        logger.debug("("+this.getClientMac()+") Sending message to "+ia.getHostAddress()+":"+port);
    ...
    {
      logger.warn("Error when sending UDP Packet: "+e);
    }
