...
public class DHCPv6StartParameter implements StartParameter {

  private static Logger logger = CentralFactoryLogging.getLogger(DHCPv6StartParameter.class);
  ...
  public DHCPv6StartParameter() {
  }


  public DHCPv6StartParameter(InetAddress localAddress, int[] remotePorts, String servermacaddress) {
  ...
  public StartParameter build(String... args) throws XACT_InvalidStartParameterCountException,
    ...
    InternetAddressBean iab =
        XynaFactory.getInstance().getFactoryManagement().getXynaFactoryControl().getNetworkConfigurationManagement()
            .getInternetAddress(localAddress, null);
    if (iab != null) {
      ip = iab.getInetAddress();
    } else {
      if (logger.isInfoEnabled()) {
        logger.info("address " + localAddress + " unknown in network configuration management.");
      }

      try {
        ip = InetAddress.getByName(localAddress);

        if (!(ip instanceof Inet6Address) && logger.isDebugEnabled()) {
    ...
    return new DHCPv6StartParameter(ip, remotePorts, servermacaddress);
  ...
  public String[][] getParameterDescriptions() {
    return new String[][] {{
        "The local ip address to listen on or the name of one definied in NetworkConfigurationManagement.",
