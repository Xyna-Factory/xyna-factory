 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
package xdnc.dhcpv6.communications.configfilegenerator;
...
public class ConfigfileGeneratorSender implements StatisticsReporterLegacy {

  private ThreadPoolExecutor tpe = new XynaThreadPoolExecutor(1, 1, 20, TimeUnit.SECONDS,
                                                              new LinkedBlockingQueue<Runnable>(500),
                                                              Executors.defaultThreadFactory(),
                                                              "ConfigFileGeneratorSender", true);

  private volatile Socket socket;
  private OutputStream out;
  private static final Logger logger = CentralFactoryLogging.getLogger(ConfigfileGeneratorSender.class);
  ...
  private class SendRunnable extends XynaRunnable {
    ...
    public void tryTransmission() throws Exception {
      if (socket != null) {
        ...
        if (logger.isDebugEnabled())
          logger.debug("No Socket! Trying to make socket ...");
        socket = new Socket(InetAddress.getLocalHost(), 2600);
        out = socket.getOutputStream();
        if (socket == null) {
          if (logger.isDebugEnabled())
            logger.debug("Could not create Socket! Got no connection to ConfigfileGenerator!");
        }
        else {
          sendPacket();
        }
    ...
    public void run() {
      try {
        if (logger.isDebugEnabled())
          logger.debug("Ringbuffer component sending packet ...");
        ...
        if (logger.isDebugEnabled())
          logger.debug("Exception while trying to send packet to ConfigfileGenerator: ", e);
        if (logger.isDebugEnabled())
        ...
        try {
          ...
          logger.warn("Could not establish connection to ConfigfileGenerator! Packet discarded! Exception: " + e);
  ...
  public ConfigfileGeneratorSender() {
    // addStatistics
    XynaFactory.getInstance().getFactoryManagement().getXynaStatisticsLegacy()
                    .registerNewStatistic("ConfigfileGeneratorSender", this);
    try {
      socket = new Socket(InetAddress.getLocalHost(), 2600);
      out = socket.getOutputStream();
  ...
  public void shutdown() {
    ...
    try {
      ...
      if (logger.isDebugEnabled())
        logger.debug("Socket did not close correctly: " + e.getStackTrace());
    }

    // removeStatistics
    XynaFactory.getInstance().getFactoryManagement().getXynaStatisticsLegacy().unregisterStatistics("ConfigfileGeneratorSender");
  }
   ...
   */
  public void sendToConfigfileGenerator(byte[] packetcontent) {
    if (logger.isDebugEnabled())
      logger.debug("trying to send packet to ConfigfileGenerator (in ringbuffer) ...");

    try {
      if (packetcontent != null)
        tpe.execute(new SendRunnable(packetcontent));
    }
    catch (Exception e) {
      logger.warn("Error creating new ConfigfileGenerator Sender! Packet discarded! Exception: " + e);
    }
  }
