 ...
 * sequentiell so oft wie gew�nscht ein runnable in dem thread mittels {@link #execute(Runnable)} ausgef�hrt werden.
 * sequentiell bedeutet in diesem fall, dass erneut {@link #execute(Runnable)} aufgerufen werden kann, sobald das
 * vorherige Runnable ausgef�hrt worden ist. Dies kann man z.B. dadurch erkennen, dass der FinishedListener aufgerufen
 * wird. <br>
 * TODO:<br>
 * - fehlerbehandlung (was wenn eines der runnables fehler wirft?) <br>
 * - synchronisierung (was, wenn man execute aufruft, w�hrend der thread bereits in der ausf�hrung begriffen ist <br>
 * - zugrundeliegenden thread lazy starten, sobald das erste mal execute aufgerufen wird <br>
 * - thread nicht suspenden (wait), wenn eh gleich der n�chste request kommt. evtl ist es performanter, ihn dann aktiv warten zu lassen oder mit kurzen sleeps?
 */
        ...
        synchronized (lock) {
          if (toExecute == null && alive) {
            try {
              ...
              // Das ist m�glicherweise schon das, was man hier erwartet.
            }
          }
        }
      }
