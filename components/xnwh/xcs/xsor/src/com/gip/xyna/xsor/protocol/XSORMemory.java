...
public class XSORMemory implements Interconnectable {
  ...
  private void lockObject(int objectIndex, boolean forceUnlock) {
    ...
    while (!lock.compareAndSet(objectIndex, 0, ownTid)) {
      ...
      if (lockedBy != tid) {
        if (lockedBy != 0) {
          //jemand anderes hat das lock als vorher -> es geht vorw�rts -> gleich wieder probieren
          sleepInMs = 0;
        }
        lockedBy = tid;
      }
      ...
      synchronized (mergeStateS) {
        if (mergeStateS.containsKey(key) && mergeStateI.containsKey(key)) {
          ...
          if (insertAtHeadOfQueue) {
            //dann wird auch SIS->S gemerged, aber das bereits in der queue vorhandene objekt ist das �berbleibende objekt, weil es neuer ist
            byte[] tmp = oldMsg;
            oldMsg = ret;
            ret = tmp;
          ...
          //evtl hat der alte �bergang nach S eine payload gehabt und der neue nicht => �bertragen
          if (!payloadNeeded && oldMsg.length > sizeTilPayload) {
            //altes bytearray wiederverwenden, das hat die richtige l�nge. einfach die ersten sizeTilPayload bytes �berschreiben
            byte[] tmp = ret;
            ret = oldMsg;
          ...
          merged = true;
          //Wegen Clusterzustands�bergang sollte im Merging Mode keiner auf einen Eintrag warten
  ...
  private int sendSpecialMessage(char specialChar) {
    byte[] ret = new byte[1 + 4 + 4]; //nur state als kennzeichen f�r SYNC-Message, type f�r dispatching und corrId f�r correlation.
    int transactionCorrId = getNextCorrId();
  ...
  public long getReleaseTime(int objectIndex) {
    //releasetime gibts nicht extra f�r writecopy, weil beim releasen das backup ja verschwindet
    //es schadet aber auch nichts, getReleaseTime mit dem writecopy index aufzurufen
    return releaseTime[objectIndex];
  }

  /**
   * gibt neuen index gelockt zur�ck, falls newIndex != oldIndex => muss noch geunlockt werden.
   */
    ...
    for (Object pk : pkIndex.keySet()) {
      ...
      if (objectIndex > -1) {
        ...
        try {
          ...
          //ist lokaler zustand E || M, so f�hre f�r diese objekte ein payload-rollback auf das backup durch)
          ...
          switch (state) {
            ...
            case 'E' :
              ...
              //objekt ist gerade in bearbeitung, aber evtl wird keine message vom anderen thread erstellt, die die payload enth�lt.
            ...
            case 'M' :
              ...
              //eingestellt.
              //ausser man ist hier gerade in disc_master. dann kommt aber wohl gleich sync und dann wird das objekt erneut �berpr�ft.
              // (bei �bergang von disc_master -> sync wird wieder addAllObjects aufgerufen)
            case 'N' :

              //state == 'N' -> ein anderer thread hat das objekt ge�ndert oder gel�scht und ihm ggfs einen neuen index gegeben.
  ...
  public void waitForQutgoingMessages() throws InterruptedException {
    ...
    Lock queueLock = outgoingQueue.getLock(); //locken, damit in der zeit keine nachrichten eingestellt oder entnommen werden k�nnen
    queueLock.lock();
    try {
      if (outgoingQueue.size() == 0) {
        //nun wei� man nicht, wieviele messages gerade unterwegs sind. man will warten, bis das letzte reply zur�ck ist.
    ...
    try {
      latch.await(); //wird unterbrochen, falls sich der clusterstate �ndert durch ein interrupt auf den thread (siehe cachecoherenceimpl)
