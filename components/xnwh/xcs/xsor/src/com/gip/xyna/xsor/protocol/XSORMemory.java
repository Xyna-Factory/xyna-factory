...
public class XSORMemory implements Interconnectable {
  ...
  public XSORMemory(int recordSize, int maxCount, int objectType, String name, boolean winner, XSORPayload example,
      IndexManagement indexManagement) {
  ...
  public String toString() {
    ...
    sb.append("XSORMemory: ").append(this.name)
        .append(" with size ").append(this.size)
        .append(" for ").append(this.example.getClass().getName())
        .append(" with size ").append(this.recordSize)
        .append(" and ").append(this.recordsPerChunk).append(" records per chunk");
  ...
  public int getInt(int objectIndex, int offset) {
    ...
    return ((data[startIndex] & 0x000000FF) << 24) + ((data[startIndex + 1] & 0x000000FF) << 16)
        + ((data[startIndex + 2] & 0x000000FF) << 8) + (data[startIndex + 3] & 0x000000FF);
  }
  ...
  public long getLong(int objectIndex, int offset) {
    ...
    int offsetBytes = getOffsetBytes(objectIndex);
    int startIndex = offsetBytes + offset;
    return ((data[startIndex] & 0x00000000000000FFl) << 56) + ((data[startIndex + 1] & 0x00000000000000FFl) << 48)
        + ((data[startIndex + 2] & 0x00000000000000FFl) << 40) + ((data[startIndex + 3] & 0x00000000000000FFl) << 32)
        + ((data[startIndex + 4] & 0x00000000000000FFl) << 24) + ((data[startIndex + 5] & 0x00000000000000FFl) << 16)
        + ((data[startIndex + 6] & 0x00000000000000FFl) << 8) + (data[startIndex + 7] & 0x00000000000000FFl);
  }
  ...
  public void setString(int objectIndex, int offset, String value) {// auf maximallaenge beschr�nken
    lockObject(objectIndex);
    try {
      if (state[objectIndex] == 'S' || state[objectIndex] == 'I') {
        return;
  ...
  void setState(int objectIndex, char s) {
    if (logger.isTraceEnabled()) {
      logger.trace(
          "setState '" + s + "'' @" + String.valueOf(objectIndex) + " and " + String.valueOf(writeCopy[objectIndex]));
      logger.trace("current stack", new Exception("current stack"));
    }
    state[objectIndex] = s;
  ...
  private void lockObject(int objectIndex, boolean forceUnlock) {
    ...
    // Der int Cast ist okay, falls ownTid!=0 . Das Eintragen einer (int)ThreadId
    // satt z.B. 1 erleichert evtl. mal Debugging.
    ...
    while (!lock.compareAndSet(objectIndex, 0, ownTid)) {
      ...
      if (toTimeout <= 0) {
        if (forceUnlock) {
          ...
          throw new RuntimeException(
              "Probable deadlock detected, have waited " + timeOutInS + " seconds for lock. Abort.");
        }
      }
      ...
      if (lockedBy != tid) {
        if (lockedBy != 0) {
          // jemand anderes hat das lock als vorher -> es geht vorw�rts -> gleich wieder
          // probieren
          sleepInMs = 0;
        }
        lockedBy = tid;
  ...
  public void clear(int objectIndex) {
    if (logger.isTraceEnabled()) {
      logger.trace("clear for " + String.valueOf(objectIndex));
    }
    byte[] data = getData(objectIndex);
  ...
  long updateChecksumAndModificationtime(int objectIndex) {
    if (logger.isTraceEnabled()) {
      logger.trace("updateChecksumAndModificationtime for " + String.valueOf(objectIndex));
      logger.trace("current stack", new Exception("current stack"));
    }
  ...
  void updateChecksumAndModificationtimeAndReleaseTime(int objectIndex, long modificationTime, long releaseTimeNew) {
    if (logger.isTraceEnabled()) {
      logger.trace("updateChecksumAndModificationtimeAndReleaseTime for " + String.valueOf(objectIndex));
    }
    objectIndex = writeCopy[objectIndex];
  ...
  private int sendMessage(Object objectId, int objectIndex, char newState, boolean payloadNeeded,
      boolean insertAtHeadOfQueue) {
      ...
      synchronized (mergeStateS) {
        if (mergeStateS.containsKey(key) && mergeStateI.containsKey(key)) {
          ...
          if (insertAtHeadOfQueue) {
            // dann wird auch SIS->S gemerged, aber das bereits in der queue vorhandene
            // objekt ist das �berbleibende objekt, weil es neuer ist
            byte[] tmp = oldMsg;
            oldMsg = ret;
            ret = tmp;
            // value von alter nachricht! z.b. wenn bei sync_master ein grab-release
            // passiert, welches keine payload verschickt
          ...
          // evtl hat der alte �bergang nach S eine payload gehabt und der neue nicht =>
          // �bertragen
          if (!payloadNeeded && oldMsg.length > sizeTilPayload) {
            // altes bytearray wiederverwenden, das hat die richtige l�nge. einfach die
            // ersten sizeTilPayload bytes �berschreiben
            byte[] tmp = ret;
            ret = oldMsg;
          ...
          merged = true;
          // Wegen Clusterzustands�bergang sollte im Merging Mode keiner auf einen Eintrag
          // warten
        } else if (!mergeStateS.containsKey(key) && !mergeStateI.containsKey(key)) {
          mergeStateS.put(key, ret);
        } else if (mergeStateS.containsKey(key)) {
          // nur S vorhanden -> kann passieren, wenn durch die synchronisierung
          // S-nachrichten eingestellt werden
          // beispiel:
          ...
          // zum mergen fehlt die I nachricht, einfach die neuere der beiden S nachrichten
          // behalten
          if (insertAtHeadOfQueue) {
        ...
        // else nur I vorhanden -> sollte nicht vorkommen, weil I nur eingetragen wird,
        // nachdem S vorhanden ist
      }
  ...
  /**
   * verschickt spezial-nachricht, die in
   * {@link #processIncommingRequest(byte[], Replyable)} auch separat behandelt
   * wird.
   */
  private int sendSpecialMessage(char specialChar) {
    byte[] ret = new byte[1 + 4 + 4]; // nur state als kennzeichen f�r SYNC-Message, type f�r dispatching und corrId
                                      // f�r correlation.
    int transactionCorrId = getNextCorrId();
  ...
  /**
   * finde dasjenige byte[], welches f�r den objectIndex zust�ndig ist.
   */
  ...
  int releaseBackup(int objectIndex) {
    if (logger.isTraceEnabled()) {
      logger.trace("releaseBackup for " + String.valueOf(objectIndex));
    }
  ...
  void updateReleaseTime(Integer objectIndex, long releaseTimeNew) {
    if (logger.isTraceEnabled()) {
      logger.trace("updateReleaseTime for " + String.valueOf(objectIndex));
      logger.trace("current stack", new Exception("current stack"));
    }
  ...
  void copyForWrite(Integer objectIndex) {
    ...
    System.arraycopy(getData(objectIndex), getOffsetBytes(objectIndex), getData(newIndex), getOffsetBytes(newIndex),
        recordSize);
  ...
  public long getReleaseTime(int objectIndex) {
    // releasetime gibts nicht extra f�r writecopy, weil beim releasen das backup ja
    // verschwindet
    // es schadet aber auch nichts, getReleaseTime mit dem writecopy index
    // aufzurufen
    return releaseTime[objectIndex];
  }

  /**
   * gibt neuen index gelockt zur�ck, falls newIndex != oldIndex => muss noch
   * geunlockt werden.
   */
  int setPayloadAndUpdateIndexAndLockNewIndex(Object objectID, int objectIndex, byte[] payload, long modTime,
      long relTime, boolean create) {
    if (logger.isTraceEnabled()) {
      logger.trace("setPayloadAndUpdateIndexAndLockNewIndex for " + String.valueOf(objectIndex));
    }
    int newIndex = objectIndex;
    XSORPayload oldPayload = null;
    if (!create) {// Im Falle create kann der objectIndex weiter verwendet werden, weil er gerade
                  // erst erzeugt wurde
  ...
  public void rollback(int objectIndex) {
    ...
    if (newIndex != objectIndex) {
      if (logger.isTraceEnabled()) {
        logger.trace("rollback for " + String.valueOf(objectIndex) + " freeing up " + String.valueOf(newIndex) );
      }
  ...
  public void updateIndex(int newObjectIndex, XSORPayload newXSORPayload, int oldObjectIndex,
      XSORPayload oldXSORPayload) {
    if (logger.isTraceEnabled()) {
  ...
  public void addAllObjectsToOutgoingQueue(boolean insertAtHeadOfQueue) {
    // f�ge alle objekte in outgoing queue ein
    int cnt = 0;
    for (Object pk : pkIndex.keySet()) {
      ...
      if (objectIndex > -1) {
        ...
        try {
          ...
          // ist lokaler zustand E || M, so f�hre f�r diese objekte ein payload-rollback
          // auf das backup durch)
          ...
          switch (state) {
            ...
            case 'E':
              ...
              // objekt ist gerade in bearbeitung, aber evtl wird keine message vom anderen
              // thread erstellt, die die payload enth�lt.
              // ->remote mit S und payload anlegen
              sendMessage(pk, objectIndex, 'S', true, insertAtHeadOfQueue);
              cnt++;
              break;
            case 'M':
              // state = 'M' gerade arbeitet ein thread auf diesem objekt, d.h. wenn er fertig
              // ist, wird es eh in die queue
              // eingestellt.
              // ausser man ist hier gerade in disc_master. dann kommt aber wohl gleich sync
              // und dann wird das objekt erneut �berpr�ft.
              // (bei �bergang von disc_master -> sync wird wieder addAllObjects aufgerufen)
            case 'N':

              // state == 'N' -> ein anderer thread hat das objekt ge�ndert oder gel�scht und
              // ihm ggfs einen neuen index gegeben.
              // a) update -> anderer thread verschickt message mit payload -> hier ist nichts
              // zu tun.
              // b) delete -> kann nur als disc_master passieren, dann kann man das einfach
              // ignorieren, weil dann keine nachricht
  ...
  public void waitForQutgoingMessages() throws InterruptedException {
    ...
    Lock queueLock = outgoingQueue.getLock(); // locken, damit in der zeit keine nachrichten eingestellt oder entnommen
                                              // werden k�nnen
    queueLock.lock();
    try {
      if (outgoingQueue.size() == 0) {
        // nun wei� man nicht, wieviele messages gerade unterwegs sind. man will warten,
        // bis das letzte reply zur�ck ist.
    ...
    try {
      latch.await(); // wird unterbrochen, falls sich der clusterstate �ndert durch ein interrupt auf
                     // den thread (siehe cachecoherenceimpl)
        ...
        switch (state) {
          ...
          case 'I':
            if (mergeStateS.containsKey(objectBigInteger) && !mergeStateI.containsKey(objectBigInteger)) {
              ...
              logger.error(
                  "error while merging found unexpectedly state I" + " objectId=" + XSORUtil.prettyPrint(objectId));
              mergeStateS.remove(objectBigInteger);
              mergeStateI.remove(objectBigInteger);
            }
            break;
  ...
  public String toString(int backupIndex, boolean skipData) {
    ...
    if (backupIndex == objectIndex) {
      ...
      if (!skipData) {
        sb.append("\nbackup-data = ")
            .append(XSORUtil.prettyPrint(getData(backupIndex), getOffsetBytes(backupIndex), recordSize));
      }
      sb.append("\nbackup-modtime = ").append(getModificationTime(backupIndex));
      sb.append("\nbackup-reltime = ").append(getReleaseTime(backupIndex));
      sb.append("\nbackup-modtimeISO = ").append(XSORUtil.timestampToISO(getModificationTime(backupIndex)));
      sb.append("\nbackup-reltimeISO = ").append(XSORUtil.timestampToISO(getReleaseTime(backupIndex)));
    }
    ...
    if (!skipData) {
      sb.append("\ndata = ")
          .append(XSORUtil.prettyPrint(getData(objectIndex), getOffsetBytes(objectIndex), recordSize));
    }
    sb.append("\nmodtime = ").append(getModificationTime(objectIndex));
    sb.append("\nreltime = ").append(getReleaseTime(objectIndex));
    sb.append("\nmodtimeISO = ").append(XSORUtil.timestampToISO(getModificationTime(objectIndex)));
    sb.append("\nreltimeISO = ").append(XSORUtil.timestampToISO(getReleaseTime(objectIndex)));
    return sb.toString();
  }
