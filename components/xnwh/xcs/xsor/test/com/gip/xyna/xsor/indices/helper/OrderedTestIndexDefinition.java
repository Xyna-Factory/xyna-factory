...
public class OrderedTestIndexDefinition extends OrderedIndexDefinition<TestObject> {

  public OrderedTestIndexDefinition(String tableName, String[] indexedColumns) {
    super(tableName, indexedColumns);
  }


  @Override
  public OrderedIndexKey createIndexKey(TestObject e) {
    return new SearchValueBasedOrderedIndexKey(e.generateSearchValuesFromColumnNames(getIndexedColumnsInOrder()));
  }

  
  //copy of StorableBasedOrderedIndexDefinition because we don't know it down here
  @Override
  public OrderedIndexSearchCriterion createIndexSearchCriterion(SearchCriterion criterion, SearchParameter parameter) {
    ...
    OrderedIndexKey rangeStart = new SearchValueBasedOrderedIndexKey(startKeyValues.toArray(new SearchValue[startKeyValues.size()]), KeyType.START, startInclusive,
                                                                     startKeyValues.size() == getIndexedColumnNamesInOrder().length ? false : true);
    OrderedIndexKey rangeStop = new SearchValueBasedOrderedIndexKey(stopKeyValues.toArray(new SearchValue[stopKeyValues.size()]), KeyType.STOP, stopInclusive,
                                                                    stopKeyValues.size() == getIndexedColumnNamesInOrder().length ? false : true);
