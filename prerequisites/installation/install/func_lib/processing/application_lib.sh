...
# Eintrag in die Verwaltungs-Listen; Prüfung dass kein Abh�ngigkeits-Zirkel auftritt
#
# Kommunikation mit Aufrufer über folgende Variablen:
# * REQ_APP_FILE_LIST    #erforderte Apps
# * FAILED_APP_LIST      #Apps mit Fehler: nicht gefunden
...
f_import_applications_internal_required () {
  ...
  #aktuelle App in REQUIRING_APP_LIST eintragen, um Zirkel erkennen zu k�nnen 
  REQUIRING_APP_LIST=$(f_add_to_list REQUIRING_APP_LIST ${APP_FILE});
  
  local REQ_APP_FILE;
  for REQ_APP_FILE in ${REQ_APP_FILE_LIST} ; do
    #evtl in Gesamt-Liste l�schen, damit nicht doppelt
    APP_LIST=$(f_remove_from_list APP_LIST ${REQ_APP_FILE});
    #Requirement vorne eintragen, damit als n�chstes ausgewertet
...
f_import_application () {
  ...
  # tail -1 gibt nur die h�chste Versionsnummer zurück, weil die Ausgabe von listapplications bereits sortiert ist. Migration von �lteren Applications muss manuell erfolgen
