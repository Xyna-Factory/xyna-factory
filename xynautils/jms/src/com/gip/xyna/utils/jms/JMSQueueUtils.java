...
/**
 * ziel:<br>
 * jms wrapper klasse, die leicht zu bedienen ist und trotzdem flexibel gegen�ber neuen anforderungen ist.<br>
 *
 * enqueue und dequeue methoden werden unterst�tzt.<br>
 * codebeispiele:<br>
 * <code>
 ...
 * public JMSQueueUtils(String jdbcUrl, String dbUser, String dbPassword,
   ...
   datenbank-connections werden nicht geschlossen, wenn die connection im konstruktor �bergeben wird,
   allerdings wirken sich rollback() und commit() nicht nur auf die jms-connection, sondern (nat�rlich)
   auch auf die db-connection aus.
 * <p>
 * wie funktioniert jms?<br>
 * 1. queue connection holen<br>
 *    das funktioniert meist �ber eine connectionfactory. implementierungen von connectionfactories
 *    sind zb standardm�ssig im applicationserver �ber JNDI zu finden. im hintergrund h�ngt im AQ
 ...
 * 2. queue session er�ffnen<br>
 *    �ber die queue connection<br>
 * 3. queue objekt bauen<br>
 *    siehe setQueue()<br>
 * 4. queue sender/receiver<br>
 *    die session stellt meist methoden zur verf�gung um sender/receiver objekte zu erstellen. diese
 *    erm�glichen dann das eigentliche enqueue oder dequeue.
 * <p>
 * TODO: topics, falls ben�tigt. <br>
 *       usePLSQL f�r bessere abw�rtskompatibilit�t mit �lteren aqjms-libs/db-versionen implementieren.<br>
...
public class JMSQueueUtils {
  ...
  /**
   * beim browsen werden Nachrichten f�r andere Consumer gelockt. �hnlich einem Select for update.
  ...
  /**
   * baut QueueConnection und QueueSession auf. Transactions-Modus ist defaultm�ssig aus.
   * @param ctx
   * @param jndiQConnectionFactory wird �ber einen lookup im InitialContext versucht zu finden.
   ...
   * Transaktions-Modus defaultm�ssig ausgeschaltet.<br>
   ...
   * @param dbUser benutzername f�r die datenbank verbindung
   * @param dbPassword password f�r die datenbank verbindung
   * @param transacted
   * @throws JMSException
   */
  public JMSQueueUtils(String jdbcUrl, String dbUser, String dbPassword,
   ...
   * Message �bernommen. Falls sie null ist, wird die in dieser Klasse gesetzte CorrelationId benutzt.<br>
   ...
   * den MessageSelector benutzen zu m�ssen:<br>
   * <code>
   * JMSQueueUtils jms = new JMSQueueUtils(...);<br>
  ...
  /**
   * gibt die verwendete QueueSession zur�ck. Mit dieser kann man zb Message oder Queue oder
  ...
  /**
   * gibt die verwendete QueueConnection zur�ck.
   * @return
   */
  ...
  /**
   * Welche Nachrichten sollen dequeued werden? Format ist SQL-�hnlich:<br>
   * <code>
   ...
   * </code>
   * <p>
   * Hierbei gibt es zu beachten, dass die Validit�t/Syntax von MessageSelektoren abh�ngig von der
  ...
  /**
   * mit dieser Priority wird enqueued. Kleiner 0 f�r Default priority (implementierungsabh�ngig)
   * @param prio
   */
  ...
  /**
   * generischer Queue-setter. Das Queue-Objekt bekommt man meistens �ber die Session. Meistens sollten
  ...
  /**
   * Gibt Nachrichten zur�ck, die sich in der Queue befinden, ohne sie zu dequeuen. Sie werden auf
   * Wunsch gelockt (nur bei AQ unterst�tzt), so dass sie niemand anderes auslesen kann, solange bis
   * die QueueSession beendet wird (unklar, ob auch schon bei commit/rollback das lock aufgegeben wird).<br>
   * Dieses Verhalten kann �ber {@link com.gip.xyna.utils.jms.JMSQueueUtils#setLockBrowsedMessages(boolean)}
   ...
   * @param maxMessages Es werden maximal soviele Nachrichten ausgelesen und zur�ckgegeben.
   ...
   * er nicht ber�cksichtigt. Der generierte MessageSelector selektiert nach CorrelationId-Gleichheit
   ...
   * Queue und CorrelationId (optional) m�ssen vorher gesetzt worden sein.<br>
   * enqueued eine TextMessage mit der �bergebenen message als Inhalt.
   * @param message
   * @throws JMSException
   */
   ...
   * setzt die CorrelationId f�r zuk�nftige Operationen.<br>
   * enqueued eine TextMessage mit der �bergebenen message als Inhalt.
   * @param correlationId
   * @param message
   * @throws JMSException
  ...
  /**
   * enqueue Convenience Methode<br>
   * setzt auch die Queue f�r zuk�nftige Operationen.<br>
  ...
  public void setQueueJNDI(String jndiQueueDestination) throws JMSException, NamingException {
    if (aq) {
      throw new JMSException("Die Queue kann nur �ber JNDI bestimmt werden, wenn die QueueConnection auch �ber JNDI besteht.");
    }
  ...
  /**
   * Gibt die erste Nachricht aus der gesetzten Queue zur�ck, die innerhalb des timeouts mit der correlationId
   * gefunden wird. Falls keine gefunden wird, wird <code>null</code> zur�ckgegeben.<br>
 ...
 /* public static void main(String[] args) {
    //junit tests br�uchten eine datenbank und einen application server... TODO mit 10.0.10.11 als junittest
    //1. AQ
  ...
  /**
   * �berschreibt den default logger, der mit Logger.getLogger("xyna.utils.jms") definiert wurde.
   * @param _logger
   */
  ...
  /**
   * in Verbindung mit AQ-spezifischem Konstruktor wird enqueue und dequeue mit PLSQL durchgef�hrt.
