 ...
 * <p>
 * Features:
 * <p>Kodierung beliebig �ber setEncoding(...), zwei vordefinierte Konstanten UTF_8 und ISO_8859_1
 * <p>Protokoll http oder https m�glich
 * <p>SOAP-Version 1.1 oder 1.2<br>
 *
 * In den SOAPUtils kann man diverse haupts�chlich die HTTPConnection betreffende
 * Einstellungen vornehmen (SOAPConnector b�ndelt das). <br>
 * Verwendung in Verkn�pfung mit SoapRequest und SoapResponse. Man erstellt zuerst
 * einen SoapRequest, und ruft damit dann das soapUtils.sendSOAPMessage() auf. Man 
 * bekommt eine SoapResponse zur�ck, auf die man diverse Operationen anwenden kann:
 * <p>Fehler-erkennung
 ...
 * </code><p>
 * 
 * F�r die Benutzung von HTTPS muss das Server-Zertifikat authentifiziert sein.
 * Falls es dies nicht automatisch ist, muss ein keyStore eingerichtet werden
 * (beispielsweise �ber keytool  -import -v -alias gip -file gip.crt ) und im aufrufenden Programm muss
 ...
 * TODO SOAP-Header nur teilweise unterst�tzt
 */
public class SOAPUtils extends SOAPConnector {
  ...
  public String generateSOAPMessage(String message) {
    try {
      ...
      //�ndert und abw�rtskompatibilit�t verloren geht. statt dessen null zur�ck
      //geben
      return null;
    }
  }
  ...
  /**
   * Schneidet den SoapTeil der Message ab, nur der reine Body wird zur�ckgegeben
   *
   * @param message
   * @return
   */
  public static String stripSOAPMessage(String message) { 
    //TODO eigtl nicht mehr ben�tigt, da die Funktion in SoapResponse integriert ist
    String result = null;
    try {
      result =
  ...
  /**
   * �ffnet httpconnection, schliesst sie aber nicht
  ...
  public SoapResponse sendSOAPMessage(SoapRequest req) throws IOException,
    ...
    // Message in ByteArray schreiben, um die Anzahl der Bytes z�hlen zu k�nnen
