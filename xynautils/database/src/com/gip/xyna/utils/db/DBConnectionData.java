...
/**
 * DBConnectionData sammelt alle Daten, die zum �ffnen einer Connection zur DB n�tig sind.
 * Die Methode {@link createSQLUtils} erzeugt eine fertige SQLUtils-Instanz.
 * 
 * Diese Klasse kann sowohl f�r MySQL als auch f�r Oracle-Connections verwendet werden.
 * Der zu verwendende Treiber kann entweder �bergeben werden oder 
...
public class DBConnectionData {
  ...
  public static class DBConnectionDataBuilder {
    ...
    public static Executor DefaultNetworkTimeoutExecutor = Executors.newCachedThreadPool(); //hier als Default, k�nnte konfigurierbar werden..
...
//          props.setProperty("SQLNET.OUTBOUND_CONNECT_TIMEOUT", String.valueOf(""));
      ...
      case MySQL:
      case MARIADB:
        //F�r weitere m�gliche Properties: http://dev.mysql.com/doc/refman/5.0/en/connector-j-reference-configuration-properties.html
        dcd.properties.setProperty("tcpKeepAlive","true");//Tcp Keepalive
        break;
      case Oracle:
        //F�r weitere m�gliche Properties: http://www.orindasoft.com/public/Oracle_JDBC_JavaDoc/javadoc1020/oracle/jdbc/pool/OracleDataSource.html#setConnectionProperties(java.util.Properties)
        break;
      case DB2:
        break;
      case SQLServer:
  ...
  protected Connection createConnectionInternal(String local_url) throws Exception {
    ...
    if (classLoaderToLoadDriver != getClass().getClassLoader()) {
      ...
      //�ber reflection getConnection von classloaderhelper aufrufen, weil um ohne connection aufrufen zu k�nnen, m�sste man 
  ...
  /*
   * cache f�r urlclassloader, die verwendet werden, um jdbc treiber zu laden.
   * der cache ist dazu da, dass nicht f�r mehrfaches createconnection unterschiedliche classloader-instanzen verwendet werden
  ...
  private static URLClassLoader getClassLoader(ClassLoader classLoaderToLoadDriver) {
    ...
    if (result == null) {
      synchronized (classLoaderCache) {
        ...
        if (result == null) {
          //FIXME nicht f�r jeden �bergebenen classloader einen helper-classloader erstellen, wenn 
          //diese classloader eigtl alle das jdbc-loading an einen gemeimsamen anderen classloader delegieren
          //beispiel: mehrere black services delegieren alle das jdbc-laden an einen sharedlib-classloader. dann br�uchte
          //man nur den classloaderhelper f�r den sharedlib-classloader
          ...
          URLClassLoader t =
              new URLClassLoader(((URLClassLoader) classLoaderToGetURLsFrom).getURLs(), classLoaderToLoadDriver) {
                ...
                public synchronized Class<?> loadClass(String name) throws ClassNotFoundException {
                  //synchronized, damit findloadedclass threadsafe wird. sonst kann es passieren, dass f�r die gleiche klasse zweimal
  ...
  public static void closeConnection(Connection connection) {
    ...
    try {
      ...
      //Fehler hier wird unterdr�ckt, da schlecht behandelbar, 
