...
/**
 * InList zum Bau von SQL-Statements und dem zugeh�rigen Parameter-Objekt.
 * Beispiel:<code><pre>
 ...
 * per Default werden bis zu maxLenght=5 Eintr�ge mit Platzhaltern eingetragen, danach 
 * werden die Werte direkt in den SQL-String �bernommen:
<code><pre>
 ...
 * Im Konstruktor kann maxLength ver�ndert werden.
 * <p>
 ...
 * Dies k�nnte dann so aussehen: "(col IN (A,B,C) OR col IN (D,E,F) OR col IN (G))"
 * <p>
 * Die Liste kann Strings enthalten, die dann automatisch mit einfachen Anf�hrungszeichen
 * umschlossen werden, bei anderen Typen wird toString aufgerufen und kein Anf�hrungszeichen gesetzt.
 * <p>
 */
public class InList {
  ...
  private void init(Object[] list, int maxLength ) throws Exception {
    ...
    if( listEmpty ) {
      ...
      boolean isString = (list[0] instanceof String); //nur erstes Element pr�fen, wer mischt ist selbst dran schuld
  ...
  public String getSQL(String colName) {
    if (sql == null) {
      if( orLists == null ) {
        //leere IN-Liste -> ung�ltige Abfrage
        return colName + " <> " + colName;
      } else {
        //zuviele Parameter in IN-Liste -> mehrere IN-Listen durch OR getrennt
        StringBuilder sb = new StringBuilder();
        sb.append("("); //Einklammern, um OR zu sch�tzen
