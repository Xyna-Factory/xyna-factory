...
public class PoolEntryConnection extends SQLInstrumentedWrappedConnection {
  ...
  //zuletzt durchgef�hrtes sql. wird genullt, wenn verbindung neu aufgemacht wird, d.h. ein neuer thread aktiv wird, weil
  //es sonst verwirrend wird, ob der aktive thread zu dem sql geh�rt.
  ...
  /**
   * gibt die Connection zur�ck an den Pool
   */
  public void close() throws SQLException {
    ...
    try {
      ...
      synchronized (this) {
        ...
        try {
          ...
          if (innerConnectionIsOpen) {
            con.rollback();
            String lastSql_t = lastSQL; //nicht �berschreiben durch markConnection
      ...
      pool.returnConnection(this); //stellt die connection wieder zur verf�gung
  ...
  /**
   * Markiert die Connection, so dass sie bei n�chster Gelegenheit geschlossen wird
   */
