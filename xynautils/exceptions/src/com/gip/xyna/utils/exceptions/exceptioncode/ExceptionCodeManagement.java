...
public class ExceptionCodeManagement {
   ...
   * @param start
   * @param length
   * @param padding formatierte l�nge von zahlen.
  ...
  /**
   * gibt pattern zur�ck, falls kein overlap gefunden wurde
    ...
    for (Map.Entry<String, CodeGroup> entry : codeGroups.entrySet()) {
      for (Pattern oldPattern : entry.getValue().patterns) {
        if (oldPattern.padding == p.padding && oldPattern.prefix.equals(p.prefix) && oldPattern.suffix.equals(p.suffix)) {
          //codes sehen �hnlich aus: wie siehts mit den bereichen aus?
          ...
          //3. jetzt kann h�chstens noch passieren, dass das oldPattern vollst�ndig im neuen enthalten ist
