...
/**
 * Checks for occurrence of token type OTHER, i.e. an unknown token.
 *
 */
public final class NoOtherTokensRule implements Rule {

    public void checkForRuleViolations(final TokenBuffer buffer)
            throws ValidationFailureException {
        if (buffer.size() < 1) {
            return;
        }
        Token token = buffer.get(0);
        if (token.getTokenType() == TokenTypes.OTHER) {
            throw new ValidationFailureException("Found an unknown token on line <" + token.getLineNumber()
                    + ">: <" + token.getCode() + ">.");
        }
    }
}
