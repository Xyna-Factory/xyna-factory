...
public class XSORClusterProvider implements ClusterProvider {
  ...
  private static final String EXTENDEDSTATUS_NAME = "XynaCluster"; //ACHTUNG. bei �nderungen auch in networkavailability demon/checkFactoryState.sh �ndern!
  ...
  public void checkInterconnect() {
    //ntbd, interconnect wird von externem prozess �berpr�ft.
  }
  ...
  private void parseAndStoreParameter(String[] startParameters) throws InvalidParametersException,
    ...
    //!!!! ACHTUNG: bei erg�nzung auch in konstante PARAMETER_DESCRIPTION anpassen und oben die parameter-l�nge anpassen!!!
  ...
  public List<Integer> getAllBindingsIncludingLocal() {
    ...
    //             in der Zwischenzeit Objekte dazugekommen sein k�nnten. Alternative: Das zentrale Objekt
    //             wird immer redundant gehalten und es wird ein Trigger an das Objekt geh�ngt, der dann
    //             aktiv hier den Cache updated. Aus Performance-Sicht w�re das sicherlich die bessere Variante.
    return null;
  }
  ...
  public boolean isConnected() {
    // TODO: was genau bedeutet connected hier? wozu gibt es die methode, wenn sie �quivalent zu clsuterstate connected ist? evtl methode rauswerfen?
  ...
  private void createClusterManagement(String hostnameRemote, int portRemote, String hostnameLocal,
    ...
    //�ber futureexecutiontask in xynaclusteringservicesmanagement ist sichergestellt, dass rmimanagement zu dem zeitpunkt bereits initialisiert wurde.
    cmRmiIf =
            ...
            waitingForRemoteBackingstoreThread = new Thread(new Runnable() {
              ...
              public void run() {
                try {
                  ...
                  //abbruch, wenn diese bedingungen gemeinsam erf�llt sind
  ...
  public void restoreClusterConnect() {
    ...
    XynaFactory.getInstance().getFutureExecution()
        .execAsync(new FutureExecutionTask(XynaFactory.getInstance().getFutureExecution().nextId()) {
          ...
          @Override
          public void execute() {
            //f�hrt dazu, dass automatisch versucht wird nach startup zu gehen, wenn das 
  ...
  private void addPropertyChangeListener() {
    ...
    try {
      ...
      //�ber die cli nicht gesetzt werden kann, w�hrend der server hochf�hrt. der wert von
      //vor dem serverstart k�nnte OK sein, was hier dann nicht passend ist.
