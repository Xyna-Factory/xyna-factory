...
public class RemoteInterfaceForClusterStateChangesImplAQ implements RemoteInterfaceForClusterStateChanges, Runnable {
  ...
  /**
   * wartezeit auf antworten, falls �ber aq kommuniziert wird
   */
  ...
  public SQLUtils createSqlUtils(final String info) throws DBNotReachableException {
    ...
    try {
      ...
      if (true) { //TODO konfigurierbar, ob logger oder null �bergeben wird, um loghierarchy steuern zu k�nnen
        localLogger = null;
      }
    ...
    try {
      while (running) {
        try {
          ...
          if (!running) {
            continue; //Thread soll beendet werden, daher zur�ck zum while
          }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          //ist die Antwort doch noch versp�tet eingetroffen?
          try {
    ...
    public void run() {
      ...
      while (!canceled && waitingRefreshed + timeout > now) {
        try {
          ...
          //Exception ingorieren: dann halt k�rzer warten
        }
