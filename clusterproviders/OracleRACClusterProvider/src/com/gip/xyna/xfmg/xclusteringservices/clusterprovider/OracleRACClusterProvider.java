...
/**
 * Cluster von Knoten, die �ber eine Oracle-Datenbank kommunizieren.<br>
 * Das createCluster legt eine Datenbank-Tabelle an, �ber die das Status-Management abgewickelt wird. Alle weiteren
 * hinzugef�gten Knoten tragen sich in dieser Tabelle ein.
 */
public class OracleRACClusterProvider implements ClusterProvider, RemoteInterfaceForClusterStateChanges, DBProblemHandler {
  ...
  private void initInternally() throws DBNotReachableException {
    ...
    dbdata =
        DBConnectionData.newDBConnectionData()
            ...
            .socketTimeoutInSeconds(tsek * 4) //muss gr��er sein als RemoteInterfaceForClusterStateChangesImplAQ.DEQUEUE_TIMEOUT
  ...
  XFMG_ClusterConnectionException {
    ...
    try {
      try {
        ...
        // es sollte schon Eintr�ge geben (sonst Inkonsistenz):
  ...
  public void restoreClusterPrepare(long id) throws XFMG_ClusterInitializationException {
    ...
    try {
      ...
      //dies h�tte nun nicht passieren d�rfen: eine unerwartete Exception ist bei der Benutzung der DB aufgetreten.
      throw new XFMG_ClusterInitializationException(TYPENAME, e);
    }
    ...
    try {
      if (! dbNotReachable.get() ) {
        ...
        try {
          ...
          //Es kann keine connection hergestellt werden. Das Retry wird innen drin schon durchgef�hrt.
  ...
  public void join(SQLUtils sqlUtils, int joinedBinding) {
    ...
    //der andere Knoten m�chte sich connecten, wird ihm das erlaubt?
    //Diese Entscheidung kann l�nger dauern, daher in eigenem Thread
    ...
    if( state == ClusterState.DISCONNECTED_SLAVE ) {
      if( dbNotReachable.compareAndSet(false, true) ) {
        if (initialized) {
          ...
          //ansonsten kann hier nichts weiter getan werden, �ber die mit setException(...) gesetzte Exception 
  ...
  public void checkInterconnect() {
    ...
    //starten k�nnen, ohne dass mehrere Connections gebraucht werden. Au�erdem m�ssen die
    //sp�ter (w�hrend der Wartezeit des ersten Threads) hinzukommenden Threads nicht mehr 
