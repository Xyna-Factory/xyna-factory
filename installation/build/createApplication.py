...
class AppCreator:
  ...
  def copy_template(self, target_dir: str, file_name: str):
    ...
    path_to_template = os.path.join(this_dir, "templates", f"{file_name}.template")
  ...
  def copy_and_set_application_xml(self, appDir: str, appName: str, appVersion: str):
    this_dir = Path(__file__).parent.absolute()
    application_xml_template = os.path.join(this_dir, "templates", "application.xml.template")
  ...
  def createApp(self, projectDir: str, appDir: str, appName: str, appVersion: str):
    ...
    if not os.path.isdir(projectDir):
      print("project directory does not exist. Creating it and placing server.properties in it.")
    ...
    relpath = os.path.relpath(this_dir.parent.parent.absolute(), Path(app_dir).absolute())
